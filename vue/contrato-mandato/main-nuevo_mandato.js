...
var vm = new Vue({
  ...
  data() {
    return {
      ...
      propietarios: [],
      arrendamiento: {
        tipo_solicitud: "Contrato de Mandato",
        tipo_contrato_id: 14,
        empresa_suscribe_id: "",
        supervisor_id: "",
        abogado_id: "",
...
function validar_contrato() {
  ...
  if (!this.arrendamiento.empresa_suscribe_id) {
    alertify.error("Ingrese al menos un empresa suscribe", 5);
    console.log("this.empresa_suscribe_id: ", this.empresa_suscribe_id);
    this.validate_errors = false;
    return false;
  }
  if (this.propietarios.length == 0) {
    alertify.error("Ingrese al menos una mandante", 5);
    this.show_modal_nuevo_propietario();
    this.validate_errors = false;
    return false;
  }
  ...
  for (let index = 0; index < this.contratos.length; index++) {
    ...
    if (!contrato.mandante_antecedente) {
      alertify.error("Agregue los antecedentes del mandante.", 5);
      this.validate_errors = false;
      return false;
    }
    if (!contrato.mandataria_objetivo) {
      alertify.error("Agregue obligaciones de la mandataria.", 5);
      this.validate_errors = false;
      return false;
    }
    if (!contrato.mandataria_retribucion) {
      alertify.error("Agregue los antecedentes de la mandataria.", 5);
      this.validate_errors = false;
      return false;
    }
    if (!contrato.plazo_duracion) {
      alertify.error("Agregue el plazo de duración.", 5);
      this.validate_errors = false;
      return false;
    }
...
function registrar_contrato() {
  ...
  data.append("action", "contrato_mandato/registrar");
