  ...
  mounted() {
    ...
    EventBus.$on("seleccionar-propietario-beneficiario", (data) => {
      ...
      this.beneficiario.tipo_docu_identidad_id =
        beneficiario.tipo_docu_identidad_id;
      ...
      this.tipo_persona_val = this.tipo_persona.find(
        (item) => item.id == beneficiario.tipo_persona_id
      );
      this.tipo_docu_identidad_val = this.tipo_docu_identidad.find(
        (item) => item.id == beneficiario.tipo_docu_identidad_id
      );
      ...
      if (beneficiario.forma_pago_id != null) {
        this.tipo_forma_pago_val = this.tipo_forma_pago.find(
          (item) => item.id == beneficiario.forma_pago_id
        );
      }
      if (beneficiario.banco_id != null) {
        this.banco_val = this.bancos.find(
          (item) => item.id == beneficiario.banco_id
        );
      }
      if (beneficiario.tipo_monto_id != null) {
        this.tipo_monto_val = this.tipo_monto.find(
          (item) => item.id == beneficiario.tipo_monto_id
        );
      }
    });
        ...
        $(me.$refs.monto).on({
          ...
          blur: function (event) {
            ...
            if (parseFloat($(event.target).val().replace(/\,/g, "")) > 0) {
              if (
                parseInt(tipo_monto_id) == 1 ||
                parseInt(tipo_monto_id) == 2
              ) {
                $(event.target).val(
...
function validar_beneficiario() {
  ...
  if (
    parseInt(this.beneficiario.tipo_docu_identidad_id) == 1 &&
    this.beneficiario.num_docu.length != 8
  ) {
    alertify.error(
      "El número de DNI posee 8 dígitos, no " +
        this.beneficiario.num_docu.length +
        " dígitos",
      5
    );
    $("#modal_beneficiario_num_docu").focus();
  ...
  if (
    parseInt(this.beneficiario.tipo_docu_identidad_id) == 2 &&
    this.beneficiario.num_docu.length != 11
  ) {
    alertify.error(
      "El número de RUC posee 11 dígitos, no " +
        this.beneficiario.num_docu.length +
        " dígitos",
      5
    );
  ...
  if (
    this.beneficiario.banco_id.length == 0 &&
    this.beneficiario.forma_pago_id != 3
  ) {
  ...
  if (
    this.beneficiario.num_cuenta_bancaria.length == 0 &&
    this.beneficiario.forma_pago_id != 3
  ) {
  ...
  if (
    this.beneficiario.num_cuenta_bancaria.length < 5 &&
    this.beneficiario.forma_pago_id != 3
  ) {
    alertify.error(
  ...
  if (
    this.beneficiario.num_cuenta_cci.length < 8 &&
    this.beneficiario.forma_pago_id != 3
  ) {
    alertify.error(
  ...
  if (
    parseInt(this.beneficiario.forma_pago_id) != 3 &&
    this.beneficiario.monto.length < 1
  ) {
  ...
  axios({
    ...
    .then(function (response) {
      ...
      if (response.data.status == 200) {
        ...
        me.beneficiarios[index].tipo_persona_id =
          response.data.result.tipo_persona_id;
        me.beneficiarios[index].tipo_docu_identidad_id =
        ...
        me.beneficiarios[index].forma_pago_id =
          response.data.result.forma_pago_id;
        me.beneficiarios[index].banco_id = response.data.result.banco_id;
        me.beneficiarios[index].num_cuenta_bancaria =
          response.data.result.num_cuenta_bancaria;
        me.beneficiarios[index].num_cuenta_cci =
          response.data.result.num_cuenta_cci;
        me.beneficiarios[index].tipo_monto_id =
          response.data.result.tipo_monto_id;
        me.beneficiarios[index].monto = response.data.result.monto;

        me.beneficiarios[index].tipo_persona =
          response.data.result.tipo_persona;
        me.beneficiarios[index].tipo_doc_identidad =
          response.data.result.tipo_doc_identidad;
