...
var vm = new Vue({
  ...
  data() {
    return {
      ...
      propietarios: [],
      arrendamiento: {
        tipo_solicitud: "Contrato de Mutuo de Dinero",
        tipo_contrato_id: 15,
        empresa_suscribe_id: "",
        supervisor_id: "",
        abogado_id: "",
...
function validar_contrato() {
  ...
  if (this.propietarios.length == 0) {
    alertify.error("Ingrese al menos un propietario.", 5);
  ...
  for (let index = 0; index < this.contratos.length; index++) {
    ...
    if (!contrato.mutuante_descripcion) {
      alertify.error("Agregue una descripción para el mutuante.", 5);
      this.validate_errors = false;
      return false;
    }

    if (!contrato.mutuatario_descripcion) {
      alertify.error("Agregue una descripción para el mutuatario.", 5);
      this.validate_errors = false;
      return false;
    }

    if (!contrato.tipo_moneda_id) {
    ...
    if (!contrato.banco_val || !contrato.banco_val.id) {
      alertify.error("Seleccione un banco válido.", 5);
      this.validate_errors = false;
      return false;
    }

    if (!contrato.num_cuenta_bancaria) {
      alertify.error("Ingrese el número de cuenta bancaria.", 5);
      this.validate_errors = false;
      return false;
    }

    if (!contrato.fecha_plazo_limite) {
      alertify.error("Seleccione una fecha de plazo límite.", 5);
      this.validate_errors = false;
      return false;
    }

    if (!contrato.ciudad) {
      alertify.error("Ingrese la ciudad.", 5);
      this.validate_errors = false;
      return false;
    }
    ...
    const fechaPlazoLimite = new Date(contrato.fecha_plazo_limite);
    const fechaSubscripcion = new Date(contrato.fecha_subscripcion);
    if (fechaPlazoLimite <= fechaSubscripcion) {
      alertify.error(
        "La fecha de plazo límite debe ser mayor a la fecha de suscripción.",
        5
      );
      this.validate_errors = false;
      return false;
...
function registrar_contrato() {
  ...
  data.append("action", "contrato_mutuodinero/registrar");
