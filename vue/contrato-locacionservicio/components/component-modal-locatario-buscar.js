Vue.component("component-modal-locatario-buscar", {
  name: "component-modal-locatario-buscar",
  template: `
        <div id="component-modal-buscar-locatario" class="modal" data-keyboard="false">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                        <h4 class="modal-title" id="modal_buscar_locatario_titulo">Buscar Locatario</h4>
                    </div>
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                ...
                                <select class="form-control-lg2" v-model="propietario.tipo_busqueda">
                                    <option value="1">Buscar por nombre de Locatario</option>
                            ...
                            <div class="col-md-5">
                                <div class="form-group">
                                    <input ref="search" type="text" minlength="3" v-model="propietario.search" class="form-control">
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    <button type="button" @click="BuscarPropietarios" class="btn btn-info">
                                        <i class="icon fa fa-search"></i><span> Buscar Locatario</span>
                                    </button>
                                </div>
                            </div>
                                            ...
                                            <tr v-for="(item, it) in resultados" :key="it">
                                                ...
                                                <td class="text-center">
                                                    <button class="btn btn-success btn-xs" type="button" @click="AgregarPropietario(it)">
                                                        <i class="fa fa-plus"></i> Agregar como locatario
                                                    </button>
                                                </td>
                                            </tr>
                                        </tbody>
                        ...
                        <div class="col-md-12 text-center">
                            <button type="button" @click="ShowModalRegistroPropietario" class="btn btn-success btn-sm">
                                ...
                                <span id="demo-button-text"> Registrar Nuevo Locatario</span>
                            </button>
                        </div>
  ...
  mounted() {
    console.log(
      "El componente 'component-modal-locatario-buscar' se ha montado."
    );

    EventBus.$on("modal-registro-locatorio-buscar", () => {
      setTimeout(() => {
...
function AgregarPropietario(index) {
  ...
  const existe = this.locatario.map((item) => item.id).indexOf(locatario.id);
  console.log(existe);
  if (existe == -1) {
    this.locatario.push(locatario);
    alertify.success("Se ha agregado al locatario", 5);
    $("#component-modal-buscar-propietario").modal("hide");
  }
}

function ShowModalRegistroPropietario() {
  console.log("llego locatario");
  let data = {
    title: "Registrar Locatario",
  };
  EventBus.$emit("modal-registro-locatorio-buscar", data);
  $("#component-modal-registro-locatario").modal("show");
  $("#component-modal-buscar-locatario").modal("hide");
}
