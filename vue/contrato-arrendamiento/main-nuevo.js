...
function validar_contrato() {
	...
	if (this.arrendamiento.empresa_suscribe_id.length == 0) {
	...
	if (this.propietarios.length == 0) {
		alertify.error("Ingrese 1 o mas propietarios", 5);
		this.show_modal_nuevo_propietario();
		this.validate_errors = false;
		return false;
	}
...
function registrar_contrato() {
	...
	.then(function (response) {
		...
		me.auditoria_send({ proceso: "respuesta_guardar_contrato", data: response.data });
		if (response.data.status == 200) {
			swal({
