...
if (isset($post["sec_caja_export"])) {
    ...
    $caja_command = "SELECT 
								...
								c.turno_id,
								c.observaciones,
								oci.titulo AS observaciones_ci,
								c.estado
							FROM tbl_caja c
    ...
    if (count($caja_data)) {
        ...
        foreach ($caja_data as $data_id => $data) {
            ...
            $tr["depositar"] = number_format(($depositar > 0 ? $depositar : 0), 2);
            ...
            $tr["observaciones_ci"] = $data["observaciones_ci"];
        ...
        $table_total_ignore[] = "observaciones_ci";
        ...
        foreach ($table["tbody"] as $tr_k => $tr_v) {
            foreach ($tr_v as $key => $value) {
                if (in_array($key, $table_total_ignore)) {
                    if (in_array($key, array("turno_id", "dia", "mes", "ano", "apertura", "accion", "fondo_fijo", "valla", "observaciones", "observaciones_ci", "fondo_fijo", "valla", "deposito", "depositar", "efectivo_sistema", "efectivo_fisico", "deuda_slot", "deuda_boveda", "saldo_kasnet", "saldo_disashop"))) {
                        $value = "";
                    }
                    if ($key != "caja_id") {
        ...
        $resumen["tarjetas"] = ($table_total["tarjeta_credito_izipay"] + $table_total["tarjeta_credito_niubiz"]);
        ...
        if ($get_data["group_by"] == "day") {
            ...
            foreach ($table["tbody"] as $key => $value) {
                ...
                if (isset($value["reduccion_fondo"])) {
                    ...
                    $reduccion_fondo_total_x_dia[$value["ano"] . "" . $value["mes"] . "" . $value["dia"]][] = "-";
                }
        ...
        $titulo_resumen_caja = "RESUMEN DEL " . date("d-m-Y", strtotime($get_data["fecha_inicio"])) . " AL " . date("d-m-Y", strtotime($get_data["fecha_fin"]));
        $titulo_reporte_cajas = "REPORTE CAJA " . $local_titulo[$get_data['local_id']];
        $titulo_file_reporte_cajas = "reporte_caja_" . $locales[$get_data['local_id']] . "_" . date("d-m-Y", strtotime($get_data["fecha_inicio"])) . "_al_" . date("d-m-Y", strtotime($get_data["fecha_fin"])) . "_" . date("Ymdhis");
        ...
        if (isset($titulo_reporte_cajas)) {
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue($l[0] . "8", "Ticket TITO/Slots")
                ->setCellValue($l[0] . "9", "Devoluciones, Pagos Manuales")
                ->setCellValue($l[0] . "10", "Sobrante / Faltante")
                ->setCellValue($l[0] . "11", "Efectivo Físico")
                ->setCellValue($l[0] . "12", "Préstamo Slot Bóveda")
                ->setCellValue($l[0] . "13", "Devolución Slot Bóveda")
                ->setCellValue($l[0] . "14", "Depósito directo de los clientes")
                ->setCellValue($l[0] . "15", "Aumento Fondo")
                ->setCellValue($l[0] . "16", "Reducción Fondo")
                ->setCellValue($l[0] . "17", "Bonos y Promociones")
                ->setCellValue($l[0] . "18", "Diferencia Real");
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue($l[4] . "8", $resumen["ticket_tito_slots"])
                ->setCellValue($l[4] . "9", $resumen["devo_manuales"])                
                ->setCellValue($l[4] . "10", $resumen["sobra_falta"])
                ->setCellValue($l[4] . "11", $resumen["efectivo_fisico"])
                ->setCellValue($l[4] . "12", $resumen["prestamo_slotboveda"])
                ->setCellValue($l[4] . "13", $resumen["devolucion_slotboveda"])
                ->setCellValue($l[4] . "14", $resumen["deposito_directo_clientes"])
                ->setCellValue($l[4] . "15", $resumen["aumento_fondo"])
                ->setCellValue($l[4] . "16", $resumen["reduccion_fondo"])
                ->setCellValue($l[4] . "17", $resumen["bonos_promociones"])
                ->setCellValue($l[4] . "18", $resumen["diff_real"]);
            ...
            $objPHPExcel->getActiveSheet()->getStyle("E14")->getNumberFormat()->setFormatCode('#,##0.00');
            $objPHPExcel->getActiveSheet()->getStyle("E15")->getNumberFormat()->setFormatCode('#,##0.00');
            $objPHPExcel->getActiveSheet()->getStyle("E16")->getNumberFormat()->setFormatCode('#,##0.00');
            $objPHPExcel->getActiveSheet()->getStyle("E17")->getNumberFormat()->setFormatCode('#,##0.00');
            $objPHPExcel->getActiveSheet()->getStyle("E18")->getNumberFormat()->setFormatCode('#,##0.00');
            ...
            $datos_fisicos_last = $datos_fisicos_first + 16;
            ...
            // 	$info_last = $info_first+12; // fila informacion turno
            // }else{
            // 	$info_last = $info_first+10; // fila informacion dia
            // }

            $info_last = $info_first + 16;
            ...
            foreach ($table["datos_sistema"]["cols"] as $ds_k => $ds_v) {
                ...
                if ($ds_k == 4) {
                    $count = $table["datos_sistema"]["num_terminals"] + 1;
                    $last = $col + $count;
                } else {
                    $count = 3;
                    $last = $col + $count;
            ...
            $conditionalStyles = $objPHPExcel->getActiveSheet()->getStyle("E4:E17")->getConditionalStyles();
            array_push($conditionalStyles, $objConditionalNegativeNumber);
            $objPHPExcel->getActiveSheet()->getStyle("E4:E17")->setConditionalStyles($conditionalStyles);
            ...
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col_datos_fisicos_first, $row, "Devoluciones de Carrera de Caballos");
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1));
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1))->applyFromArray($estiloTituloTabla);
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1))->applyFromArray($estiloSubTituloOpcional);
            $col_datos_fisicos_first++;
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col_datos_fisicos_first, $row, "Tickets TITO/Slots");
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1));
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1))->applyFromArray($estiloTituloTabla);
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1))->applyFromArray($estiloSubTituloOpcional);
            $col_datos_fisicos_first++;
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col_datos_fisicos_first, $row, "Bonos y Promociones");
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1));
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1))->applyFromArray($estiloTituloTabla);
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1))->applyFromArray($estiloSubTituloOpcional);
            $col_datos_fisicos_first++;
            ...
            $col_datos_fisicos_first++;
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col_datos_fisicos_first, $row, "Resultado Real");
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1));
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1))->applyFromArray($estiloTituloTabla);
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1))->applyFromArray($estiloSubTituloOpcional);
            $objPHPExcel->getActiveSheet()->getColumnDimension($l[$col_datos_fisicos_first])->setWidth(12);
            $col_datos_fisicos_first++;
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col_datos_fisicos_first, $row, "Depósito Directo Clientes");
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1));
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1))->applyFromArray($estiloTituloTabla);
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1))->applyFromArray($estiloSubTituloOpcional);
            $col_datos_fisicos_first++;
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col_datos_fisicos_first, $row, "Reclamación de terceros");
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1));
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1))->applyFromArray($estiloTituloTabla);
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1))->applyFromArray($estiloSubTituloOpcional);
            $col_datos_fisicos_first++;
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col_datos_fisicos_first, $row, "NIUBIZ (antes: Tarjeta de crédito)");
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1));
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1))->applyFromArray($estiloTituloTabla);
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1))->applyFromArray($estiloSubTituloOpcional);
            $col_datos_fisicos_first++;
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col_datos_fisicos_first, $row, "IZIPAY");
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1));
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1))->applyFromArray($estiloTituloTabla);
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first] . ($row + 1))->applyFromArray($estiloSubTituloOpcional);
            $col_datos_fisicos_first++;
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col_datos_fisicos_first, $row, "Fondo");
            ...
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col_datos_fisicos_first, ($row + 1), "Aumento");
            ...
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($col_datos_fisicos_first + 1), ($row + 1), "Reduccion");
            ...
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first + 1] . ($row))->applyFromArray($estiloTituloTabla);
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_datos_fisicos_first] . $row . ":" . $l[$col_datos_fisicos_first + 1] . ($row))->applyFromArray($estiloSubTitulo);
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col_datos_fisicos_first, ($row + 1), "Slot / Tienda");
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_datos_fisicos_first] . ($row + 1) . ":" . $l[$col_datos_fisicos_first] . ($row + 1))->applyFromArray($estiloSubTituloOpcional);
            ...
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col_info_first, $row, "Hermeticase");
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($l[$col_info_first] . $row . ":" . $l[$col_info_first + 1] . ($row));
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_info_first] . $row . ":" . $l[$col_info_first + 1] . ($row))->applyFromArray($estiloSubTituloOpcional);

            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col_info_first, ($row + 1), "Bóveda");
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_info_first] . ($row + 1) . ":" . $l[$col_info_first] . ($row + 1))->applyFromArray($estiloSubTituloOpcional);

            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(($col_info_first + 1), ($row + 1), "Venta");
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_info_first + 1] . ($row + 1) . ":" . $l[$col_info_first + 1] . ($row + 1))->applyFromArray($estiloSubTituloOpcional);
            $col_info_first++;
            $col_info_first++;
            ...
            $col_info_first++;
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col_info_first, $row, "Deuda Boveda");
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_info_first] . $row)->applyFromArray($estiloTituloTabla);
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_info_first] . $row . ":" . $l[$col_info_first] . ($row + 1))->applyFromArray($estiloSubTituloOpcional);
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($l[$col_info_first] . $row . ":" . $l[$col_info_first] . ($row + 1));
            $col_info_first++;
            ...
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($l[$col_info_first] . $row . ":" . $l[$col_info_first] . ($row + 1));
            $col_info_first++;
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col_info_first, $row, "Saldo Disashop");
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_info_first] . $row)->applyFromArray($estiloTituloTabla);
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_info_first] . $row . ":" . $l[$col_info_first] . ($row + 1))->applyFromArray($estiloSubTituloOpcional);
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($l[$col_info_first] . $row . ":" . $l[$col_info_first] . ($row + 1));
            $col_info_first++;
            ...
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($l[$col_info_first] . $row . ":" . $l[$col_info_first] . ($row + 1));
            $col_info_first++;
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col_info_first, $row, "Depositar");
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_info_first] . $row)->applyFromArray($estiloTituloTabla);
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_info_first] . $row . ":" . $l[$col_info_first] . ($row + 1))->applyFromArray($estiloSubTituloOpcional);
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($l[$col_info_first] . $row . ":" . $l[$col_info_first] . ($row + 1));
            ...
            $col_info_first++;
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col_info_first, $row, "Observaciones Control Interno");
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_info_first] . $row)->applyFromArray($estiloTituloTabla);
            $objPHPExcel->getActiveSheet()->getStyle($l[$col_info_first] . $row . ":" . $l[$col_info_first] . ($row + 1))->applyFromArray($estiloSubTituloOpcional);
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells($l[$col_info_first] . $row . ":" . $l[$col_info_first] . ($row + 1));
            $objPHPExcel->getActiveSheet()->getColumnDimension($l[$col_info_first])->setAutoSize(true);
            ...
            foreach ($table["total"] as $key => $t_val) {
                ...
                //format
                $objPHPExcel->getActiveSheet()->getStyle("F" . $row_body_total . ":" . $l[$col_body_total] . "" . $row_body_total)->getNumberFormat()->setFormatCode('#,##0.00');
                $col_body_total++;
            ...
            $objPHPExcel->getActiveSheet()->getStyle("A19:A21")->applyFromArray($estiloTituloTabla);
            $objPHPExcel->getActiveSheet()->getStyle("B19:D20")->applyFromArray($estiloTituloTabla);
            $objPHPExcel->getActiveSheet()->getStyle("E19:E21")->applyFromArray($estiloTituloTabla);
            $objPHPExcel->getActiveSheet()->getStyle("F19:F21")->applyFromArray($estiloTituloTabla);

            $objPHPExcel->getActiveSheet()->getStyle("B16")->applyFromArray($estiloTituloTabla);
            $objPHPExcel->getActiveSheet()->getStyle("C16")->applyFromArray($estiloTituloTabla);
            $objPHPExcel->getActiveSheet()->getStyle("D16")->applyFromArray($estiloTituloTabla);
            $objPHPExcel->getActiveSheet()->getStyle("A3:E3")->applyFromArray($estiloTituloResumen);
            $objPHPExcel->getActiveSheet()->getStyle("A4:E17")->applyFromArray($estiloTablaResumen);
            $objPHPExcel->getActiveSheet()->getStyle("A18:E18")->applyFromArray($estiloDiferenciaReal);
            //merge
            ...
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells("A16:D16");
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells("A17:D17");
            ...
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells("A19:A21");
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells("B19:D20");
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells("E19:E21");
            $objPHPExcel->setActiveSheetIndex(0)->mergeCells("F19:F21");

            //height
            $objPHPExcel->getActiveSheet()->getRowDimension('1')->setRowHeight(30);
            $objPHPExcel->getActiveSheet()->getRowDimension('3')->setRowHeight(20);
            //$objPHPExcel->getActiveSheet()->insertNewRowBefore(17, 1);
            ...
            $path_excel = '/var/www/html/export/files_exported/caja_reporte/';
            if (!is_dir($path_excel)) {
                mkdir($path_excel, 0777, true);
            }

            $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
            $excel_path = '/var/www/html/export/files_exported/caja_reporte/' . $titulo_file_reporte_cajas . '.xls';
            $excel_path_download = '/export/files_exported/caja_reporte/' . $titulo_file_reporte_cajas . '.xls';
