<?php
	if(isset($_POST["sec_caja_hermeticase_concar_excel_ant"])){
		$get_data = $_POST["sec_caja_hermeticase_concar_excel"];
		$return = array();
		...
		$local_id = $get_data["sec_caja_hermeticase_concar_local_id"];
		$tipo_cambio = $get_data["sec_caja_hermeticase_tipo_cambio"];
		$correlativo_inicial = $get_data["sec_caja_hermeticase_correlativo_inicial"];
		$fecha_inicio = date("Y-m-d",strtotime($get_data["input_text-sec_caja_hermeticase_fecha_inicio_concar"]));
		$fecha_inicio_pretty = date("d-m-Y",strtotime($get_data["input_text-sec_caja_hermeticase_fecha_inicio_concar"]));
		$fecha_fin = date("Y-m-d",strtotime($get_data["input_text-sec_caja_hermeticase_fecha_fin_concar"]));
		$fecha_fin = date('Y-m-d', strtotime("+1 day", strtotime($fecha_fin)));
		$fecha_fin_pretty = date("d/m/Y",strtotime($get_data["input_text-sec_caja_hermeticase_fecha_fin_concar"]));
		...
		if($local_id != "" ){
			...
			$caja_command .= " AND c.fecha_operacion >= '".$fecha_inicio."'
				...
				AND 
				(
					(SELECT SUM(IFNULL(df.valor,0)) FROM tbl_caja_datos_fisicos df WHERE df.caja_id = c.id AND df.tipo_id = '27')
					+
			...
			$pagos_data = [];
			$codigo_cajas = array();
			//echo "<pre>cajas_id: ";print_r($cajas_id);echo "</pre>";
			foreach($cajas_id as $caja_id)
			{
				$caja_command = "SELECT
						c.id,
						c.turno_id,
						(SELECT
							...
							WHERE df.caja_id = c.id AND df.tipo_id = '27'
						) AS hermeticase_venta,
						(SELECT
							SUM(IFNULL(df.valor,0))
							FROM tbl_caja_datos_fisicos df
							WHERE df.caja_id = c.id AND df.tipo_id = '26'
						) AS hermeticase_boveda
					FROM tbl_caja c
					LEFT JOIN tbl_local_cajas lc ON(lc.id = c.local_caja_id)
					LEFT JOIN tbl_locales l ON (l.id = lc.local_id)
					WHERE c.estado = '1'
					...
					AND 
					(
						(SELECT SUM(IFNULL(df.valor,0)) FROM tbl_caja_datos_fisicos df
							WHERE df.caja_id = c.id AND df.tipo_id = '27'
						) 
						+
						(SELECT SUM(IFNULL(df.valor,0)) FROM tbl_caja_datos_fisicos df
					...
					> 0
				";
				$caja_query = $mysqli->query($caja_command)->fetch_assoc();

				$caja_turno_values = [];
				$caja_turno_values[ $caja_query["id"] ]["hermeticase_venta"] = 0;
				$caja_turno_values[ $caja_query["id"] ]["hermeticase_boveda"] = 0;

				if( $caja_query["hermeticase_venta"] > 0 )
				{
					$caja_turno_values[ $caja_query["id"] ]["hermeticase_venta"] = $caja_query["hermeticase_venta"];
				}
				if( $caja_query["hermeticase_boveda"] > 0 )
				{
					$caja_turno_values[ $caja_query["id"] ]["hermeticase_boveda"] = $caja_query["hermeticase_boveda"];
				}

				
				$sql_tra = "SELECT caja_id ,
					l.id AS local_id,
					l.cc_id,
				...
				if($result_transac -> num_rows > 0)
				{
					...
					{
						$pagos_data[] = $r;
						array_push($codigo_cajas, $r['local_id']);
					};
				}
			}
			//echo "<pre>PAGOS DATA : ";print_r($pagos_data);echo "</pre>";
			$mysqli->query("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED");
			$table = array();
			$table["tbody"] = array();
			$codigo_cajas_agrupado = array_values(array_unique($codigo_cajas));

			$fecha_inicio_correlativo = date("Y-m-01",strtotime($get_data["input_text-sec_caja_hermeticase_fecha_inicio_concar"]));
			$fecha_fin_correlativo = date("Y-m-01",strtotime($get_data["input_text-sec_caja_hermeticase_fecha_fin_concar"]));

			$caja_correlativo = $get_data["sec_caja_hermeticase_correlativo_inicial"];
			...
			foreach ($codigo_cajas_agrupado as $key => $valueCaja) { 
				...
				foreach ($pagos_data as $key => $value) {
					if($valueCaja == $value["local_id"]){
						...
						$glosa_principal = cortar_cadena('Hermeticase '.$value["cc_id"]." ".$value["local_nombre"], 30);
						$cuenta_contable = '10411024';//bbva
						...
						$tr["nro_Documento"] = '0000'.(string)$value["nro_operacion"];
						...
						$tr["codigo_Anexo_Auxiliar"] = 'PA';
						$tr["medio_Pago"] = '001';
						$tr["tipo_Documento_Referencia"] = '';
						$tr["numero_Documento_Referencia"] = '';
						$tr["fecha_Documento_Referencia"] = date("d/m/Y",strtotime($value["fecha_Excel"]));
						$tr["nro_Registradora"] = '';
						$tr["base_Imponible"] = '';
						$tr["igv_Documento"] = '';
						$tr["tipo_Referencia"] = '';
				...
				$tr["codigo_Anexo"] =  $value["cc_id"]."-FONDO BOVEDA";// isset($locales_array[$valueCaja] ) ? $locales_array[$valueCaja]["ruc"] : "";
				$tr["codigo_Centro_Costo"] = '';
				$tr["debe_Haber"] = 'H';
			...
			if ($get_data['sec_caja_hermeticase_concar_local_id'] == '_all_'){
			...
			{
				$local_titulo_parcial = $local_titulo[$get_data['sec_caja_hermeticase_concar_local_id']];
				$local_id_parcial = $locales[$get_data['sec_caja_hermeticase_concar_local_id']];
				$local_id_name = $get_data['sec_caja_hermeticase_concar_local_id'];
			}

			$titulo_reporte_cajas = "REPORTE CONCAR ".$local_titulo_parcial;
			$titulo_file_reporte_cajas = "Depositos_Hermeticase_Caja_Concar_".$local_id_parcial."_".date("d-m-Y",strtotime($get_data["input_text-sec_caja_hermeticase_fecha_inicio_concar"]))."_al_".date("d-m-Y",strtotime($get_data["input_text-sec_caja_hermeticase_fecha_fin_concar"]))."_".date("Ymdhis");
			...
			$mysqli->query("
				INSERT INTO tbl_concar_hermeticase_historico (
					local_id,
					exported_id,
					usuario_id,
					cambio,
					...
					".$exported_id.",
					".$login["id"].",
					".$get_data["sec_caja_hermeticase_tipo_cambio"].",
					".(($get_data["sec_caja_hermeticase_correlativo_inicial"] != "") ? "'".$get_data["sec_caja_hermeticase_correlativo_inicial"]."'" : "null").",
					'".date('Y-m-d H:i:s')."',
					'".$fecha_inicio."',
					'".$fecha_fin."'
				)
	...
	if(isset($_POST["sec_caja_hermeticase_concar_excel"])){
        $get_data = $_POST["sec_caja_hermeticase_concar_excel"];
        $return = array();
        $return["memory_init"]=memory_get_usage();
        $return["time_init"] = microtime(true);
        ...
        if($local_id != "" ){
            ...
            $caja_command = "SELECT
                 c.id,
				(SELECT SUM(IFNULL(df.valor,0)) FROM tbl_caja_datos_fisicos df WHERE df.caja_id = c.id AND df.tipo_id = '27') AS hermeticase_venta,
            ...
            $sql_tra = "SELECT 
				...
				WHERE th.caja_id IS NOT NULL
				$fecha_query
                ";

            $result_transac = $mysqli->query($sql_tra);
            $movimientos_nro_doc_agrupado = [];
            while($r = $result_transac->fetch_assoc()){
                $movimientos_nro_doc_agrupado[$r["nro_doc"]][] = $r;
            }
            ...
            foreach ($movimientos_nro_doc_agrupado as $nro_doc => $transacciones) { 
                $haber_importe_Original = 0;
                $haber_importe_dolares = 0;
                $haber_importe_soles = 0;
                $nombre_Glosa = "";
                ...
                foreach ($transacciones as $key => $value) {
					...
					$caja_boveda = 0 ;
					// echo $value["caja_id"];
					if( isset( $cajas[ $value["caja_id"] ] ) )
					{
						$caja_venta  = $cajas[$value["caja_id"]]["hermeticase_venta"];
						$caja_boveda  = $cajas[$value["caja_id"]]["hermeticase_boveda"];
					}
					$tipo_documento = "ME";
					if( $value["importe"] == $caja_venta )
					{
						$tipo_documento = "ME";

					}
					if( $value["importe"] == $caja_boveda )
					{
						$tipo_documento = "PR";
					}
					$debe_Haber = "H";
