...
function build_formula_add_html($f_id){
	?>
	<div class="input-group">
		<span class="input-group-btn">
			<button 
				...
				data-what='add'
				data-id='f_<?php echo $f_id;?>'><span class="glyphicon glyphicon-plus"></span></button>
		</span>
	</div>
	<?php
}
...
<div class="content container-fluid contratos_form_contrato">
		...
		<div class="row">
			<div class="col-xs-12 ">
				...
				if($item_id){
					...
					<?php 
					// if(array_key_exists($this_menu["id"], $usuario_permisos) && in_array("save", $usuario_permisos[$this_menu["id"]])){ 
					?>
						<button type="button" data-form="contrato_form" data-then="exit" class="contrato_save_btn btn btn-success" data-button="save">
							<i class="glyphicon glyphicon-floppy-save"></i>
							Guardar y Salir
						</button>
						<button type="button" data-form="contrato_form" data-then="reload" class="contrato_save_btn btn btn-success" data-button="save">
							<i class="glyphicon glyphicon-floppy-save"></i>
							Guardar
						</button>
						<!-- <button type="button" 
					...
					<a 	href="./?sec_id=<?php echo $sec_id;?>&amp;item_id=new"
						...
						data-table="tbl_contratos"
						class="btn btn-rounded btn-min-width btn-success btn-add contrato_add_btn">
					...
					<?php 
						//if(array_key_exists($this_menu["id"], $usuario_permisos) && in_array("export", $usuario_permisos[$this_menu["id"]])){
						if(1==1){ ?>
					...
					<a href="export.php?export=tbl_contratos&amp;type=excel"
							type="button"
					...
					<button type="button" class="btn btn-success modal_open_btn btnelegircolumnassec_contratos" data-button="visibility" data-target="#filter_holder">
	...
	if($item_id){
		...
		$item = $mysqli->query("SELECT con.*
								...
								, cli.dni
								, cli.tipo_cliente_id
								, (SELECT l.nombre FROM tbl_locales l WHERE l.id = con.local_id) AS local_nombre
		...
		<div class="modal" id="edit_formula_modal">
			<div class="modal-dialog modal-sm">
				<div class="modal-content modal-rounded">
					<div class="modal-header">
						<button type="button" class="close close_btn"><span aria-hidden="true">&times;</span></button>
						<h4 class="modal-title">Agregar/Editar Condicional/Accion</h4>
					</div>
					<div class="modal-body">

							<select class="form-control select_what">
								<option value="if">SI (condicional)</option>
								<option value="action">ACCION</option>
							</select>
						<table class="table table-condensed table-bordered table_if">
							<tr>
								<th>SI</th>
								<td>
									<select class="form-control edit_input" name="var1">
										<option value="apostado">Apostado (apostado)</option>
										<option value="resultado">Produccion (resultado)</option>
										<option value="depositado">Depositado (depositado)</option>
										<option value="odds">Cuota (odds)</option>
										<option value="is_live" selected="selected">En Vivo (is_live)</option>
										<option value="type">Type (single/multiple)</option>
									</select>
									<!-- <input type="text" class="edit_input" name="var1" value=""> -->
								</td>
							</tr>
							<tr>
								<th>ES</th>
								<td>
									<select class="form-control edit_input" name="var_operador">
									...
									<!-- <input type="text" class=e"edit_input" name="var_operador" value=""> -->
								</td>
							</tr>
							<tr>
								<th>A</th>
								<td>
									<input type="text" class="form-control edit_input" name="var2" value="">
								</td>
							</tr>
						</table>
						<table class="table table-condensed table-bordered table_action">
							<tr>
								<th>VALOR</th>
								<td>
									<input type="text" class="form-control edit_input" name="valor" value="">
								</td>
							</tr>
							<tr>
								<th>OPERADOR</th>
								<td>
									<select class="form-control edit_input" name="valor_operador">
										<option value="%" selected="selected">%</option>
									</select>
										<!-- <input type="text" class="edit_input" name="valor_operador" value=""></td> -->
							</tr>
							<tr>
								<th>COLUMNA</th>
								<td>
									<select class="form-control edit_input" name="donde">
										<option value="apostado" selected="selected">Apostado (apostado)</option>
										<option value="resultado">Produccion (resultado)</option>
										<option value="depositado">Depositado (depositado)</option>
									</select>
									<!-- <input type="text" class="edit_input" name="donde" value=""> -->
								</td>
							</tr>
						</table>
					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-default close_btn">Cerrar</button>
						<button type="button" class="btn btn-success change_btn"><span class="glyphicon glyphicon-floppy-save"></span> OK</button>
					</div>
				</div>
			</div>
		</div>
		<div class="col-xs-12">
			<?php if(array_key_exists("local_nombre", $item)) {?>
			[<?php echo $item["local_id"];?>] <?php echo $item["local_nombre"];?>
			<?php } ?>
		</div>
								...
								<div class="panel" id="datos_de_contrato">
									...
									<div id="panel-datos_de_contrato" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="panel-collapse-1-heading">
										<div class="panel-body">
											...
											<div class="form-group form-group-fecha_registro">
												...
												<div class="input-group col-xs-7">
													...
													<input 
														type="text" 
														class="form-control contratos_datepicker" 
										...
										<div class="form-group">
												<label class="col-xs-5 control-label" for="select-tipo_contrato_id">Estado</label>
												<div class="input-group col-xs-7">
													<select
														class="form-control input_text"
														data-col="estado"
														data-table="tbl_contratos"
														name="estado"
														id="select-tipo_estado"
														title="Seleccione el estado">
														<option value="">- Seleccione -</option>
                                                        <?php foreach($select_status as $key => $value) :?>
                                                            <option value="<?= $key ?>" <?= $key == ($item['estado'] ?? "") ? 'selected' : '';?>><?= $value ?></option>
                                                        <?php endforeach; ?>
													</select>
												</div>
											</div>
												...
												<div class="input-group col-xs-7">
													<select 
														...
														$sel_query = $mysqli->query("SELECT 
																						...
																						,CONCAT('[',id,']',' ',razon_social) AS razon_social 
																					FROM tbl_clientes 
																					WHERE estado = '1' 
																					ORDER BY nombre ASC, razon_social ASC");
														while($sel=$sel_query->fetch_assoc()){
												...
												<div class="input-group col-xs-7">
														...
														if($item_id=="new"){
															...
															<?php
																	...
																	while($sel=$sel_query->fetch_assoc()){
																		?><option value="<?php echo $sel["id"];?>" <?php if($item["local_id"]==$sel["id"]){?> selected="selected" <?php } ?> >[<?php echo $sel["id"];?>] <?php 
																			echo $sel["nombre"];
																			?></option><?php
																	}
																?>
									...
									<div class="file-item sort_item" 
										...
										<div class="file-thumb">
											...
											if(in_array($f_v["ext"], $images_exts)){
												?>
												<img class="img-thumbnail" src="files_bucket/<?php echo $f_v["archivo"];?>">
												<?php
											}elseif($f_v["ext"]=="pdf"){
										...
										<div class="file-opciones">
											...
											<a class="btn btn-default btn-xs" href="files_bucket/<?php echo $f_v["archivo"];?>" target="_blank">
										...
										<div class="panel form-add_producto hidden">
											<div class="panel-body">
												...
												<div class="col-xs-12 col-md-5">
													<button class="btn btn-default btn-sm contrato_add_producto_btn" data-button="new">Agregar</button>
											...
											<div class="panel">
												...
												<div id="panel-collapse-<?php echo $pro["id"];?>" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="panel-collapse-1-heading">
													<div class="panel-body">
														...
														$con_for_det_que_command = "SELECT id,formula_id,desde,hasta,monto,contrato_id,servicio_id,producto_id
															...
															<div class="col-xs-12 panel formula_holder" data-pro-id="<?php echo $pro["id"];?>">																
																...
																if(count($contrato_formula)==1){
																	if($formula["tipo_id"]==3){
																		...
																		?>
																		<div class="input-group">
																			<select class="form-control formula_tipo_3_copy_select_<?php echo $contrato_formula[0]["producto_id"];?>">
																				<option value="0">Seleccione un local y producto</option>
																				<?php
																				$to_copy_command = "SELECT 
																									...
																									LEFT JOIN tbl_productos p ON (p.id = cp.producto_id)
																									WHERE cf.tipo_id = '3'
																									AND cf.contrato_id != '".$item["id"]."'";
																				$to_copy_query = $mysqli->query($to_copy_command);
																				while ($to_copy = $to_copy_query->fetch_assoc()) {
																					?><option value="<?php echo $to_copy["id"];?>">
																						[<?php echo $to_copy["local_id"];?>] <?php echo $to_copy["nombre"];?>
																							(<?php echo $to_copy["pro_nombre"];?>)
																						</option><?php
																				}
																				?>
																			</select>
																			<span class="input-group-btn">
																				<button 
																					class="btn btn-default btn-sm formula_tipo_3_copy_btn" 
																					type="button"
																					data-producto_id="<?php echo $contrato_formula[0]["producto_id"];?>">Copiar</button>
																			</span>
																		</div>
																		<button 
																			data-formula_id="<?php echo $contrato_formula[0]["formula_id"];?>"
																			data-contrato_id="<?php echo $contrato_formula[0]["contrato_id"];?>"
																			data-servicio_id="<?php echo $contrato_formula[0]["servicio_id"];?>"
																			data-producto_id="<?php echo $contrato_formula[0]["producto_id"];?>"
																			class="btn btn-block btn-success formula_tipo_3_save_btn">
																				<span class="glyphicon glyphicon-floppy-disk"></span> Guardar
																		</button>
																		<br>
																		<?php
																		$condicionales = get_condicionales($contrato_formula[0]);
																		if(count($condicionales)){
																			$first_id = array_values($condicionales)[0]["id"];
																			?>
																			<div class="formula_tipo_3_holder formula_tipo_3_holder_<?php echo $contrato_formula[0]["producto_id"];?>">
																				<?php build_formula($condicionales,$first_id); ?>
																			</div>
																			<?php
																		}else{
																			$first_id = "new";
																			...
																			<div class="formula_tipo_3_holder formula_tipo_3_holder_<?php echo $contrato_formula[0]["producto_id"];?>">
																				<?php 
																					// build_formula_add_html('new'); 
																					c_form_con_build_buttons(3);
																					?>
																			</div>
																			<?php
																		}
																		...
																		<table class="table table-striped table-user-information">
																			<tr>
																				<td class="td-title">Porcentaje Cliente</td>
																				<td><?php echo $contrato_formula[0]["monto"];?></td>
																				<td class="td-title">Porcentaje Free Games</td>
																				<td><?php echo number_format(100 - $contrato_formula[0]["monto"],2);?></td>
																			</tr>
																		</table>
																		<?php
																	}
																}elseif(count($contrato_formula)>1){
																	?>
																	<table class="table table-striped table-user-information">
																		<tr>
																			<td class="td-title">Desde</td>
																			<td class="td-title">Hasta</td>
																			<td class="td-title">Monto<br>Cliente</td>
																			<td class="td-title">Monto<br>FreeGames</td>
																		</tr>
																		<?php
																		foreach ($contrato_formula as $key => $value) {
																			...
																			<tr>
																				...
																				<td><?php echo number_format(100 - $value["monto"],2);?></td>
																			</tr>
																			<?php
																		}
																		?>
														...
														<div class="form-group form-formula_id-<?php echo $pro["id"];?>">	
															...
															<div class="input-group col-xs-7">
																...
																<label 
																	class="input-group-addon glyphicon glyphicon-new-window select_add_dialog_btn hidden"
																	data-name="Formula"
		...
		<div class="col-md-2 hidden-xs hidden-sm">
			<ul class="nav nav-tabs tabs-right contrato_tabs">
				<li class="active"><a class="tab_btn" href="#tab_contrato" data-tab="tab_contrato" data-button="contract" >Contrato</a></li>
				<li class=""><a class="tab_btn" href="#tab_archivos" data-tab="tab_archivos" data-button="archive" >Archivos</a></li>
				<li class=""><a class="tab_btn" href="#tab_comercial" data-tab="tab_comercial" data-button="commercial">Comercial</a></li>
			</ul>
		</div>
		<?php
	}else{
		...
		$list_query=$mysqli->query("SELECT 
									...
									-- ,cli.nombre
									,CONCAT('[',cli.id,']',' ',cli.nombre) AS nombre
									-- , cli.razon_social
									,CONCAT('[',cli.id,']',' ',cli.razon_social) AS razon_social
									, cli.tipo_cliente_id, cli.dni, cli.ruc
									,cli_tipo.nombre AS cliente_tipo
									,loc.tipo_id
									-- ,loc.nombre AS nombre_de_local
									,CONCAT('[',con.local_id,']',' ',loc.nombre) AS nombre_de_local
		...
		$list_cols=array();
			...
			$list_cols["cliente"]="[ID] Cliente";
			$list_cols["dni"]="DNI";
			$list_cols["razon_social"]="[ID] Razón Social";
			$list_cols["ruc"]="RUC";

			//$list_cols["dni_o_ruc"]="DNI/RUC";
			$list_cols["tipo_de_local"]="Tipo de Local";
			$list_cols["direccion"]="Dirección";
			$list_cols["nombre_de_local"]="[ID] Nombre de Local";
		...
		<table 
			id="tbl_contratos_datatable"
			class="table table-striped table-hover table-condensed table-bordered dt-responsive display" cellspacing="0" width="100%">
			<thead>
				<tr>
					<?php
					foreach ($list_cols_show as $key => $value) {
						if($key=="id"){
							...
							?>
							<th id="th_switch<?php echo $key;?>" class="w-85px">Estado</th>
							<th id="th_switch2<?php echo $key;?>" class="w-85px">Estados</th><?php
						}else{
							?>
				...
				<tr>
					...
					foreach ($list_cols_show as $key => $value) {
						if($key=="id"){
							...
							?><th id="th_switch<?php echo $key;?>" class="w-85px">Estado</th><th id="th_switch2<?php echo $key;?>" class="w-85px">Estados</th><?php
						}else{
							?>
							<th id="th_<?php echo $key;?>"><?php echo $value;?></th>
							<?php
							...
							if($key=="opciones"){
								...
								<td class="text-center">
									<?php //if(array_key_exists($this_menu["id"], $usuario_permisos) && in_array("edit", $usuario_permisos[$this_menu["id"]])){ 
										if(1==1){?>
									<a 
										class="btn btn-rounded btn-default btn-sm btn-edit btn_editar_contratos" 
										title="Editar"
										data-button="edit"
								...
								?>
								<td><?php if($l_v["estado"]==0){ echo "Pendiente";}
								if($l_v["estado"]==1){ echo "Con Contrato";}
								if($l_v["estado"]==2){ echo "Terminado";}
								?>
								</td>	
								<td>
									<input  
									<?php if($l_v["estado"]==2){echo "disabled";}?> 
									<?php if($l_v["estado"]==1){echo "checked";}?> 
									<?php if($l_v["estado"]==0){echo "checked";}?> 
									data-toggle="toggle"  
									data-onstyle="success" 
									data-offstyle="danger" 
									data-on='<?php if($l_v["estado"]==0){echo "Pendiente";}
									if($l_v["estado"]==1){echo "Con Contrato";}?>'
									data-off='Terminado'
									data-size="mini" 
									id="checkbox_<?php echo $l_v['id'];?>" 
									type="checkbox" 
									data-table="tbl_contratos" 
