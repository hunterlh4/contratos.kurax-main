...
$(document).ready(function() {
    ...
    if ($contratos_meses.length > 0) {
        new Chart($contratos_meses, {
            type: 'line',
            data: {
                labels: ['Jul 16', 'Ago 16', 'Sep 16', 'Oct 16', 'Nov 16', 'Dic 16','Ene 17', 'Feb 17', 'Mar 17', 'Abr 17'],
                datasets: [{
	...
	$(".sort_list").each(function(index, el) {		
		$(el).sortable({
			...
			stop: function( event, ui ) {
				//console.log(ui);
				var list = {};
				$(el).children(".sort_item").each(function(i, e) {
...
function btn_settings(){
	...
	$('input[type=radio][name=tbl_locales_otra_casa_apuestas]').change(function() {
		if (this.value == '0') {
	...
	$('input[type=radio][name=tbl_locales_experiencia_casa_apuestas]').change(function() {
		if (this.value == '0') {
...
function btn_events(){
	/*get_id_sec_sub_sec(sec_id,sub_sec_id);*/
	validar_permiso_usuario(1,9,sec_id,sub_sec_id);
	...
	sec_general_validacion_permisos_usuarios();
		...
		.keyup(function(event) {
			...
			if(input.data("list-item")){
				...
				$('.'+input.data("list-item")).each(function(index, el) {
					//console.log(el);
					var key = $(el).find("."+input.data("list-key")).html();
	...
	$(".btn-print")
		...
		.click(function(event) {
			event.preventDefault();
			console.log();
			window.open($(this).attr("href"), "Imprimir", "width=550, height=600, menubar=no, resizable=no, status=no, titlebar=no, toolbar=no, top=10", true);
		});
...
function select_add(btn) {
	...
	$(".input_text").each(function(index, i_el) {
		...
		if($(i_el).data("col")){
			if($(i_el).data("col").search(save_data["table"]+"_")>=0){
				save_data.values[$(i_el).data("col").replace(save_data["table"]+"_","")]=$(i_el).val();
			}
		}
	});
	console.log(save_data);
...
/* /utils*/
function validar_permiso_usuario(cod_user,cod_btn,sec_id,sub_sec_id){
	var data = {};
	data.where="validar_usuario_permisos";
	data.filtro={};
	data.filtro.cod_user=cod_user;
	data.filtro.cod_btn = cod_btn;
	data.filtro.sec_id=sec_id;
	data.filtro.sub_sec_id = sub_sec_id;
		auditoria_send({"proceso":"validar_permiso_usuario","data":data});
		$.ajax({
			data: data,
			type: "POST",
			dataType: "json",
		...
		.done(function( dataresponse) {
			...
			$(document).trigger({
				...
				"event_data":dataresponse.data
			});			
		})
		.fail(function( jqXHR, textStatus, errorThrown ) {
			if ( console && console.log ) {
				console.log( "La solicitud validar permisos usuario a fallado: " +  textStatus);
			}
		})
}
function sec_general_validacion_permisos_usuarios(){
	$(".save_btn").off().on("click",function(e){
		...
		/*
			...
			validar_permiso_usuario(user_id,9,sec_id,sub_sec_id);	
		*/	
