function sec_conciliacion_reporte_anulacion() {

		loading(true);
		conci_reporte_anulacion_events();
		conci_reporte_anulacion_obtener_proveedor();
		conci_reporte_anulacion_settings();
		loading();
		conci_reporte_anulacion_get_anulaciones();

		function conci_reporte_anulacion_settings() {
			$('.conci_reporte_anulacion_select2').select2({
				closeOnSelect: false,
				allowClear: false,
			});
			
			$('.conci_reporte_anulacion_zona').select2({
				closeOnSelect: false,
				allowClear: false
			});
			
			}
		function conci_reporte_anulacion_events() {
			$(".conci_reporte_anulacion_btn_filtrarAnulacion")
				.off("click")
				.on("click", function () {
					loading(true);
					conci_reporte_anulacion_get_anulaciones();
		...
		function conci_reporte_anulacion_get_anulaciones() {
			loading(true);
			var get_anulaciones_data = {};
			get_anulaciones_data.filtro = {};
			get_anulaciones_data.filtro.zona_id = $("#search_conci_reporte_anulacion_zona").val();
			get_anulaciones_data.filtro.locales = $("#search_conci_reporte_anulacion_proveedor_id").val();
			get_anulaciones_data.filtro.canales_de_venta = $('#search_conci_reporte_anulacion_count').val();
			get_anulaciones_data.filtro.supervisores = $('#search_conci_reporte_anulacion_supervisor').val();
			get_anulaciones_data.filtro.estado_locales = "activos";

			console.log(get_anulaciones_data.filtro.canales_de_venta);
			get_anulaciones_data.accion = "conci_reporte_anulacion_Datatable";

			$.ajax({
				data: get_anulaciones_data,
				type: "POST",
				url: "/sys/get_conciliacion_reporte_anulacion.php",
				async: "false"
			})
				.done(function (responsedata, textStatus, jqXHR) {
					try {
						...
						conci_reporte_anulacion_mostrar_datatable(obj);
					} catch (err) {
						console.log(err);
						swal({
				...
				.fail(function (jqXHR, textStatus, errorThrown) {
					console.log("La solicitud anulaciones a fallado: " + textStatus);
				});
			}
		...
		function conci_reporte_anulacion_mostrar_datatable(obj) {
		
			var datatable_data = obj.datatable_data;
			var meses = obj.meses;
			var concepto = "GASTOS";
		
			$('#tabla_conci_reporte_anulacion tfoot').empty();
			$('#tabla_conci_reporte_anulacion tfoot').append("<tr>");
			$('#tabla_conci_reporte_anulacion tfoot tr')
				.append("<th>")
				.append("<th>");
			...
			var columnas = [];
			columnas.push({ title: "PROVEEDOR", data: "PROVEEDOR", className: "tabla_conci_reporte_anulacion_local_td", defaultContent: "---" });
			columnas.push({ title: "CONCEPTOS", data: "CANAL DE VENTA", className: "tabla_search_conci_reporte_anulacion_count_td", defaultContent: "---" });
		
			$(meses).each(function (i, e) {
				columnas.push({
					title: moment(e, "YYYY-MM").locale("es").format("MMMM YYYY").toUpperCase(),
					data: e,
					className: "text-right tabla_conci_reporte_anulacion_tfoot_meses",
					defaultContent: "-",
			...
			var table_dt = $('#tabla_conci_reporte_anulacion').DataTable({
				"bDestroy": true,
				scrollX: true,
				scrollY: false,
				fixedColumns: {
				...
				columnDefs: [
					{
						aTargets: 'tabla_conci_reporte_anulacion_tfoot_meses',
	...
	function conci_reporte_anulacion_obtener_proveedor() {
		let select = $("[name='search_conci_reporte_anulacion_proveedor_id']");
			...
			$.ajax({
				...
				success: function (resp) {
					...
					if (parseInt(respuesta.http_code) == 200) {
						...
						$("#search_conci_reporte_anulacion_proveedor_id").html(html).trigger("change");
						$("#search_conci_reporte_anulacion_proveedor_id").val("all").trigger("change");
						return false;
					}
