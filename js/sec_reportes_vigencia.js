function sec_reportes_vigencia() {
	$(".mask_centro_costos").mask("0000");
	$(".select2").select2({
		width: "100%"
	});
	sec_reportes_vigencia_obtener_departamentos();
	sec_reportes_vigencia_obtener_opciones("obtener_arrendatario_v", "[name='sec_rep_v_empresa']");
...
function sec_reportes_vigencia_obtener_opciones(accion, select) {
	$.ajax({
		url: "/sys/get_reportes_vigencia.php",
		type: 'POST',
		data: {
...
function sec_reportes_vigencia_obtener_departamentos() {
...
function sec_reportes_vigencia_btn_buscar() {
	...
	sec_reportes_vigencia_mostrar_excel();
	$.ajax({
		url: "/sys/get_reportes_vigencia.php",
		type: 'POST',
		data: data,
		success: function(resp) {
			...
			if (parseInt(respuesta.status) == 200) {
				...
				sec_reportes_vigencia_initialize_table('sec_rep_vigencia');
				return false;
			}
		},
		error: function() {}
...
function sec_reportes_vigencia_mostrar_excel() {
	var empresa = $('#sec_rep_v_empresa').val();
	var centro_costo = $('#sec_rep_v_cc').val();
	var renta_m = $('#sec_rep_v_renta_m').val();
	var nomb_tienda = $('#sec_rep_v_n_tienda').val();
	...
	var n_adendas = $('#sec_rep_v_n_adendas').val();
	var estado = $('#sec_rep_v_estado').val();
	document.getElementById('sec_rep_vg_btn_excel').innerHTML = '<a href="export.php?export=reporte_por_vigencia&amp;type=lista&amp;empresa=' + empresa + '&amp;centro_costo=' + centro_costo + '&amp;renta_m=' + renta_m + '&amp;nomb_tienda=' + nomb_tienda + '&amp;departamento=' + departamento + '&amp;provincia=' + provincia + '&amp;distrito=' + distrito + '&amp;direccion=' + direccion + '&amp;fec_suscrip=' + fec_suscrip + '&amp;fec_inicio=' + fec_inicio + '&amp;fec_fin=' + fec_fin + '&amp;n_adendas=' + n_adendas + '&amp;estado=' + estado + '" class="btn btn-success form-control export_list_btn" download="reporte_por_vigencia.xls"><span class="glyphicon glyphicon-export"></span> Exportar excel</a>';
}

function sec_reportes_vigencia_initialize_table(tabla) {
