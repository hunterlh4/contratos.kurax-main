function sec_conciliacion_reporte_liquidacion() {

		loading(true);
		conci_reporte_liquidacion_events();
		conci_reporte_liquidacion_obtener_proveedor();
		conci_reporte_liquidacion_settings();
		loading();
		conci_reporte_liquidacion_get_liquidaciones();

		function conci_reporte_liquidacion_settings() {
			$('.conci_reporte_liquidacion_select2').select2({
				closeOnSelect: false,
				allowClear: false,
			});
			
			$('.conci_reporte_liquidacion_zona').select2({
				closeOnSelect: false,
				allowClear: false
			});
			
			}
		function conci_reporte_liquidacion_events() {
			$(".conci_reporte_liquidacion_btn_filtrarLiquidacion")
				.off("click")
				.on("click", function () {
					loading(true);
					conci_reporte_liquidacion_get_liquidaciones();
		...
		function conci_reporte_liquidacion_get_liquidaciones() {
			loading(true);
			var get_liquidaciones_data = {};
			get_liquidaciones_data.filtro = {};
			get_liquidaciones_data.filtro.zona_id = $("#search_conci_reporte_liquidacion_zona").val();
			get_liquidaciones_data.filtro.locales = $("#search_conci_reporte_liquidacion_proveedor_id").val();
			get_liquidaciones_data.filtro.canales_de_venta = $('#search_conci_reporte_liquidacion_count').val();
			get_liquidaciones_data.filtro.supervisores = $('#search_conci_reporte_liquidacion_supervisor').val();
			get_liquidaciones_data.filtro.estado_locales = "activos";

			console.log(get_liquidaciones_data.filtro.canales_de_venta);
			get_liquidaciones_data.accion = "conci_reporte_liquidacion_Datatable";

			$.ajax({
				data: get_liquidaciones_data,
				type: "POST",
				url: "/sys/get_conciliacion_reporte_liquidacion.php",
				async: "false"
			})
				.done(function (responsedata, textStatus, jqXHR) {
					try {
						...
						conci_reporte_liquidacion_mostrar_datatable(obj);
					} catch (err) {
						console.log(err);
						swal({
		...
		function conci_reporte_liquidacion_mostrar_datatable(obj) {
		
			var datatable_data = obj.datatable_data;
			var meses = obj.meses;
			var concepto = "GASTOS";
		
			$('#tabla_conci_reporte_liquidacion tfoot').empty();
			$('#tabla_conci_reporte_liquidacion tfoot').append("<tr>");
			$('#tabla_conci_reporte_liquidacion tfoot tr')
				.append("<th>")
				.append("<th>");
			...
			var columnas = [];
			columnas.push({ title: "PROVEEDOR", data: "PROVEEDOR", className: "tabla_conci_reporte_liquidacion_local_td", defaultContent: "---" });
			columnas.push({ title: "CONCEPTOS", data: "CANAL DE VENTA", className: "tabla_search_conci_reporte_liquidacion_count_td", defaultContent: "---" });
		
			$(meses).each(function (i, e) {
				columnas.push({
					title: moment(e, "YYYY-MM").locale("es").format("MMMM YYYY").toUpperCase(),
					data: e,
					className: "text-right tabla_conci_reporte_liquidacion_tfoot_meses",
					defaultContent: "-",
					render: function (data, type, row, meta) {
						if (concepto != 'CANTIDAD DE TICKETS') {
							...
							if (row["CANAL DE VENTA"] === "Diferencia de Comisión" && data > 0) {
			...
			var table_dt = $('#tabla_conci_reporte_liquidacion').DataTable({
				"bDestroy": true,
				scrollX: true,
				scrollY: false,
				fixedColumns: {
				...
				buttons: [
					...
					},
					{
						...
						footer: true,
						className: 'csvButton',
						filename: 'Reporte_de_liquidacion_' + moment(new Date()).format("YYYY_MM_DD_HH_mm_ss")
					},
					{
						extend: 'excelHtml5',
						text: 'Excel',
				...
				columnDefs: [
					{
						aTargets: 'tabla_conci_reporte_liquidacion_tfoot_meses',
	...
	function conci_reporte_liquidacion_obtener_proveedor() {
		let select = $("[name='search_conci_reporte_liquidacion_proveedor_id']");
			...
			$.ajax({
				...
				success: function (resp) {
					...
					if (parseInt(respuesta.http_code) == 200) {
						...
						$("#search_conci_reporte_liquidacion_proveedor_id").html(html).trigger("change");
						$("#search_conci_reporte_liquidacion_proveedor_id").val("all").trigger("change");
						return false;
					}
