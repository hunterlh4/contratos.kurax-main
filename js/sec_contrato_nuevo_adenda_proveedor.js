...
function sec_contrato_nuevo_adenda_proveedor() {
	...
	sec_con_nuevo_aden_prov_obtener_opciones("obtener_proveedores","[name='sec_con_nuevo_proveedor']");
	// sec_con_nuevo_aden_prov_obtener_opciones("obtener_abogados","[name='abogado_id']");
	// sec_con_nuevo_aden_prov_obtener_opciones("obtener_empresa_at","[name='sec_con_nuevo_empresa_grupo_at_2']");
	// sec_con_nuevo_aden_prov_obtener_opciones("obtener_bancos","[name='sec_con_nuevo_banco']");
	// sec_con_nuevo_aden_prov_obtener_opciones("obtener_periodo","[name='periodo']");
	sec_con_nuevo_aden_prov_obtener_opciones("obtener_monedas","[name='modal_contr_ade_int_moneda_id']");
	sec_con_nuevo_aden_prov_obtener_opciones("obtener_forma_pago","[name='modal_contr_ade_int_forma_pago']");
	sec_con_nuevo_aden_prov_obtener_opciones("obtener_tipo_comprobante","[name='modal_contr_ade_int_tipo_comprobante']");
	sec_contrato_nuevo_obtener_opciones("obtener_directores", "[name='director_aprobacion_id']");

	setTimeout(function () {
		$('#sec_con_nuevo_proveedor').focus();
		$('#sec_con_nuevo_proveedor').select2("open");
	}, 1000);
}

function sec_con_nuevo_aden_prov_obtener_opciones(accion,select){
	$.ajax({
...
function sec_con_nuevo_aden_prov_obtener_contratos() {

	var proveedor = $('#sec_con_nuevo_proveedor').val();
...
function sec_con_nuevo_aden_prov_obtener_contratos_interno_id() {
...
function sec_con_nuevo_aden_prov_solicitud_editar_campo_adenda(nombre_menu_usuario, nombre_tabla, nombre_campo, nombre_campo_usuario, tipo_valor, valor_actual, metodo_select, id_del_registro) {
	...
	if (tipo_valor == 'select_option') {
		...
		if (nombre_campo == 'ubigeo_id') {
			...
			$('#div_adenda_valor_select_option').hide();
			sec_con_nuevo_aden_prov_obtener_opciones(metodo_select,$("[name='adenda_inmueble_id_departamento']"));
			...
			sec_con_nuevo_aden_prov_obtener_opciones(metodo_select,$("[name='adenda_valor_select_option']"));
			setTimeout(function() {
...
function sec_con_nuevo_aden_prov_guardar_detalle_adenda(name_modal_close) {
	...
	$.ajax({
		url: "/sys/set_contrato_nuevo_adenda_proveedor.php",
		type: 'POST',
		data: data,
		beforeSend: function() {
			loading("true");
		...
		success: function(resp) { //  alert(datat)
			...
			if (parseInt(respuesta.http_code) == 200) {
				...
				sec_con_nuevo_aden_prov_asignar_detalle_a_la_adenda(respuesta.result, name_modal_close);
...
function sec_con_nuevo_aden_prov_asignar_detalle_a_la_adenda(id_adenda,modal){
	if (array_adendas_contrato.includes(id_adenda) === false){
		array_adendas_contrato.push(id_adenda)
	}
	
	
	sec_con_nuevo_aden_prov_actualizar_tabla_detalle_adenda();
	
}

function sec_con_nuevo_aden_prov_actualizar_tabla_detalle_adenda(){
...
function sec_con_nuevo_aden_prov_eliminar_detalle_adenda(id_adenda){
...
function sec_con_nuevo_aden_prov_agregar_representante(){
	$('#modalNuevoProveedor').modal({backdrop: 'static', keyboard: false});
	// sec_con_detalle_int_limpiarInputsRL();
}

function sec_con_nuevo_aden_prov_guardar_nuevo_representante_legal(){
	...
	$.ajax({
		...
		success: function(response, status) {
			...
			if (parseInt(respuesta.http_code) == 200) {
				...
				sec_con_nuevo_aden_prov_asignar_detalle_a_la_adenda(respuesta.result, 'modalNuevoProveedor');
			}
		},
...
function sec_con_nuevo_aden_prov_nuevo_contraprestacion_modal() {
...
$("#modal_contr_ade_int_tipo_igv_id").change(function () {
	$("#modal_contr_ade_int_tipo_igv_id option:selected").each(function () {
		...
		if (modal_contr_ade_int_tipo_igv_id != 0) {
			sec_con_nuevo_aden_prov_calcular_subtotal_y_igv(modal_contr_ade_int_tipo_igv_id);
			setTimeout(function() {
...
function sec_con_nuevo_aden_prov_calcular_subtotal_y_igv(tipo)
{
...
function sec_con_nuevo_aden_prov_nuevo_contraprestacion() {
	var contrato_id = $('#id_registro_contrato_id').val();
	...
	$.ajax({
		...
		success: function(response) { //  alert(datat)
			...
			if (parseInt(respuesta.http_code) == 200) {
				$('#frm_adenda_nuevo_proveedor')[0].reset();
				sec_con_nuevo_aden_prov_asignar_detalle_a_la_adenda(respuesta.result, 'modalNuevoContraprestacion');
			}
...
function sec_con_nuevo_aden_prov_guardar_adenda() {
	...
	var arrayJSON = JSON.stringify(array_nuevos_files_anexos_proveedor);
...
var array_nuevos_files_anexos_proveedor = [];

function sec_modal_nuevo_archivo_anexo(tipo_contrato_id) {
...
function sec_con_nuevo_aden_anadir_archivo_proveedor() {
		...
		var name_file = tipo_archivo_id + "_anexo"+Tiempo;
...
function borrarArchivo_proveedor(id_anexo) {
	//Restamos el número de archivos
	contArchivos = contArchivos - 1;

	array_nuevos_files_anexos_proveedor = array_nuevos_files_anexos_proveedor.filter((item) => item.id_objeto !== id_anexo);
	$("div[name=" + id_anexo + "]").remove();
}

function file_proveedor(event, id) {
	var id_ = "#" + id;
	...
	array_nuevos_files_anexos_proveedor.push(objeto);

	console.log(array_nuevos_files_anexos_proveedor);
}
