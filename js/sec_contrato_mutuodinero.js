...
function sec_contrato_mutuodinero() {
  $(".select2").select2({
    width: "100%",
  });
  ...
  $(".limpiar_select2").click(function () {
    $("#" + $(this).attr("limpiar"))
      ...
      .val("")
      .trigger("change");
    buscarContratoPorParametros_MututoDinero();
  });
  ...
  sec_contrato_mutuodinero_listar_contratos_datatable();
...
function buscarContratoPorParametros_MututoDinero() {
...
function sec_contrato_mutuodinero_listar_contratos_datatable() {
  ...
  var data = {
    accion: "cont_listar_mutuodinero",
  ...
  tabla = $("#cont_locales_datatable")
    .dataTable({
      ...
      ajax: {
        url: "/sys/set_contrato_mutuodinero.php",
        data: data,
        type: "POST",
        dataType: "json",
        error: function (e) {
    ...
    .DataTable();
  mostrar_switch_contrato_locales();
  sec_contrato_locales_mostrarReporteExcel_mutuodinero();
...
function sec_contrato_locales_mostrarReporteExcel_mutuodinero() {
  var id_empresa = $("#search_id_empresa").val();
  var nombre_tienda = $("#search_nombre_tienda").val();
  var centro_costos = $("#search_centro_costos").val();
  var moneda = $("#search_moneda").val();
  ...
  var fecha_fin_suscripcion = $("#fecha_fin_suscripcion").val();
  var etapa = $("#search_etapa").val();
  document.getElementById("cont_locales_excel_mutuodinero").innerHTML =
    '<a href="export.php?export=cont_contrato_mutuodinero&amp;type=lista&amp;' +
...
function contrato_alerta_mutuodinero(id) {
  limpiar_tr_modal_alerta();
  $("#configurarAlerta_mutuodinero").modal("show");
  var data = {
  ...
  $.ajax({
    ...
    success: function (resp) {
      ...
      $("#condicion_economica_id").val(respuesta.idmutuodinero);
...
function registrar_alerta_mutuodinero() {
  ...
  $.ajax({
    ...
    success: function (resp) {
      var respuesta = JSON.parse(resp);
      if (respuesta == "ok") {
        $("#configurarAlerta_mutuodinero").modal("hide");
        limpiar_tr_modal_alerta();
        m_reload();
      } else {
        mensajeAlerta(
