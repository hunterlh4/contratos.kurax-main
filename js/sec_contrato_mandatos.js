...
function sec_contrato_mandatos() {
  $(".select2").select2({
    width: "100%",
  });
  ...
  $(".limpiar_select2").click(function () {
    $("#" + $(this).attr("limpiar"))
      ...
      .val("")
      .trigger("change");
    buscarContratoPorParametros_mandato();
  });
  ...
  sec_contrato_mandatos_listar_contratos_datatable_mandatos();
...
function buscarContratoPorParametros_mandato() {
...
function sec_contrato_mandatos_listar_contratos_datatable_mandatos() {
  ...
  tabla = $("#cont_locales_datatable")
    .dataTable({
      ...
      ajax: {
        url: "/sys/set_contrato_mandatos.php",
        data: data,
        type: "POST",
        dataType: "json",
        error: function (e) {
    ...
    .DataTable();
  mostrar_switch_contrato_locales();
  sec_contrato_locales_mostrarReporteExcel_mandato();
...
function sec_contrato_locales_mostrarReporteExcel_mandato() {
  var id_empresa = $("#search_id_empresa").val();
  var nombre_tienda = $("#search_nombre_tienda").val();
  var centro_costos = $("#search_centro_costos").val();
  var moneda = $("#search_moneda").val();
  ...
  var fecha_fin_suscripcion = $("#fecha_fin_suscripcion").val();
  var etapa = $("#search_etapa").val();
  document.getElementById("cont_locales_excel_mandato").innerHTML =
    '<a href="export.php?export=cont_contrato_mandato&amp;type=lista&amp;' +
...
function contrato_alerta_mandato(id) {
  limpiar_tr_modal_alerta();
  $("#configurarAlerta_mandato").modal("show");
  var data = {
  ...
  $.ajax({
    ...
    success: function (resp) {
      var respuesta = JSON.parse(resp);
      $("#condicion_economica_id").val(respuesta.idmandato);
...
function registrar_alerta_mandato() {
  ...
  $.ajax({
    ...
    success: function (resp) {
      var respuesta = JSON.parse(resp);
      if (respuesta == "ok") {
        $("#configurarAlerta_mandato").modal("hide");
        limpiar_tr_modal_alerta();
        m_reload();
      } else {
        mensajeAlerta(
