...
function sec_contrato_detalle_adenda_locacion() {
...
function sec_con_detalle_aden_arrend_obtener_contratos() {
	...
	$.ajax({
		url: "/sys/get_contrato_nuevo_adenda_locacion.php",
		type: 'POST',
		data: data,
		beforeSend: function () {
		},
...
function sec_con_detalle_aden_arrend_obtener_contratos_interno_id() {
	...
	let data = {
		accion:'obtener_contrato_locacion_por_id',
...
function sec_con_detalle_aden_arrend_solicitud_editar_campo_adenda(nombre_menu_usuario, nombre_tabla, nombre_campo, nombre_campo_usuario, tipo_valor, valor_actual, metodo_select, id_del_registro) {
...
function sec_con_detalle_aden_arrend_guardar_detalle_adenda(name_modal_close) {
    ...
    if (nombre_campo === "fecha_fin") {
        ...
        if (fechaNueva < fechaOriginal) {
            ...
            $("#modal_adenda_mensaje").html("La fecha suscripción no puede ser menor que la fecha original (" + valor_actual + ").");
            return; 
        }
    }

    if (nombre_campo === "fecha_suscripcion") {
        var fechaOriginal = new Date(valor_actual.split('-').reverse().join('-'));
        var fechaNueva = new Date(valor_date.split('-').reverse().join('-'));
        if (fechaNueva < fechaOriginal) {
            $("#div_modal_adenda_mensaje").show();
	...
	$.ajax({
		url: "/sys/set_contrato_detalle_adenda_locacion.php",
		type: 'POST',
		data: data,
		beforeSend: function() {
			loading("true");
...
function sec_con_detalle_aden_arrend_agignar_propietario(id_persona){
	...
	auditoria_send({ "proceso": "guardar_adenda_detalle_nuevos_registros", "data": data });
	$.ajax({
		url: "/sys/set_contrato_nuevo_adenda_locacion.php",
		type: "POST",
		data: data,
		success: function(response, status) {
			var respuesta = JSON.parse(response);
...
function sec_con_detalle_aden_locacion_eliminar_detalle_adenda(id_adenda){
...
var array_nuevos_files_anexos = [];
function sec_con_detalle_aden_anadir_archivo_locacion() {
...
function sec_modal_adenda_locacion_nuevo_archivo_anexo(tipo_contrato_id, contrato_detalle_id) {
...
function reenviar_solicitud_adenda_locacion(adenda_id) {

	var data = {
		adenda_id : adenda_id,
		accion: 'reenviar_email_solicitud_adenda_locacion',
	};
  swal({
    html:true,
	...
	$.ajax({
		...
		success: function(resp) {
			...
			if (parseInt(respuesta.status) == 200) {
				...
				swal({
					title: "Error al reenviar la Solicitud de Adenda de locacion",
		...
		error: function(textStatus) {
			console.log("La solicitud para reenviar la Solicitud de Adenda de locacion a fallado: " + textStatus);
		},
	});
