...
function sec_contrato_nuevo_adenda_acuerdo_confidencialidad() {
	...
	sec_con_nuevo_aden_acu_conf_obtener_opciones("obtener_proveedor","[name='sec_con_nuevo_proveedor_cont']");
	// sec_con_nuevo_aden_acu_conf_obtener_opciones("obtener_proveedor","[name='sec_con_nuevo_empresa_grupo_at_2']");
	// sec_con_nuevo_aden_acu_conf_obtener_opciones("obtener_bancos","[name='sec_con_nuevo_banco']");
	// sec_con_nuevo_aden_acu_conf_obtener_opciones("obtener_periodo","[name='periodo']");
	sec_con_nuevo_aden_acu_conf_obtener_opciones("obtener_monedas","[name='modal_contr_ade_int_moneda_id']");
	sec_con_nuevo_aden_acu_conf_obtener_opciones("obtener_forma_pago","[name='modal_contr_ade_int_forma_pago']");
	sec_con_nuevo_aden_acu_conf_obtener_opciones("obtener_tipo_comprobante","[name='modal_contr_ade_int_tipo_comprobante']");
	sec_contrato_nuevo_obtener_opciones("obtener_directores", "[name='director_aprobacion_id']");

	setTimeout(function () {
		$('#sec_con_nuevo_proveedor_cont').focus();
		$('#sec_con_nuevo_proveedor_cont').select2("open");
	}, 1000);
}

function sec_con_nuevo_aden_acu_conf_obtener_opciones(accion,select){
	$.ajax({
	   url: "/sys/get_contrato_nuevo_adenda_acuerdo_confidencialidad.php",
	   type: 'POST',
...
function sec_con_nuevo_aden_acu_conf_obtener_contratos() {

	var num_ruc_proveedor = $('#sec_con_nuevo_proveedor_cont').val();
...
function sec_con_nuevo_aden_acu_conf_obtener_contratos_interno_id() {

	var contrato_id = $('#sec_con_nuevo_contrato_id').val();
	let data = {
		accion:'obtener_contrato_interno_por_id',
...
function sec_con_nuevo_aden_acu_conf_solicitud_editar_campo_adenda(nombre_menu_usuario, nombre_tabla, nombre_campo, nombre_campo_usuario, tipo_valor, valor_actual, metodo_select, id_del_registro) {
	...
	if (tipo_valor == 'select_option') {
		...
		if (nombre_campo == 'ubigeo_id') {
			...
			$('#div_adenda_valor_select_option').hide();
			sec_con_nuevo_aden_acu_conf_obtener_opciones(metodo_select,$("[name='adenda_inmueble_id_departamento']"));
			...
			sec_con_nuevo_aden_acu_conf_obtener_opciones(metodo_select,$("[name='adenda_valor_select_option']"));
			setTimeout(function() {
				$('#adenda_valor_select_option').focus();
				$('#adenda_valor_select_option').select2("open");
			}, 1000);
		}   
...
function sec_con_nuevo_aden_acu_conf_guardar_detalle_adenda(name_modal_close) {
	...
	$.ajax({
		url: "/sys/set_contrato_nuevo_adenda_acuerdo_confidencialidad.php",
		type: 'POST',
		data: data,
		beforeSend: function() {
			loading("true");
		...
		success: function(resp) { //  alert(datat)
			...
			if (parseInt(respuesta.http_code) == 200) {
				...
				sec_con_nuevo_aden_acu_conf_asignar_detalle_a_la_adenda(respuesta.result, name_modal_close);
...
function sec_con_nuevo_aden_acu_conf_asignar_detalle_a_la_adenda(id_adenda,modal){
	if (array_adendas_contrato.includes(id_adenda) === false){
		array_adendas_contrato.push(id_adenda)
	}
	
	
	sec_con_nuevo_aden_acu_conf_actualizar_tabla_detalle_adenda();
	
}

function sec_con_nuevo_aden_acu_conf_actualizar_tabla_detalle_adenda(){
...
function sec_con_nuevo_aden_acu_conf_eliminar_detalle_adenda(id_adenda){
...
function sec_con_nuevo_aden_acu_conf_agregar_representante(){
	$('#modalNuevoProveedor').modal({backdrop: 'static', keyboard: false});
	// sec_con_detalle_int_limpiarInputsRL();
}

function sec_con_nuevo_aden_acu_conf_guardar_nuevo_representante_legal(){
	...
	$.ajax({
		...
		success: function(response, status) {
			...
			if (parseInt(respuesta.http_code) == 200) {
				...
				sec_con_nuevo_aden_acu_conf_asignar_detalle_a_la_adenda(respuesta.result, 'modalNuevoProveedor');
			}
		},
...
function sec_con_nuevo_aden_acu_conf_nuevo_contraprestacion_modal() {
...
$("#modal_contr_ade_int_tipo_igv_id").change(function () {
	$("#modal_contr_ade_int_tipo_igv_id option:selected").each(function () {
		...
		if (modal_contr_ade_int_tipo_igv_id != 0) {
			sec_con_nuevo_aden_acu_conf_calcular_subtotal_y_igv(modal_contr_ade_int_tipo_igv_id);
			setTimeout(function() {
...
function sec_con_nuevo_aden_acu_conf_calcular_subtotal_y_igv(tipo)
{
...
function sec_con_nuevo_aden_acu_conf_nuevo_contraprestacion() {
	var contrato_id = $('#id_registro_contrato_id').val();
	...
	$.ajax({
		...
		success: function(response) { //  alert(datat)
			...
			if (parseInt(respuesta.http_code) == 200) {
				$('#frm_adenda_nuevo_proveedor')[0].reset();
				sec_con_nuevo_aden_acu_conf_asignar_detalle_a_la_adenda(respuesta.result, 'modalNuevoContraprestacion');
			}
...
function sec_con_nuevo_aden_acu_conf_guardar_adenda() {
	...
	var arrayJSON = JSON.stringify(array_nuevos_files_anexos_acuerdo);
	...
	auditoria_send({ "proceso": "guardar_adenda_ac", "data": data });
	...
	$.ajax({
		...
		success: function(resp) { 
			var respuesta = JSON.parse(resp);
			auditoria_send({ "proceso": "guardar_adenda_ac", "data": respuesta });
			if (parseInt(respuesta.http_code) == 400) {
				swal({
...
function sec_con_nuevo_aden_acu_conf_guardar_adenda_firmada(adenda_id) {
	...
	dataForm.append("accion","guardar_adenda_acuerdo_confidencialidad_firmada");
	dataForm.append("contrato_id", contrato_id);
	dataForm.append("adenda_id", adenda_id);

	auditoria_send({ "proceso": "guardar_adenda_acuerdo_confidencialidad_firmada", "data": dataForm });
	...
	$.ajax({
		...
		success: function(resp) { //  alert(datat)
			var respuesta = JSON.parse(resp);
			auditoria_send({ "respuesta": "guardar_adenda_acuerdo_confidencialidad_firmada", "data": respuesta });
			console.log(respuesta);
...
var array_nuevos_files_anexos_acuerdo = [];
function anadirArchivo_adenda_acuerdo() {
	console.log('acuerdo')
		...
		var onclick = "borrarArchivo_acuerdo('" + id_nuevo_objeto_nuevo_anexo + "')";
...
function borrarArchivo_acuerdo(id_anexo) {
	//Restamos el número de archivos
	contArchivos = contArchivos - 1;

	array_nuevos_files_anexos_acuerdo = array_nuevos_files_anexos_acuerdo.filter((item) => item.id_objeto !== id_anexo);
	$("div[name=" + id_anexo + "]").remove();
}

function file_acuerdo(event, id) {
	var id_ = "#" + id;
	...
	array_nuevos_files_anexos_acuerdo.push(objeto);

	console.log(array_nuevos_files_anexos_acuerdo);
}
