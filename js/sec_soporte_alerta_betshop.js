$(document).ready(function () {
    if ($('#sec_soporte_alerta_betshop').length == 0) {
    } else {

        fncRenderizarDataTableListadoLocalesAlertaBetShop();
...
function fncRenderizarDataTableListadoLocalesAlertaBetShop() {

    var table = $('#tbl_alerta_betshop_negra_switch').DataTable();
    table.clear();
    table.destroy();
    var table = $('#tbl_alerta_betshop_negra_switch').DataTable({
        'destroy': true,
        ...
        "ajax": {
            type: "POST",
            async: false,
            "url": "sys/get_soporte_alerta_betshop.php",
            "data": { accion: 'sec_soporte_alerta_betshop_listar_locales' }
    ...
    $('#tbl_alerta_betshop_negra_switch tbody').off('click');

    $('#tbl_alerta_betshop_negra_switch tbody').on('click', '.btn', function () {
        ...
        if (data == undefined) {
            ...
            rowData = $('#tbl_alerta_betshop_negra_switch').DataTable().row(selected_row).data();
        } else {
            rowData = data;
        }
