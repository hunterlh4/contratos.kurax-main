$(document).ready(function () {
    if ($('#sec_soporte_alerta_betshop_retail').length == 0) {
    } else {

        fncRenderizarDataTableListadoLocalesAlertaBetShopRetail();
...
function fncRenderizarDataTableListadoLocalesAlertaBetShopRetail() {

    var table = $('#tbl_alerta_betshop_retail_negra_switch').DataTable();
    table.clear();
    table.destroy();
    var table = $('#tbl_alerta_betshop_retail_negra_switch').DataTable({
        'destroy': true,
        ...
        "ajax": {
            type: "POST",
            async: false,
            "url": "sys/get_soporte_alerta_betshop_retail.php",
            "data": { accion: 'sec_soporte_alerta_betshop_retail_listar_locales' }
    ...
    $('#tbl_alerta_betshop_retail_negra_switch tbody').off('click');

    $('#tbl_alerta_betshop_retail_negra_switch tbody').on('click', '.btn', function () {
        ...
        if (data == undefined) {
            ...
            rowData = $('#tbl_alerta_betshop_retail_negra_switch').DataTable().row(selected_row).data();
        } else {
            rowData = data;
        }
        var data = new FormData();		
		data.append('accion', 'sec_soporte_alerta_betshop_cambiar_estado');
