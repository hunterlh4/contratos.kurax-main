...
function sec_contrato_nuevo_adenda_interno() {
	...
	sec_con_nuevo_aden_int_obtener_opciones("obtener_empresa_at","[name='sec_con_nuevo_empresa_grupo_at_1']");
	sec_con_nuevo_aden_int_obtener_opciones("obtener_empresa_at","[name='sec_con_nuevo_empresa_grupo_at_2']");
	// sec_con_nuevo_aden_int_obtener_opciones("obtener_bancos","[name='sec_con_nuevo_banco']");
	// sec_con_nuevo_aden_int_obtener_opciones("obtener_periodo","[name='periodo']");
	sec_con_nuevo_aden_int_obtener_opciones("obtener_monedas","[name='modal_contr_ade_int_moneda_id']");
	sec_con_nuevo_aden_int_obtener_opciones("obtener_forma_pago","[name='modal_contr_ade_int_forma_pago']");
	sec_con_nuevo_aden_int_obtener_opciones("obtener_tipo_comprobante","[name='modal_contr_ade_int_tipo_comprobante']");
}

function sec_con_nuevo_aden_int_obtener_opciones(accion,select){
	$.ajax({
	   url: "/sys/get_contrato_nuevo_adenda_interno.php",
	   type: 'POST',
...
function sec_con_nuevo_aden_int_obtener_contratos() {

	var empresa_grupo_at_1 = $('#sec_con_nuevo_empresa_grupo_at_1').val();
	var empresa_grupo_at_2 = $('#sec_con_nuevo_empresa_grupo_at_2').val();

	let data = {
		accion:'obtener_contratos',
		empresa_grupo_at_1:empresa_grupo_at_1,
		empresa_grupo_at_2:empresa_grupo_at_2,
	};
...
function sec_con_nuevo_aden_int_obtener_contratos_interno_id() {
...
function sec_con_nuevo_aden_int_solicitud_editar_campo_adenda(nombre_menu_usuario, nombre_tabla, nombre_campo, nombre_campo_usuario, tipo_valor, valor_actual, metodo_select, id_del_registro) {
	...
	if (tipo_valor == 'select_option') {
		...
		if (nombre_campo == 'ubigeo_id') {
			...
			$('#div_adenda_valor_select_option').hide();
			sec_con_nuevo_aden_int_obtener_opciones(metodo_select,$("[name='adenda_inmueble_id_departamento']"));
			...
			sec_con_nuevo_aden_int_obtener_opciones(metodo_select,$("[name='adenda_valor_select_option']"));
			setTimeout(function() {
...
function sec_con_nuevo_aden_int_guardar_detalle_adenda(name_modal_close) {
	...
	$.ajax({
		url: "/sys/set_contrato_nuevo_adenda_interno.php",
		type: 'POST',
		data: data,
		beforeSend: function() {
			loading("true");
		...
		success: function(resp) { //  alert(datat)
			...
			if (parseInt(respuesta.http_code) == 200) {
				...
				sec_con_nuevo_aden_int_asignar_detalle_a_la_adenda(respuesta.result, name_modal_close);
...
function sec_con_nuevo_aden_int_asignar_detalle_a_la_adenda(id_adenda,modal){
	if (array_adendas_contrato.includes(id_adenda) === false){
		array_adendas_contrato.push(id_adenda)
	}
	
	
	sec_con_nuevo_aden_int_actualizar_tabla_detalle_adenda();
	
}

function sec_con_nuevo_aden_int_actualizar_tabla_detalle_adenda(){
...
function sec_con_nuevo_aden_int_eliminar_detalle_adenda(id_adenda){
...
function sec_con_nuevo_aden_int_agregar_representante(){
	$('#modalNuevoProveedor').modal({backdrop: 'static', keyboard: false});
	// sec_con_detalle_int_limpiarInputsRL();
}

function sec_con_nuevo_aden_int_guardar_nuevo_representante_legal(){
	...
	$.ajax({
		...
		success: function(response, status) {
			...
			if (parseInt(respuesta.http_code) == 200) {
				...
				sec_con_nuevo_aden_int_asignar_detalle_a_la_adenda(respuesta.result, 'modalNuevoProveedor');
			}
		},
...
function sec_con_nuevo_aden_int_nuevo_contraprestacion_modal() {
...
$("#modal_contr_ade_int_tipo_igv_id").change(function () {
	$("#modal_contr_ade_int_tipo_igv_id option:selected").each(function () {
		...
		if (modal_contr_ade_int_tipo_igv_id != 0) {
			sec_con_nuevo_aden_int_calcular_subtotal_y_igv(modal_contr_ade_int_tipo_igv_id);
			setTimeout(function() {
...
function sec_con_nuevo_aden_int_calcular_subtotal_y_igv(tipo)
{
...
function sec_con_nuevo_aden_int_nuevo_contraprestacion() {
	var contrato_id = $('#id_registro_contrato_id').val();
	...
	$.ajax({
		...
		success: function(response) { //  alert(datat)
			...
			if (parseInt(respuesta.http_code) == 200) {
				$('#frm_adenda_nuevo_proveedor')[0].reset();
				sec_con_nuevo_aden_int_asignar_detalle_a_la_adenda(respuesta.result, 'modalNuevoContraprestacion');
			}
...
function sec_con_nuevo_aden_int_guardar_adenda() {
	...
	var arrayJSON = JSON.stringify(array_nuevos_files_anexos_interno);
...
var array_nuevos_files_anexos_interno = [];
function anadirArchivo_adenda_interno() {
	console.log('arrenda')
		...
		var onclick = "borrarArchivo_interno('" + id_nuevo_objeto_nuevo_anexo + "')";
...
function borrarArchivo_interno(id_anexo) {
	//Restamos el número de archivos
	contArchivos = contArchivos - 1;

	array_nuevos_files_anexos_interno = array_nuevos_files_anexos_interno.filter((item) => item.id_objeto !== id_anexo);
	$("div[name=" + id_anexo + "]").remove();
}

function file_interno(event, id) {
	var id_ = "#" + id;
	...
	array_nuevos_files_anexos_interno.push(objeto);

	console.log(array_nuevos_files_anexos_interno);
}
