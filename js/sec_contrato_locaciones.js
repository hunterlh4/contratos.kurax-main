...
function sec_contrato_locaciones() {
  $(".select2").select2({
    width: "100%",
  });
  ...
  $(".limpiar_select2").click(function () {
    $("#" + $(this).attr("limpiar"))
      ...
      .val("")
      .trigger("change");
    buscarContratoPorParametros_locaciones();
  });
  ...
  sec_contrato_locaciones_listar_contratos_datatable();
...
function buscarContratoPorParametros_locaciones() {
...
function sec_contrato_locaciones_listar_contratos_datatable() {
  ...
  tabla = $("#cont_locales_datatable")
    .dataTable({
      ...
      ajax: {
        url: "/sys/set_contrato_locaciones.php",
        data: data,
        type: "POST",
        dataType: "json",
        error: function (e) {
    ...
    .DataTable();
  mostrar_switch_contrato_locales();
  sec_contrato_locales_mostrarReporteExcel_locacion();
...
function sec_contrato_locales_mostrarReporteExcel_locacion() {
  var id_empresa = $("#search_id_empresa").val();
  var nombre_tienda = $("#search_nombre_tienda").val();
  var centro_costos = $("#search_centro_costos").val();
  var moneda = $("#search_moneda").val();
  ...
  var fecha_fin_suscripcion = $("#fecha_fin_suscripcion").val();
  var etapa = $("#search_etapa").val();
  document.getElementById("cont_locales_excel_locacion").innerHTML =
    '<a href="export.php?export=cont_contrato_locacion&amp;type=lista&amp;' +
...
function contrato_alerta_locacionservicio(id) {
  limpiar_tr_modal_alerta();
  $("#configurarAlerta_locaciones").modal("show");
  var data = {
  ...
  $.ajax({
    ...
    success: function (resp) {
      var respuesta = JSON.parse(resp);
      $("#condicion_economica_id").val(respuesta.idlocacion);
...
function registrar_alerta_locacion() {
  ...
  $.ajax({
    ...
    success: function (resp) {
      var respuesta = JSON.parse(resp);
      if (respuesta == "ok") {
        $("#configurarAlerta_locaciones").modal("hide");
        limpiar_tr_modal_alerta();
        m_reload();
      } else {
        mensajeAlerta(
