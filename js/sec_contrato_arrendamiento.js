...
function sec_contrato_arrendamiento() {
	$(".cont_arrendamiento_param_ruc").mask("00000000000");
	sec_contrato_arrendamiento_cargar_fechas();
	sec_contrato_arrendamiento_cargar_data_ini();
	...
	$('.limpiar_select2').click(function() {
		...
		cont_arrendamiento_buscar_arrendamiento_por_parametros();
	});

	$('.change-contrato-arrendamiento-firmado').on('select2:select', function (e) {
		cont_arrendamiento_buscar_arrendamiento_por_parametros();
	});


	$('#btn_limpiar_filtros_de_busqueda').click(function() {
		$('#cont_arrendamiento_param_empresa').select2().val('').trigger("change");
		$('#cont_arrendamiento_param_area_solicitante').select2().val('').trigger("change");
		$('#cont_arrendamiento_param_ruc').val('');
		$('#cont_arrendamiento_param_razon_social').val('');
		$('#cont_arrendamiento_param_moneda').select2().val('').trigger("change");
...
function sec_contrato_arrendamiento_cargar_fechas() {
	// INICIO INICIALIZACION DE DATEPICKER
	$(".cont_arrendamiento_datepicker").datepicker({
		dateFormat: "yy-mm-dd",
		changeMonth: true,
		changeYear: true,
...
function sec_contrato_arrendamiento_cargar_data_ini() {
	// INICIO INICIALIZACION DE DATEPICKER
	if (sec_id == "contrato" && sub_sec_id == "arrendamiento") {
		var cont_arrendamiento_param_empresa = $("#cont_arrendamiento_param_empresa").val();
		var cont_arrendamiento_param_area_solicitante = $("#cont_arrendamiento_param_area_solicitante").val();
		var cont_arrendamiento_param_ruc = $("#cont_arrendamiento_param_ruc").val();
		var cont_arrendamiento_param_razon_social = $("#cont_arrendamiento_param_razon_social").val();
		var cont_arrendamiento_param_moneda = $("#cont_arrendamiento_param_moneda").val();
		...
		var data = {
			accion: "cont_listar_arrendamientos",
			cont_arrendamiento_param_empresa: cont_arrendamiento_param_empresa,
			cont_arrendamiento_param_area_solicitante: cont_arrendamiento_param_area_solicitante,
			cont_arrendamiento_param_ruc: cont_arrendamiento_param_ruc,
			cont_arrendamiento_param_razon_social: cont_arrendamiento_param_razon_social,
			cont_arrendamiento_param_moneda: cont_arrendamiento_param_moneda,
		...
		$("#div_arrendamiento_boton_export").show();
		$("#cont_contrato_arrendamiento_div_tabla").show();

		$('#cont_locales_arrendamiento_datatable tfoot th').each(function () {
			var title = $(this).text();
			$(this).html('<input type="text" style="width:100%;" placeholder="Buscar ' + title + '" />');
		});

		tabla = $("#cont_locales_arrendamiento_datatable").dataTable({
			language: {
				decimal: "",
			...
			aServerSide: true,
			ajax: {
				url: "/sys/set_contrato_arrendamiento.php",
				data: data,
				type: "POST",
				dataType: "json",
				error: function(e) {
...
function cont_arrendamiento_buscar_arrendamiento_por_parametros() {
	sec_contrato_arrendamiento_listar_contratos_Datatable();
}

function sec_contrato_arrendamiento_listar_contratos_Datatable() {
	if (sec_id == "contrato" && sub_sec_id == "arrendamiento") {
		var cont_arrendamiento_param_empresa = $("#cont_arrendamiento_param_empresa").val();
		var cont_arrendamiento_param_area_solicitante = $("#cont_arrendamiento_param_area_solicitante").val();
		var cont_arrendamiento_param_ruc = $("#cont_arrendamiento_param_ruc").val();
...
$("#cont_arrendamiento_btn_export_arrendamiento").on("click", function() {
	var cont_arrendamiento_param_empresa = $("#cont_arrendamiento_param_empresa").val();
	var cont_arrendamiento_param_area_solicitante = $("#cont_arrendamiento_param_area_solicitante").val();
	var cont_arrendamiento_param_ruc = $("#cont_arrendamiento_param_ruc").val();
	var cont_arrendamiento_param_razon_social = $("#cont_arrendamiento_param_razon_social").val();
	...
	var data = {
		accion: "cont_arrendamiento_reporte_arrendamiento_excel",
		cont_arrendamiento_param_empresa: cont_arrendamiento_param_empresa,
		cont_arrendamiento_param_area_solicitante: cont_arrendamiento_param_area_solicitante,
		cont_arrendamiento_param_ruc: cont_arrendamiento_param_ruc,
		cont_arrendamiento_param_razon_social: cont_arrendamiento_param_razon_social,
...
function sec_contrato_arrendamiento_validar_solo_numeros(e) {
...
function sec_contrato_arrendamiento_alerta(contrato_id) {
	...
	$.ajax({
		...
		success: function(resp) {
			...
			if (parseInt(respuesta.http_code) == 200) {
				...
				body += '<td class="text-center">' + datos_alerta.fecha_vencimiento_arrendamiento + '</td>';
...
function sec_contrato_arrendamiento_registrar_alerta() {
	...
	$.ajax({
		...
		success: function(resp) {
			...
			if (parseInt(respuesta.http_code) == 200) {
				...
				$("#cont_arrendamiento_btn_buscar").click();
			} else {
...
function sec_contrato_arrendamiento_btn_descargar(ruta_archivo)
{
	var extension = "";
