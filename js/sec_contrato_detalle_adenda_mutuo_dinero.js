...
function sec_contrato_detalle_adenda_mutuo_dinero() {
	...
	$('.sec_contrato_nuevo_adenda_datepicker').datepicker({
		dateFormat:'dd-mm-yy',
		changeMonth: true,
		changeYear: true
	})
	...
	sec_con_detalle_aden_arrend_obtener_adenda_detalle_contratos();
	sec_con_detalle_aden_arrend_obtener_adenda_archivos_anexos();

	sec_con_detalle_aden_arrend_obtener_opciones("obtener_tiendas","[name='sec_con_nuevo_tienda']");
	// sec_con_detalle_aden_arrend_obtener_opciones("obtener_empresa_at","[name='sec_con_nuevo_empresa_grupo_at_2']");
	// sec_con_detalle_aden_arrend_obtener_opciones("obtener_bancos","[name='sec_con_nuevo_banco']");
	// sec_con_detalle_aden_arrend_obtener_opciones("obtener_periodo","[name='periodo']");
	// sec_con_detalle_aden_arrend_obtener_opciones("obtener_monedas","[name='modal_contr_ade_int_moneda_id']");
	// sec_con_detalle_aden_arrend_obtener_opciones("obtener_forma_pago","[name='modal_contr_ade_int_forma_pago']");
	// sec_con_detalle_aden_arrend_obtener_opciones("obtener_tipo_comprobante","[name='modal_contr_ade_int_tipo_comprobante']");
		...
		$("#modal_contr_ade_int_tipo_igv_id").change(function () {
			$("#modal_contr_ade_int_tipo_igv_id option:selected").each(function () {
				...
				if (modal_contr_ade_int_tipo_igv_id != 0) {
					sec_con_detalle_aden_arrend_calcular_subtotal_y_igv(modal_contr_ade_int_tipo_igv_id);
					setTimeout(function() {
...
function sec_con_detalle_aden_arrend_obtener_opciones(accion,select){
	$.ajax({
	   url: "/sys/get_contrato_nuevo_adenda_mutuodinero.php",
	   type: 'POST',
...
function sec_con_detalle_aden_arrend_obtener_contratos() {
	...
	$.ajax({
		url: "/sys/get_contrato_nuevo_adenda_arrendamiento.php",
		type: 'POST',
		data: data,
		beforeSend: function () {
		},
...
function sec_con_detalle_aden_arrend_obtener_contratos_interno_id() {
...
function sec_con_detalle_aden_mutuo_dinero_solicitud_editar_campo_adenda(nombre_menu_usuario, nombre_tabla, nombre_campo, nombre_campo_usuario, tipo_valor, valor_actual, metodo_select, id_del_registro) {
	...
	if (tipo_valor == 'select_option') {
		...
		if (nombre_campo == 'ubigeo_id') {
			...
			$('#div_adenda_valor_select_option').hide();
			sec_con_detalle_aden_arrend_obtener_opciones(metodo_select,$("[name='adenda_inmueble_id_departamento']"));
			...
			sec_con_detalle_aden_arrend_obtener_opciones(metodo_select,$("[name='adenda_valor_select_option']"));
			setTimeout(function() {
...
function sec_con_detalle_aden_mutuo_dinero_guardar_detalle_adenda(name_modal_close) {
	...
	if (nombre_campo === "fecha_fin") {
        var fechaOriginal = new Date(valor_actual.split('-').join('-'));
        var fechaNueva = new Date(valor_date.split('-').reverse().join('-'));
        if (fechaNueva < fechaOriginal) {
            $("#div_modal_adenda_mensaje").show();
            $("#modal_adenda_mensaje").html("La fecha de fin no puede ser menor que la fecha original (" + valor_actual + ").");
            return; 
        }
    }

	if (nombre_campo === "plazo_devolucion") {
        var fechaOriginal = new Date(valor_actual.split('-').join('-'));
        var fechaNueva = new Date(valor_date.split('-').reverse().join('-'));
        if (fechaNueva < fechaOriginal) {
            $("#div_modal_adenda_mensaje").show();
            $("#modal_adenda_mensaje").html("La fecha de plazo devolución no puede ser menor que la fecha original (" + valor_actual + ").");
            return; 
        }
    }
	...
	$.ajax({
		url: "/sys/set_contrato_detalle_adenda_mutuo_dinero.php",
		type: 'POST',
		data: data,
		beforeSend: function() {
			loading("true");
		...
		success: function(resp) { //  alert(datat)
			...
			if (parseInt(respuesta.http_code) == 200) {
				...
				sec_con_detalle_aden_arrend_asignar_detalle_a_la_adenda(new_adenda, '');
...
function sec_con_detalle_aden_arrend_asignar_detalle_a_la_adenda(adenda,modal){

	const index = array_adendas_contrato.map(item => item.id).includes(adenda.id);
	if (index == false) {
		array_adendas_contrato.push(adenda)
	}
	sec_con_detalle_aden_arrend_actualizar_tabla_detalle_adenda();
	
}

function sec_con_detalle_aden_arrend_eliminar_detalle_adenda(id_adenda){
...
// function sec_con_detalle_aden_arrend_actualizar_tabla_detalle_adenda(){

// 	if(array_adendas_contrato.length > 0) {

// 		var data = {
// 			"accion": "obtener_adendas_detalle_mutuo_dinero",
// 			"adendas": array_adendas_contrato,
// 		}

// 		auditoria_send({ "proceso": "obtener_adendas_detalle_mutuo_dinero", "data": data });
// 		$.ajax({
// 			url: "/sys/set_contrato_detalle_adenda_mutuo_dinero.php",
// 			type: 'POST',
// 			data: data,
...
var ultimaSolicitud = null;

function sec_con_detalle_aden_arrend_actualizar_tabla_detalle_adenda() {
    // Verificar si hay adendas
    if (array_adendas_contrato.length > 0) {
        var data = {
            "accion": "obtener_adendas_detalle_mutuo_dinero",
            "adendas": array_adendas_contrato, // Envía todas las adendas en un solo array
        };

        // Auditoría (opcional)
        auditoria_send({
            "proceso": "obtener_adendas_detalle_mutuo_dinero",
            "data": data
        });

        // Cancelar la solicitud anterior si existe
        if (ultimaSolicitud !== null) {
            ultimaSolicitud.abort(); // Cancela la solicitud AJAX anterior
        }

        // Realizar la solicitud AJAX
        ultimaSolicitud = $.ajax({
            url: "/sys/set_contrato_detalle_adenda_mutuo_dinero.php",
            type: 'POST',
            data: data,
            beforeSend: function () {
                loading("true"); // Mostrar carga
            },
            complete: function () {
                loading(); // Ocultar carga
            },
            success: function (resp) {
                var respuesta = JSON.parse(resp);
                ...
                // Manejar errores
                if (parseInt(respuesta.http_code) == 400) {
                    console.error("Error: No se pudieron obtener las adendas.");
                    return false;
                }
                ...
                if (parseInt(respuesta.http_code) == 200) {
                    ...
                    if ($.fn.DataTable.isDataTable('#tablaAdendas')) {
                        $('#tablaAdendas').DataTable().destroy();
                    }

                    // Actualizar la tabla con los datos recibidos
                    $('#divTablaAdendas').html(respuesta.result);

                    // Inicializar el datatable con los nuevos datos
                    $('#tablaAdendas').DataTable();
                }
            },
            error: function (jqXHR, textStatus, errorThrown) {
                if (textStatus !== "abort") { // Ignorar errores por cancelación
                    console.error("Error en la solicitud AJAX:", errorThrown);
                }
            }
        });
    } else {
        // Si no hay adendas, limpiar la tabla
        $('#divTablaAdendas').html('');
...
function sec_con_detalle_aden_arrend_agregar_representante(){
	$('#modalNuevoProveedor').modal({backdrop: 'static', keyboard: false});
	// sec_con_detalle_int_limpiarInputsRL();
}

function sec_con_detalle_aden_arrend_guardar_nuevo_representante_legal(){
	...
	$.ajax({
		url: "/sys/set_contrato_detalle_adenda_arrendamiento.php",
		type: "POST",
		data: form_data,
		cache: false,
		contentType: false,
		...
		success: function(response, status) {
			...
			if (parseInt(respuesta.http_code) == 200) {
				...
				sec_con_detalle_aden_arrend_asignar_detalle_a_la_adenda(new_adenda, 'modalNuevoProveedor');
			}
		},
...
function sec_con_detalle_aden_arrend_asignar_detalle_a_la_adenda(id_adenda,modal){
...
function sec_con_detalle_aden_arrend_nuevo_contraprestacion_modal() {
...
function sec_con_detalle_aden_arrend_calcular_subtotal_y_igv(tipo)
{
...
function sec_con_detalle_aden_arrend_nuevo_contraprestacion() {
	var contrato_id = $('#id_registro_contrato_id').val();
	...
	$.ajax({
		...
		success: function(response) { //  alert(datat)
			...
			if (parseInt(respuesta.http_code) == 200) {
				...
				sec_con_detalle_aden_arrend_asignar_detalle_a_la_adenda(new_adenda, 'modalNuevoContraprestacion');
			}
...
function sec_con_detalle_aden_arrend_guardar_adenda() {
...
////// new
function sec_con_detalle_aden_arrend_buscar_propietario_modal(tipo,id_propiertario_old,id_persona_id) {
...
function  sec_con_detalle_aden_arrend_buscar_propietario() {
...
function sec_con_detalle_aden_arrend_agignar_propietario(id_persona){
	var contrato_id = $('#id_registro_contrato_id').val();
	...
	auditoria_send({ "proceso": "guardar_adenda_detalle_nuevos_registros", "data": data });
	$.ajax({
		url: "/sys/set_contrato_nuevo_adenda_arrendamiento.php",
		type: "POST",
		data: data,
		success: function(response, status) {
			var respuesta = JSON.parse(response);
			...
			if (parseInt(respuesta.http_code) == 200) {		
				...
				sec_con_detalle_aden_arrend_asignar_detalle_a_la_adenda(new_adenda, 'modalBuscarPropietario_ca');
...
function sec_con_detalle_aden_arrend_reemplazar_propietario(id_persona){
...
function sec_con_detalle_aden_arrend_modal_eliminar_propietario(id_propietario){
...
function sec_con_detalle_aden_arrend_guardar_nuevo_representante_legalaaaaaa(){
	var contrato_id = $('#id_registro_contrato_id').val();
...
function sec_con_detalle_aden_arrend_propietario_modal() {
	...
	sec_con_detalle_aden_arrend_resetear_formulario_nuevo_propietario_agente();
...
function sec_con_detalle_aden_arrend_resetear_formulario_nuevo_propietario_agente() {
...
function sec_con_detalle_aden_arrend_nuevo_propietario() {
	...
	$.ajax({
		...
		success: function (resp) {
			...
			if (parseInt(respuesta.http_code) == 200) {
				...
				sec_con_detalle_aden_arrend_asignar_detalle_a_la_adenda(new_adenda, 'modalNuevoPropietario_aa');
			}
		},
		error: function () {},
	});
...
////BENEFICIARIOS
function sec_con_detalle_aden_arrend_modal_nuevo_beneficiario(tipo,id_beneficiario, contrato_detalle_id) {	
	...
	sec_con_detalle_aden_arrend_modal_resetear_formulario_nuevo_beneficiario("new");
...
function sec_con_detalle_aden_arrend_modal_resetear_formulario_nuevo_beneficiario(evento) {
...
function sec_con_detalle_aden_arrend_guardar_beneficiario() {
	var contrato_id = $('#id_registro_contrato_id').val();
	...
	$.ajax({
		...
		success: function (resp) {
			...
			if (parseInt(respuesta.http_code) == 200) {
				...
				sec_con_detalle_aden_arrend_asignar_detalle_a_la_adenda(new_adenda, 'modalNuevoBeneficiario');
				sec_con_detalle_aden_arrend_modal_resetear_formulario_nuevo_beneficiario();
			}
		},
		error: function () {},
	});
...
function sec_con_detalle_aden_arrend_modal_eliminar_beneficiario(id_beneficiario,contrato_detalle_id){
...
////BENEFICIARIOS
function sec_con_detalle_aden_arrend_modal_nuevo_responsable_ir(tipo,id_responsable_ir, contrato_detalle_id) {	
	$('#modal_responsable_ir_id_responsable_ir_para_cambios').val(id_responsable_ir);
	...
	sec_con_detalle_aden_arrend_modal_resetear_formulario_nuevo_responsable_ir("new");
...
function sec_con_detalle_aden_arrend_modal_resetear_formulario_nuevo_responsable_ir(evento) {
...
function sec_con_detalle_aden_arrend_guardar_responsable_ir() {

	var contrato_id = $('#id_registro_contrato_id').val();
	var id_responsable_ir_para_cambios = $("#modal_responsable_ir_id_responsable_ir_para_cambios").val();
	var tipo_solicitud = $("#modal_responsable_ir_tipo_solicitud").val();
	var contrato_detalle_id = $("#modal_responsable_ir_contrato_detalle_id").val();
	...
	$.ajax({
		...
		success: function (resp) {
			...
			if (parseInt(respuesta.http_code) == 200) {
				...
				sec_con_detalle_aden_arrend_asignar_detalle_a_la_adenda(new_adenda, 'modalNuevoResponsableIR');
				sec_con_detalle_aden_arrend_modal_resetear_formulario_nuevo_responsable_ir('new');
			}
		},
		error: function () {},
...
function sec_con_detalle_aden_arrend_modal_eliminar_responsable_ir(id_responsable_ir, contrato_detalle_id){
...
// INICIO INCREMENTOS
function sec_con_detalle_aden_arrend_modal_agregar_incrementos(contrato_detalle_id) {
	sec_con_detalle_aden_arrend_resetear_formulario_nuevo_incremento("new");
...
function sec_con_detalle_aden_arrend_resetear_formulario_nuevo_incremento(evento) {
...
function sec_con_detalle_aden_arrend_solicitud_guardar_incremento() {
	...
	$.ajax({
		...
		success: function (resp) {
			...
			if (parseInt(respuesta.http_code) == 200) {
				...
				sec_con_detalle_aden_arrend_asignar_detalle_a_la_adenda(new_adenda, 'modal_adenda_agregar_incrementos');	
...
// INICIO ENMIENDA
function sec_con_detalle_aden_arrend_solicitud_guardar_enmienda() {
}
// FIN ENMIENDA
...
function sec_con_detalle_aden_arrend_importar_contratos() {
	var data = {
...
// INICIO ENMIENDA
function sec_con_detalle_aden_arrend_modal_agregar_enmienda() {
	$("#modal_adenda_agregar_enmienda").modal({ backdrop: "static", keyboard: false });
}
...
/// INICIO INFLACION
function sec_con_detalle_aden_arrend_modal_agregar_inflacion(contrato_detalle_id) {
...
function sec_con_detalle_aden_arrend_agregar_inflacion() {
	...
	$.ajax({
		...
		success: function(resp) { //  alert(datat)
			...
			if (parseInt(respuesta.http_code) == 200) {
				...
				sec_con_detalle_aden_arrend_asignar_detalle_a_la_adenda(new_adenda, 'modalAgregarInflacion');
				document.getElementById('divTablaAdendas').focus();
				return false;
			}
...
function sec_con_detalle_aden_arrend_eliminar_inflacion(inflacion_id,contrato_detalle_id) {
...
function sec_con_detalle_aden_arrend_modal_agregar_cuota_extraordinaria(contrato_detalle_id) {
...
function sec_con_detalle_aden_arrend_agregar_cuota_extraordinaria() {
	...
	$.ajax({
		...
		success: function(resp) { //  alert(datat)
			...
			if (parseInt(respuesta.http_code) == 200) {
				...
				sec_con_detalle_aden_arrend_asignar_detalle_a_la_adenda(new_adenda, 'modalAgregarCuotaExtraordinaria');
				document.getElementById('divTablaAdendas').focus();
				return false;
			}
...
function sec_con_detalle_aden_arrend_eliminar_cuota_extraordinaria(cuota_extraordinaria_id, contrato_detalle_id) {
...
/// INICIO SUMINSITROS
function sec_con_detalle_aden_arrend_modal_agregar_suministro(tipo_servicio, inmueble_id, contrato_detalle_id) {
...
function sec_con_detalle_aden_arrend_modal_change_tipo_compromiso() {
	var tipo_compromiso = $('#modal_suministro_compromiso_pago').val();
	if (tipo_compromiso == 1 || tipo_compromiso == 2 || tipo_compromiso == 6 || tipo_compromiso == 7) {
...
function sec_con_detalle_aden_arrend_agregar_suministro() {
...
function sec_con_detalle_aden_arrend_eliminar_suministro(suministro_id,contrato_detalle_id) {
...
function sec_con_detalle_aden_arrendamiento_eliminar_detalle_adenda(id_adenda){
...
var array_nuevos_files_anexos = [];
function sec_con_detalle_aden_anadir_archivo_arrendamiento() {
	...
	var tipo_archivo_nombre = $('#modal_nuevo_select_archivo_anexo').find('option:selected').text();
	...
	var name_file = tipo_archivo_id +"_"+ contrato_detalle_id+ "_anexo"+Tiempo;
	//var onclick = "sec_nuevo_modal_eliminar_nuevo_anexo('" + id_nuevo_objeto_nuevo_anexo + "')";

	var onclick = "borrarArchivo_proveedor('" + id_nuevo_objeto_nuevo_anexo + "')";
	...
	html +=
		'<input name="'+name_file+'" type="file" id="' +
...
function sec_modal_adenda_arrendamiento_nuevo_archivo_anexo(tipo_contrato_id, contrato_detalle_id) {
...
function sec_con_detalle_aden_arrend_obtener_adenda_detalle_contratos() {
		...
		success: function (datos) {
			...
			if (respuesta.status == 200) {
				for (let index = 0; index < respuesta.result.length; index++) {
					sec_con_detalle_aden_arrend_asignar_detalle_a_la_adenda(respuesta.result[index],'');
				}
			}
		},
		error: function () {
...
function sec_con_detalle_aden_arrend_obtener_adenda_archivos_anexos() {
...
function sec_con_detalle_aden_arrend__eliminar_archivo_anexo(archivo_id) {
...
function reenviar_solicitud_adenda_arrendamiento(adenda_id) {

	var data = {
		adenda_id : adenda_id,
		accion: 'reenviar_email_solicitud_adenda_arrendamiento',
	};
  swal({
    html:true,
	...
	$.ajax({
		...
		success: function(resp) {
			...
			if (parseInt(respuesta.status) == 200) {
				...
				swal({
					title: "Error al reenviar la Solicitud de Adenda de Arrendamiento",
		...
		error: function(textStatus) {
			console.log("La solicitud para reenviar la Solicitud de Adenda de Arrendamiento a fallado: " + textStatus);
		},
	});
