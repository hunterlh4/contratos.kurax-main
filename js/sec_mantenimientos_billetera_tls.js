...
function sec_mantenimientos_billetera_tls() {	
	if (sec_id == 'mantenimientos' && sub_sec_id=='billetera_tls') {
...
$(function () {
	...
	if (sec_id == 'mantenimientos' && sub_sec_id=='billetera_tls') {
		$('#mant_bil_cliente_fecha_inicio').datetimepicker({
			format: 'YYYY-MM-DD'
		});
		$('#mant_bil_cliente_fecha_fin').datetimepicker({
			format: 'YYYY-MM-DD'
		});
		$('#mant_bil_cliente_fecha_inicio').val($('#mant_bil_g_fecha_hace_7_dias').val());
		$('#mant_bil_cliente_fecha_fin').val($('#mant_bil_g_fecha_actual').val());

		$('#cliente_local').select2();

		$('#mant_bil_cliente_fecha_inicio').change(function () {
			var var_fecha_change = $('#mant_bil_cliente_fecha_inicio').val();
			if (!(parseInt(var_fecha_change.length) > 0)) {
				$("#mant_bil_cliente_fecha_inicio").val($("#mant_bil_g_fecha_hace_7_dias").val());
			}
		});
		$('#mant_bil_cliente_fecha_fin').change(function () {
			var var_fecha_change = $('#mant_bil_cliente_fecha_fin').val();
			if (!(parseInt(var_fecha_change.length) > 0)) {
				$("#mant_bil_cliente_fecha_fin").val($("#mant_bil_g_fecha_actual").val());
			}
		});

		//$('#mb_modal_add_reason_new').hide();

		$('input:radio[name=buscador_tipo]').on('change', function () {
			setTimeout(function () {
				$("#mant_bil_buscador_lbl_mensaje").html('');
				$("#mant_bil_buscador_texto").val(''); // Limpiar input
				$("#mant_bil_buscador_texto").removeAttr('maxLength');

				var busc_tipo = $("input:radio[name='buscador_tipo']:checked").val(); //Obtener valor del radio seleccionado
				// Celular
				if (parseInt(busc_tipo) == 9) {
					$("#mant_bil_buscador_texto").attr('maxLength', '9');
				}
				// DNI
				if (parseInt(busc_tipo) == 0) {
					$("#mant_bil_buscador_texto").attr('maxLength', '8');
				}
				// CE/PTP
				if (parseInt(busc_tipo) == 1) {
					$("#mant_bil_buscador_texto").attr('maxLength', '12');
				}
				// PASAPORTE
				...
				$("#mant_bil_buscador_texto").focus(); // Dar foco al input
				return false;
			}, 100);
		});

		$('#mant_bil_form_consultar').on('submit', function () {
			mant_bil_buscar_cliente();
			return false;
			
		});

		$("#mb_modal_nuevo_balance").on({
		...
		$('#mant_bil_btn_actualizar_tabla_transacciones').click(function () {
			mant_bil_listar_transacciones(gen_cliente_id);
		});

		$("#modal_sbBal_balance_tipo").on({
			"change": function (event) {
				$('#modal_sbBal_balance_tipo_actual_div').hide();
				$('#modal_sbBal_balance_tipo_nuevo_div').hide();
				var temp_tipo_balance = $(event.target).val();
				if(parseInt(temp_tipo_balance)>0){
					if(parseInt(temp_tipo_balance)===4){
						$('#modal_sbBal_balance_tipo_actual_texto').html('Balance No Retirable Actual S/');
						$('#modal_sbBal_balance_tipo_nuevo_texto').html('Nuevo Balance No Retirable S/');
					}
					if(parseInt(temp_tipo_balance)===5){
						$('#modal_sbBal_balance_tipo_actual_texto').html('Balance Retirable Actual S/');
						$('#modal_sbBal_balance_tipo_nuevo_texto').html('Nuevo Balance Retirable S/');
					}
					if(parseInt(temp_tipo_balance)===6){
						$('#modal_sbBal_balance_tipo_actual_texto').html('Dinero AT Actual S/');
						$('#modal_sbBal_balance_tipo_nuevo_texto').html('Nuevo Dinero AT S/');
					}
					$('#modal_sbBal_balance_tipo_actual_div').show();
					$('#modal_sbBal_balance_tipo_nuevo_div').show();
				}
				modal_sbBal_monto_onchange();
				//console.log('focus');
			}
		});

		$("#modal_sbBal_monto").on({
			"focus": function (event) {
				$(event.target).select();
				//console.log('focus');
			},
...
var array_clientes = [];
function mant_bil_buscar_cliente() {
	$("#mant_bil_buscador_lbl_mensaje").html('');

	var busc_tipo = $("input:radio[name='buscador_tipo']:checked").val();
	var buscador_texto = $.trim($("#mant_bil_buscador_texto").val());

	// Celular
	if (parseInt(busc_tipo) == 9) {
		if (buscador_texto.length !== 9) {
			$("#mant_bil_buscador_lbl_mensaje").html('El número de celular debe tener 9 dígitos.');
			$("#mant_bil_buscador_texto").focus();
			return false;
		}
		if (!(parseInt(buscador_texto) > 900000000 && parseInt(buscador_texto) <= 999999999)) {
			$("#mant_bil_buscador_lbl_mensaje").html('Número de celular inválido.');
			$("#mant_bil_buscador_texto").focus();
			return false;
		}
	}
	...
	if (parseInt(busc_tipo) == 0) {
		if (buscador_texto.length !== 8) {
			$("#mant_bil_buscador_lbl_mensaje").html('El número de DNI debe tener 8 dígitos.');
			$("#mant_bil_buscador_texto").focus();
			return false;
		}
		if (!(parseInt(buscador_texto) >= 1 && parseInt(buscador_texto) <= 99999999)) {
			$("#mant_bil_buscador_lbl_mensaje").html('Número de DNI inválido.');
			$("#mant_bil_buscador_texto").focus();
			return false;
		}
	}
	...
	if (parseInt(busc_tipo) == 1) {
		if (buscador_texto.length < 4) {
			$("#mant_bil_buscador_lbl_mensaje").html('El número de CE/PTP debe tener al menos 4 dígitos.');
			$("#mant_bil_buscador_texto").focus();
			return false;
		}
		if (!(parseInt(buscador_texto) >= 0 && parseInt(buscador_texto) <= 999999999)) {
			$("#mant_bil_buscador_lbl_mensaje").html('Número de CE/PTP inválido.');
			$("#mant_bil_buscador_texto").focus();
			return false;
		}
	}
	...
	if (parseInt(busc_tipo) == 2) {
		if (!(buscador_texto.length > 0)) {
			$("#mant_bil_buscador_lbl_mensaje").html('El número de PASAPORTE no debe estar vacío.');
			$("#mant_bil_buscador_texto").focus();
			return false;
		}
	}
	...
	if (parseInt(busc_tipo) == 8) {
		if (!(buscador_texto.length > 0)) {
			$("#mant_bil_buscador_lbl_mensaje").html('El número de ID-WEB no debe estar vacío.');
			$("#mant_bil_buscador_texto").focus();
			return false;
		}
		if (!(parseInt(buscador_texto) > 0)) {
			$("#mant_bil_buscador_lbl_mensaje").html('ID-WEB inválido.');
			$("#mant_bil_buscador_texto").focus();
			return false;
		}
	}
	...
	if (parseInt(buscador_cliente_id) === 0) {
		$.ajax({
			url: "/sys/set_mantenimientos_billetera_tls.php",
			type: 'POST',
			data: data,
			beforeSend: function () {
				loading("true");
		...
		$("#mant_bil_buscador_texto").val('');
		mant_bil_seleccionar_cliente(buscador_cliente_id, false);
	}
}
...
function mant_bil_seleccionar_cliente(id_cliente, nuevo) {
	...
	$('.mant_bil_div_etiquetas').show();
	$('.mant_bil_div_cliente').show();
	$('.mant_bil_div_transacciones').show();
	...
	$.each(array_clientes, function (index, item) {
		if (parseInt(item.id) === parseInt(id_cliente)) {
			...
			mant_bil_cargar_titular_abono_cliente();
	...
	$('.mant_bil_div_tab_cliente').removeClass('active');
	if (nuevo === true) {
		$('#mant_bil_div_tabs_contenedor').append(
				'<div class="col-md-4 div_tab" id="' + id_cliente + '">' +
				'<div class="row mant_bil_div_tab_cliente active">' +
				'<div class="col-md-10 col-xs-10 mant_bil_div_tab_cliente_texto" onclick="mant_bil_seleccionar_cliente(' + id_cliente + ', false)">' + titulo_tab + '</div>' +
				'<div class="col-md-2 col-xs-2 mant_bil_div_tab_cliente_close" onclick="mant_bil_remover_cliente(' + id_cliente + ')">' +
		...
		$('#' + id_cliente).find('.mant_bil_div_tab_cliente').addClass('active');
	}

	mant_bil_limpiar_campos_div_cliente();
	...
	mant_bil_bloquear_div_cliente();

	mant_bil_listar_transacciones(id_cliente);
}

function mant_bil_remover_cliente(id_cliente) {
	$('#' + id_cliente).remove();
	if (parseInt(gen_cliente_id) === parseInt(id_cliente)) {
		$('#mant_bil_div_resultado').hide();
	}
...
function mant_bil_listar_transacciones(id_cliente) {
	var div_sec_televentas_fecha_inicio = $('#mant_bil_cliente_fecha_inicio').val();
	var div_sec_televentas_fecha_fin = $('#mant_bil_cliente_fecha_fin').val();
	var tipo_balance = $("#mant_bil_cbx_tipo_balance").val();
	if (div_sec_televentas_fecha_inicio.length !== 10) {
		$("#mant_bil_cliente_fecha_inicio").focus();
		return false;
	}
	if (div_sec_televentas_fecha_fin.length !== 10) {
		$("#mant_bil_cliente_fecha_fin").focus();
		return false;
	}

	mant_bil_limpiar_tabla_transacciones();
	$.post("/sys/set_mantenimientos_billetera_tls.php", {
		...
		tipo_balance: tipo_balance
			...
			.done(function (data) {
				try {
					...
					if (parseInt(respuesta.http_code) == 400) {
						$('#mant_bil_tabla_transacciones').append(
								'<tr>' +
					...
					if (parseInt(respuesta.http_code) == 200) {
						...
						respuesta.result_balance_dinero_at = respuesta.result_balance_dinero_at.replace(/\D/g, "")
								.replace(/([0-9])([0-9]{2})$/, '$1.$2')
								.replace(/\B(?=(\d{3})+(?!\d)\.?)/g, ",");
						$('#mant_bil_span_dinero_at').html(respuesta.result_balance_dinero_at);
						...
						if (respuesta.result.length > 0) {
							$.each(respuesta.result, function (index, item) {
								...
								var variables = "'" + item.trans_id + "','" + item.txn_id + "','" + item.tipo_id + "','" + web_id + "','" +
										...
										"'" + item.nuevo_balance + "', '" + item.id_tipo_balance + "','" + item.tipo_transaccion + "'";
								var btn_ver = '';
								if (parseInt(item.tipo_id) === 21 || parseInt(item.tipo_id) === 22 || parseInt(item.tipo_id) === 24 || parseInt(item.tipo_id) === 25){
									btn_ver = '<button type="button" class="btn btn-primary" style="padding: 2px 3px;"' +
											'    onclick="mant_bil_ver_voucher(' + variables + ')">' +
								...
								if(parseInt(item.tipo_id) == 38 || parseInt(item.tipo_id) == 39){
									btn_ver = '<button type="button" class="btn btn-primary" style="padding: 2px 3px;"' +
											'    onclick="mant_bil_ver_voucher(' + variables + ')">' +
											'<span class="fa fa-eye"></span>' +
											'</button>';
							...
							tabla_transacciones_datatable_formato('#mant_bil_tabla_transacciones');
							/*$('#mant_bil_tabla_transacciones').append(
...
function mant_bil_limpiar_campos_div_cliente() {
...
function mant_bil_limpiar_tabla_transacciones() {
	$('#mant_bil_tabla_transacciones').html(
			'<thead>' +
			'<tr>' +
...
function mant_bil_limpiar_tabla_diferencias() {
	$('#mant_bil_tabla_diferencias_balance').html(
			'<thead>' +
			'<tr>' +
			'<td class="text-center">Tipo Documento</td>' +
			'<td class="text-center">Número Documento</td>' +
			'<td class="text-center">Nombre</td>' +
			'<td class="text-center">Apellido Paterno</td>' +
			'<td class="text-center">Apellido Materno</td>' +
			'<td class="text-right">Balance General</td>' +
			'<td class="text-right">Balance No Retirable</td>' +
			'<td class="text-right">Balance Retirable</td>' +
			'<td class="text-right">Balance Bono AT</td>' +
			'</tr>' +
			'</thead>' +
			'<tbody>'
			);
...
function mant_bil_bloquear_div_cliente() {
...
function mant_bil_estado_cuenta() {
	$('#mant_bil_modal_estado_cuenta').modal('show');
...
$("#mant_bil_cbx_tipo_balance").on("change", function(){
	var tls_tipo_balance_id_temp = $("#mant_bil_cbx_tipo_balance").val();
	mant_bil_listar_transacciones(gen_cliente_id);
})

function mant_bil_cargar_titular_abono_cliente() {
	$('#sec_tlv_hist_cliente_tercero_titular').empty();
...
function mant_bil_ver_voucher(trans_id, txn_id, tipo_id, web_id, cliente, monto_deposito, monto_comision, monto, bono_nombre, bono_monto, total,
		fecha_hora, estado, local, obs_cajero, obs_validador, obs_supervisor, tipo_rechazo, registro_deposito, num_operacion, banco_pago,
		proveedor_id, proveedor_name, usuario, cajero, tipo_operacion, motivo_dev, img_comision, tipo_jugada, nuevo_balance, tipo_balance, tipo_transaccion) {
    ...
    if([38,39].includes(parseInt(tipo_id))){
    	var nombre_tipo_balance = '';
    	switch(parseInt(tipo_balance)){
    		case 1:
    			nombre_tipo_balance = 'Balance General';
    			break;
    		case 4:
    		...
    		case 5:
    			...
    			break;
    		case 6:
    			nombre_tipo_balance = 'Balance Bono AT';
    			break;
    		default:
    			nombre_tipo_balance = 'Balance';
    	};

    	$('#mb_modal_correc_transaccion').html(tipo_transaccion);
    	$('#mb_modal_correc_tipo_balance').html(nombre_tipo_balance);
    	$('#mb_modal_correc_monto').html(monto);
    	$('#mb_modal_correc_nuevo_balance').html(nuevo_balance);
    	$('#mb_modal_correc_observacion').html(obs_cajero);
    	$('#mb_modal_correc_usuario').html(usuario);
        $('#mant_bil_modal_voucher_correccion_balance').modal();
    }
}

$('#mb_btn_edit_balance').click(function(){
	$('#mb_modal_select_tipo_balance').val('0').trigger('change');
	$('#mb_modal_balance_actual').val('0.00');
	$('#mb_modal_nuevo_balance').val('');
	$('#mb_modal_observacion_edit').val('');
	$('#mb_modal_btn_guardar_edit_balance').show();
	$('#mb_modal_edit_balance').modal('show');
});

$('#mb_btn_subir_balance').click(function(){
	mb_modal_sbBal('up_balance');
});

$('#mb_btn_bajar_balance').click(function(){
	mb_modal_sbBal('down_balance');
});

$('#mant_bil_btn_consultar_dif_cliente').click(function(){
	$('#mant_bil_tabla_diferencias_balance').html('');
	$('#mb_modal_ver_dif_balance').modal('show');

	$('#mant_bil_tabla_diferencias_balance').append(
		'<tr>' +
		'<td colspan="7" class="text-center" style="font-size: 16px;color: #716363;font-style: italic;">Buscando diferencias en los balances del cliente...</td>' +
		'</tr>'
		);	
	...
	setTimeout(function() {
		...
		$.post("/sys/set_mantenimientos_billetera_tls.php", {
			accion: "obtener_diferencias_balance"
		})
		.done(function (data) {
			try {
				var respuesta = JSON.parse(data);
				...
				if (parseInt(respuesta.http_code) == 200) {
					mant_bil_limpiar_tabla_diferencias();
					if (respuesta.result.length > 0) {
						$.each(respuesta.result, function (index, item) {
							...
							$('#mant_bil_tabla_diferencias_balance').append(
									...
									'<td class="text-center">' + item.apellido_paterno + '</td>' +
									'<td class="text-right">' + item.apellido_materno + '</td>' +
									'<td class="text-right">' + item.balance + '</td>' +
									'<td class="text-right">' + item.balance_deposito + '</td>' +
									'<td class="text-right">' + item.balance_retiro_disponible + '</td>' +
									'<td class="text-right">' + item.balance_bono_at + '</td>' +
									'</tr>'
									);
						});
						tabla_transacciones_datatable_formato('#mant_bil_tabla_diferencias_balance');
						$('#mant_bil_tabla_diferencias_balance').append(
							'<button class="btn btn-success" onclick="exportar_tabla_diferencias_balance()" style="width: 100%;margin: 20px;">' +
							'<span class="glyphicon glyphicon-download-alt"></span> Excel' +
							'</button>'
							);
					} else {
...
function exportar_tabla_diferencias_balance() {
 
	var data = {
        "accion": "tabla_diferencias_balance_export_xls" 
    }
	$.ajax({
        url: "/sys/set_mantenimientos_billetera_tls.php",
        type: 'POST',
...
$('#mb_modal_select_tipo_balance').on('change', function(){
	try{
		const balance_type = $(this).val();
		if(balance_type == 0){
			$('#mb_modal_balance_actual').css('color', 'black');
			$('#mb_modal_balance_actual').val('0.00');
			return;
		}
		//Obtener el monto del balance seleccionado
		var data = {
			"accion" : "sec_mant_bil_obtener_balance_x_tipo",
			"tipo_balance" : balance_type,
			"id_cliente" : gen_cliente_id
		}
		...
		$.ajax({
			...
			success: function (resp) {
				...
				if (parseInt(respuesta.http_code) == 200) {
					const balance = respuesta.balance;
					...
					$('#mb_modal_balance_actual').css('color', color);
					$('#mb_modal_balance_actual').val(balance);
				}
				return false;
			},
			error: function (result) {
		...
		console.log("Error - select tipo balance: " + error);
	}
});

$('#mb_modal_btn_guardar_edit_balance').click(function(){
	try{
		$('#mb_modal_btn_guardar_edit_balance').hide();
		const balance_type_text = $('#mb_modal_select_tipo_balance option:selected').text();
		const balance_type = $('#mb_modal_select_tipo_balance').val();
		var monto_actual = $('#mb_modal_balance_actual').val().replace(/\,/g, '');;
		var monto_nuevo = $('#mb_modal_nuevo_balance').val().replace(/\,/g, '');;

		if(balance_type == 0){
			swal('Aviso','Debe seleccionar el tipo de balance a editar','warning');
			$('#mb_modal_btn_guardar_edit_balance').show();
			return;
		}
		if(monto_nuevo == ''){
		...
		if(parseFloat(monto_nuevo) < 0 ){
			swal('Aviso','El monto nuevo debe ser mayor o igual a 0','warning');
			$('#mb_modal_btn_guardar_edit_balance').show();
			return;
		}

		if(parseFloat(monto_nuevo) == parseFloat(monto_actual)){
			swal('Aviso','El monto ingresado es igual al monto actual','warning');
			$('#mb_modal_btn_guardar_edit_balance').show();
			return;
		}
		...
		var monto_actual_text = monto_actual.replace(/\D/g, "")
								.replace(/([0-9])([0-9]{2})$/, '$1.$2')
								.replace(/\B(?=(\d{3})+(?!\d)\.?)/g, ",");
		var monto_nuevo_text = monto_nuevo.replace(/\D/g, "")
								.replace(/([0-9])([0-9]{2})$/, '$1.$2')
								.replace(/\B(?=(\d{3})+(?!\d)\.?)/g, ",");

		var nombre_cliente = gen_cliente_nombres;
		var num_doc_cliente = $('#cliente_num_doc').val();
		var tipo_documento_cliente = $('#cliente_tipo_doc option:selected').text();
		var observacion = $('#mb_modal_observacion_edit').val();
		swal({
			html: true,
			title: '¿Desea actualizar el saldo del <br><b>' + balance_type_text + '</b> <br> De <b>' + sig + monto_actual_text + '</b> a <b>' + monto_nuevo_text + '</b>' ,
			text: "",
			type: 'warning',
			showCancelButton: true,
			confirmButtonColor: '#0336FF',
			cancelButtonColor: '#d33',
			confirmButtonText: 'SI, ACTUALIZAR SALDO',
			cancelButtonText: 'CANCELAR',
			closeOnConfirm: false
		}, function (isConfirmed) {
			if (isConfirmed){
				var data = {
					"accion" : "sec_mant_bil_set_balance_edit",
					"tipo_balance" : balance_type,
					"id_cliente" : gen_cliente_id,
					"monto" : monto_nuevo,
					"monto_anterior" : sig + monto_actual,
					"nombre_balance" : balance_type_text,
					"cliente_nombre" : nombre_cliente,
					"num_doc_cliente" : num_doc_cliente,
					"tipo_doc" : tipo_documento_cliente,
				...
				$.ajax({
					...
					success: function (resp) {
						...
						if (parseInt(respuesta.http_code) == 200) {
							$('#mb_modal_edit_balance').modal('hide');
							mant_bil_listar_transacciones(gen_cliente_id);
		...
		console.log("Error - editar balance: " + error);
	}
});

$('#mv_btn_add_reason_edit_balance').click(function(){
	$('#mb_modal_select_tipo_edit_balance').val('0').trigger('change');
	/*$('#mb_modal_balance_actual').val('0.00');
	$('#mb_modal_nuevo_balance').val('');
	$('#mb_modal_observacion_edit').val('');
	$('#mb_modal_btn_guardar_edit_balance').show();*/
	$('#mb_modal_add_reason_edit_balance').modal('show');
});

$('#mb_modal_select_tipo_edit_balance').on('change', function(){
	var tipo_edit = $('#mb_modal_select_tipo_edit_balance option:selected').val();
	/*try{
		var data = {
			"accion": "listar_motivos_balance",
			"tipo_edit": tipo_edit
		}
		$.ajax({
			url: "/sys/set_mantenimientos_billetera_tls.php",
			...
			success: function (datos) {
				...
				$('#mb_modal_table_reasons_edit_balance tbody').html('');
				$(respuesta.result).each(function (index, item) {
				  $('#mb_modal_table_reasons_edit_balance tbody').append(
				  		'<tr>' + 
				  		'	<td>' + item.id + '</td>' +
				  		'	<td>' + item.nombre + '</td>' +
				  		'	<td>' + item.estado + '</td>' +
				...
				//tabla_datatable_formato('#mb_modal_table_reasons_edit_balance');
			},
		  error: function () {},
		});
	}catch(error){
		console.log("Error: mb_modal_select_tipo_edit_balance on change: " + error);
	}*/
});

$('#mb_modal_add_reason').click(function(){
	//$('#mb_modal_add_reason_new').show();
	var tipo_edit = $('#mb_modal_select_tipo_edit_balance option:selected').val();
	if(tipo_edit == 0){
		swal('Aviso', 'Debe seleccionar un tipo de edición', 'warning');
		return;
	}

	var nombre = $('#mb_modal_nombre_reason').val();
	if($.trim(nombre) == ''){
		swal('Aviso', 'Debe ingresar el nombre del motivo', 'warning');
		return;
	}
	try{
		var data = {
			"accion": "guardar_motivo_edit_balance",
			"tipo_edit": tipo_edit,
			"nombre": nombre
		}
		$.ajax({
			...
			success: function (datos) {
				...
				if(respuesta.http_code == 200){
					$('#mb_modal_nombre_reason').val('');
					$('#mb_modal_add_reason_edit_balance').modal('hide');
...
///////// SUBIR Y BAJAR BALANCE

function mb_modal_sbBal(tipo_transaccion) {
	modal_sbBal_tipo_transaccion = tipo_transaccion;
	limpiar_bordes_modal_sbBal();
	limpiar_campos_modal_sbBal();
	$('#modal_sbBal_balance_tipo_actual_div').hide();
	$('#modal_sbBal_balance_tipo_nuevo_div').hide();
	$('#modal_sbBal_btn_guardar').show();
	$('#modal_sbBal').modal();
	if(modal_sbBal_tipo_transaccion==='up_balance'){
		$("#modal_sbBal_btn_guardar_texto").html('SUBIR BALANCE');
		$("#modal_sbBal_monto_texto").html('<spam style="color: red; font-weight: bold;">(*)</spam> Balance a Sumar S/');
		$('#modal_sbBal_btn_guardar').css('background','#00c770');
		$('#modal_sbBal_btn_guardar').css('border-color','#00c770');
		mb_obtener_opciones("listar_motivos_balance_sube", $("[name='modal_sbBal_motivo_balance_id']"));
		mb_obtener_opciones("listar_juegos_balance", $("[name='modal_sbBal_juego_motivo_balance']"));
		mb_obtener_opciones("listar_supervisores_tlv", $("[name='modal_sbBal_supervisor_motivo_balance']"));
		mb_obtener_opciones("listar_cajeros_tlv", $("[name='modal_sbBal_cajero_motivo_balance']"));
	}
	if(modal_sbBal_tipo_transaccion==='down_balance'){
		$("#modal_sbBal_btn_guardar_texto").html('BAJAR BALANCE');
		$("#modal_sbBal_monto_texto").html('<spam style="color: red; font-weight: bold;">(*)</spam> Balance a Restar S/');
		$('#modal_sbBal_btn_guardar').css('background','#ff0018');
		$('#modal_sbBal_btn_guardar').css('border-color','#ff0018');
		mb_obtener_opciones("listar_motivos_balance_baja", $("[name='modal_sbBal_motivo_balance_id']"));
		mb_obtener_opciones("listar_juegos_balance", $("[name='modal_sbBal_juego_motivo_balance']"));
		mb_obtener_opciones("listar_supervisores_tlv", $("[name='modal_sbBal_supervisor_motivo_balance']"));
		mb_obtener_opciones("listar_cajeros_tlv", $("[name='modal_sbBal_cajero_motivo_balance']"));
	}
...
function modal_sbBal_monto_onchange() {
	var monto = $('#modal_sbBal_monto').val().replace(/\,/g, '');
	if(!(parseFloat(monto)>0)){
		monto = 0;
	}
	var temp_tipo_balance = $('#modal_sbBal_balance_tipo').val();
	if(parseInt(temp_tipo_balance)===4){
		$("#modal_sbBal_balance_tipo_actual").val(gen_balance_no_retirable_disponible);
	}
	if(parseInt(temp_tipo_balance)===5){
		$("#modal_sbBal_balance_tipo_actual").val(gen_balance_retiro_disponible);
	}
	if(parseInt(temp_tipo_balance)===6){
		$("#modal_sbBal_balance_tipo_actual").val(gen_balance_dinero_at);
	}
	if(modal_sbBal_tipo_transaccion==='up_balance'){
		if(parseInt(temp_tipo_balance)===4){
			$("#modal_sbBal_balance_tipo_nuevo").val((parseFloat(gen_balance_no_retirable_disponible)+parseFloat(monto)).toFixed(2));
		}
		if(parseInt(temp_tipo_balance)===5){
			$("#modal_sbBal_balance_tipo_nuevo").val((parseFloat(gen_balance_retiro_disponible)+parseFloat(monto)).toFixed(2));
		}
		if(parseInt(temp_tipo_balance)===6){
			$("#modal_sbBal_balance_tipo_nuevo").val((parseFloat(gen_balance_dinero_at)+parseFloat(monto)).toFixed(2));
	...
	$("#modal_sbBal_balance_nuevo").val(monto);
}


function sb_balance() {

	$('#modal_sbBal_btn_guardar').hide();
	limpiar_bordes_modal_sbBal();

	var balance_tipo = $('#modal_sbBal_balance_tipo').val();

	var motivo_balance = $('#modal_sbBal_motivo_balance_id').val();
	var juego_balance = $('#modal_sbBal_juego_motivo_balance').val();
	var id_transaccion_juego = $('#modal_sbBal_idtrans').val().trim();
	var supervisor_balance = $('#modal_sbBal_supervisor_motivo_balance').val();
	var cajero_balance = $('#modal_sbBal_cajero_motivo_balance').val();


	var monto = $('#modal_sbBal_monto').val().replace(/\,/g, '');
	var balance_nuevo = $('#modal_sbBal_balance_nuevo').val().replace(/\,/g, '');
    var observacion = $('#modal_sbBal_observacion').val();
    var balance_actual_disponible = $('#modal_sbBal_balance_tipo_actual').val();
    var nuevo_balance_disponible = $('#modal_sbBal_balance_tipo_nuevo').val();
    var texto_balance_disponible = $('#modal_sbBal_balance_tipo_nuevo_texto').html()

	if (!(parseInt(balance_tipo) > 0)) {
		$('#modal_sbBal_balance_tipo').css('border', '1px solid red');
		$('#modal_sbBal_balance_tipo').focus();
		$('#modal_sbBal_btn_guardar').show();
		swal({
	...
	if (parseInt(motivo_balance) == 0) {
		...
		swal({
			...
			$("#modal_sbBal_motivo_balance_id").focus();
    		$("#modal_sbBal_motivo_balance_id").select2("open");
		});
	...
	if (parseInt(juego_balance) == 0) {
		...
		swal({
			...
			$("#modal_sbBal_juego_motivo_balance").focus();
    		$("#modal_sbBal_juego_motivo_balance").select2("open");
		});
		return false; 
	}
	...
	if (parseInt(cajero_balance) == 0) {
		...
		swal({
			...
			$("#modal_sbBal_cajero_motivo_balance").focus();
    		$("#modal_sbBal_cajero_motivo_balance").select2("open");
		});
		return false; 
	}
	...
	if (parseInt(supervisor_balance) == 0) {
		...
		swal({
			...
			$("#modal_sbBal_supervisor_motivo_balance").focus();
    		$("#modal_sbBal_supervisor_motivo_balance").select2("open");
		});
		return false; 
	}
	...
	if (!(parseFloat(monto) > 0)) {
		$('#modal_sbBal_monto').css('border', '1px solid red');
		$('#modal_sbBal_monto').focus();
		$('#modal_sbBal_btn_guardar').show();
		swal({
    ...
    if($.trim(observacion) === ''){
        $('#modal_sbBal_observacion').css('border', '1px solid red');
        $('#modal_sbBal_observacion').focus();
        $('#modal_sbBal_btn_guardar').show();
		swal({
	...
	data.append('tipo_transaccion', modal_sbBal_tipo_transaccion);
...
function limpiar_bordes_modal_sbBal() {
    $('#modal_sbBal_monto').css('border', '');
	$('#modal_sbBal_idtrans').css('border', '');
    $('#modal_sbBal_balance_tipo').css('border', '');
    $('#modal_sbBal_observacion').css('border', '');
	
}

function limpiar_campos_modal_sbBal() {
	$("#modal_sbBal_balance_tipo").val('0');
	$("#modal_sbBal_motivo_balance_id").val('0');
	$("#modal_sbBal_juego_motivo_balance").val('0');
	$("#modal_sbBal_idtrans").val('0');
	$("#modal_sbBal_supervisor_motivo_balance").val('0');
	$("#modal_sbBal_cajero_motivo_balance").val('0');
	$("#modal_sbBal_balance_actual").val(gen_balance_total);
	$("#modal_sbBal_monto").val('0');
	$("#modal_sbBal_balance_nuevo").val('0');
    $("#modal_sbBal_observacion").val('');
}


function mb_obtener_opciones(accion, select) {
	$.ajax({
	  url: "/sys/set_televentas.php",
	  type: "POST",
