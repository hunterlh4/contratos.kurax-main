function sec_contrato_detalle_adenda_acuerdo_confidencialidad() {
	...
	sec_con_detalle_aden_acu_confidencialidad_obtener_adenda_detalle_contratos();
	sec_con_detalle_aden_acu_confidencialidad_obtener_opciones("obtener_agente","[name='sec_con_nuevo_agente']");
	sec_con_detalle_aden_acu_confidencialidad_obtener_opciones("obtener_monedas","[name='modal_contr_ade_int_moneda_id']");
	sec_con_detalle_aden_acu_confidencialidad_obtener_opciones("obtener_forma_pago","[name='modal_contr_ade_int_forma_pago']");
	sec_con_detalle_aden_acu_confidencialidad_obtener_opciones("obtener_tipo_comprobante","[name='modal_contr_ade_int_tipo_comprobante']");
	sec_contrato_nuevo_obtener_opciones("obtener_directores", "[name='director_aprobacion_id']");
}

function sec_con_detalle_aden_acu_confidencialidad_obtener_opciones(accion,select){
    loading(true);
	$.ajax({
...
function sec_con_detalle_aden_acu_confidencialidad_obtener_adenda_detalle_contratos() {
	...
	$.ajax({
		url: "/sys/set_contrato_detalle_adenda_acuerdo_confidencialidad.php",
		type: 'POST',
		data: data,
		beforeSend: function () {
		},
		...
		success: function (datos) {
			...
			if (respuesta.status == 200) {
				for (let index = 0; index < respuesta.result.length; index++) {
					sec_con_detalle_aden_acu_confidencialidad_asignar_detalle_a_la_adenda(respuesta.result[index],'');
				}
			}
...
function sec_con_detalle_aden_acu_confidencialidad_solicitud_editar_campo_adenda(nombre_menu_usuario, nombre_tabla, nombre_campo, nombre_campo_usuario, tipo_valor, valor_actual, metodo_select, id_del_registro) {
	...
	if (tipo_valor == 'varchar') {
		...
		if (nombre_campo == 'ubigeo_id') {
			...
			$('#div_adenda_valor_select_option').hide();
			sec_con_detalle_aden_acu_confidencialidad_obtener_opciones(metodo_select,$("[name='adenda_inmueble_id_departamento']"));
			...
			sec_con_detalle_aden_acu_confidencialidad_obtener_opciones(metodo_select,$("[name='adenda_valor_select_option']"));
			setTimeout(function() {
...
function sec_con_detalle_aden_acu_confidencialidad_guardar_detalle_adenda(name_modal_close) {
	...
	$.ajax({
		...
		success: function(resp) { //  alert(datat)
			...
			if (parseInt(respuesta.http_code) == 200) {
				...
				sec_con_detalle_aden_acu_confidencialidad_asignar_detalle_a_la_adenda(new_adenda, '');
...
function sec_con_detalle_aden_acu_confidencialidad_asignar_detalle_a_la_adenda(adenda,modal){

	const index = array_adendas_contrato.map(item => item.id).includes(adenda.id);
	if (index == false) {
		array_adendas_contrato.push(adenda)
	}
	sec_con_detalle_aden_acu_confidencialidad_actualizar_tabla_detalle_adenda();
	
}

function sec_con_detalle_aden_acu_confidencialidad_actualizar_tabla_detalle_adenda(){
...
function sec_con_detalle_aden_acu_confidencialidad_eliminar_detalle_adenda(id_adenda){
...
function sec_con_detalle_aden_acu_confidencialidad_agregar_representante(){
	$('#modalNuevaCuentaBancaria').modal({backdrop: 'static', keyboard: false});
}

function sec_con_detalle_aden_acu_confidencialidad_guardar_nuevo_representante_legal(){
	var contrato_id = $('#id_registro_contrato_id').val();
	var numero_dni = $('#modal_ade_acu_confidencialidad_numero_dni').val();
	var nombre_representante = $('#modal_ade_acu_confidencialidad_representante_legal').val();
	var id_banco = $('#modal_ade_acu_confidencialidad_banco').val();
	var numero_cuenta = $('#modal_ade_acu_confidencialidad_n_cuenta').val();
	var numero_cci = $('#modal_ade_acu_confidencialidad_n_cci').val();
...
function sec_con_detalle_aden_acu_confidencialidad_guardar_adenda() {
