...
class ContratoArrendamientoController
{
    public function registrar()
    {
        ...
        try {
            ...
            if (($model_contrato->get_usuario_id()) > 0) {
                ...
                $data_cotrato['persona_responsable_id'] = NULL;
                $data_cotrato['abogado_id'] = $request['abogado_id'];
                $data_cotrato['cargo_aprobador_id'] = 0;
                ...
                $tipo_contrato_version_actual = $model_contrato->obtener_formato_version_actual($request['tipo_contrato_id']);
                $data_cotrato['idformato'] = $tipo_contrato_version_actual['idformato'];
                ...
                $files_upload = $this->subir_archivos_contrato_arrendamiento($contrato_id, $_FILES, $ficha_contratos['contratos']);
                ...
                // $this->send_email_confirmacion_solicitud_contrato_arrendamiento($contrato_id, false);
                $this->enviar_email_confirmacion_solicitud_contrato_arrendamiento($contrato_id, false);
                // $this->send_email_solicitud_contrato_arrendamiento_detallado(contrato_id: $contrato_id, false, false);

                $connection->commit();
                $result['status'] = 200;
                $result['result'] = $contrato_id;
                $result['message'] = 'El contrato se ha registrado exitosamente' . 'formato:' . $data_cotrato['idformato'];
                $result['errors_uploads'] = $files_upload['status'] == 200 ? $files_upload['errors_uploads'] : '';
    ...
    public function registrar_version_formato()
    {
        $db = Database::getInstance();
        $connection = $db->getConnection();
        ...
        try {
            ...
            if (($model_contrato->get_usuario_id()) > 0) {
                ...
                $connection->commit();
                $result['status'] = 200;
                $result['result'] = $data_cotrato['idformato'];
                $result['message'] = 'El contrato se ha registrado exitosamente' . $data_cotrato['idformato'];
                $result['errors_uploads'] = '';
                return json_encode($result, JSON_UNESCAPED_UNICODE);
            }
        ...
        try {
            ...
            if (($model_contrato->get_usuario_id()) > 0) {
                ...
                for ($l = 0; $l < count($contratos); $l++) {
                    ...
                    $inmueble_destinado = $contrato['inmueble_destinado'];


                    var_dump($inmueble['observaciones_mueble']);
                    var_dump($inmueble_destinado);
                    die();
                    ...
                    $data_inmueble['inmueble_destinado'] = $inmueble_destinado;
                    $data_inmueble['observaciones_mueble'] = $inmueble['observaciones_mueble'];
        ...
        try {
            ...
            if (($model_contrato->get_usuario_id()) > 0) {
                ...
                for ($l = 0; $l < count($contratos); $l++) {
                    ...
                    $insert_cond_eco = $model_condicion_economica->registrar_v2($data_cond_eco);
                    if ($insert_cond_eco['status'] == 404) {
                    ...
                    foreach ($beneficiarios as $beneficiario) {
                        $data_beneficiario['contrato_detalle_id'] = $contrato_detalle_id;
                        $data_beneficiario['contrato_id'] = $contrato_id;
                        $data_beneficiario['tipo_persona_id'] = 2;
                        $data_beneficiario['tipo_docu_identidad_id'] = $beneficiario['tipo_docu_identidad_id'];
                        $data_beneficiario['num_docu'] = $beneficiario['num_docu'];
                        $data_beneficiario['nombre'] = $beneficiario['nombre'];
                        $data_beneficiario['banco_id'] = $beneficiario['banco_id'];
                        $data_beneficiario['tipo_monto_id'] = 3;
                        $data_beneficiario['forma_pago_id'] = 2;
                        $data_beneficiario['num_cuenta_bancaria'] = $beneficiario['num_cuenta_bancaria'];
                        $data_beneficiario['user_updated_id'] = $model_contrato->get_usuario_id();
                        $data_beneficiario['created_at'] = $created_at;

                        $insert_beneficiario = $modal_beneficiario->registrarExtendido($data_beneficiario);
                        if ($insert_beneficiario['status'] == 404) {
    ...
    //         var_dump($result);
    //         die("enviar_email_confirmacion_contrato_arrendamiento");
    //         return json_encode($result, JSON_UNESCAPED_UNICODE);
    //     } catch (\Exception $e) {
    //         $result['status'] = 404;
    //         $result['result'] = $e->getMessage();
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //NOTIFICAR A LOS ABOGADOS CARGO 34 SOBRE LA CREACION DE SOLICITUD DE ARRENDAMIENTO //////////////////////////////
    function enviar_email_confirmacion_solicitud_contrato_arrendamiento($contrato_id, $reenviar)
    {
        $host = $_SERVER["HTTP_HOST"];
        $model_contrato = new ContratoArrendamiento();
        $data_contrato2 = $model_contrato->obtener_solicitud_contrato_local_detallado($contrato_id);
        ...
        <body>
            <div class="title">Nueva Solicitud de Contrato</div>';

        foreach ($data_contrato2['result']['datos_generales'] as $sel) {
            ...
            <table>
                ...
                <tr>
                    ...
                    <td>' . $sel['empresa_suscribe'] . '</td>
                </tr>
                <tr>
                    <td><strong>Código de Contrato</strong></td>
                    <td>' . $sigla_correlativo . $codigo_correlativo . '</td>
                </tr>
                <tr>
        ...
        $body .= '
            ...
            <table>
                ...
                <tr>
                    ...
                    <td>' . $data_contrato2['result']['data_propietarios'][0]['nombre'] . '</td>
                </tr>
                <tr>
                    <td><strong>Tipo de Persona</strong></td>
                    <td>' . $data_contrato2['result']['data_propietarios'][0]['tipo_persona'] . '</td>
                </tr>
                <tr>
                    <td><strong>Documento de Identidad</strong></td>
                    <td>' . $data_contrato2['result']['data_propietarios'][0]['tipo_docu_identidad'] . ': ' . $data_contrato2['result']['data_propietarios'][0]['num_docu'] . '</td>
                </tr>
                <tr>
                    <td><strong>Dirección</strong></td>
                    <td>' . $data_contrato2['result']['data_propietarios'][0]['direccion'] . '</td>
                </tr>
                <tr>
                    <td><strong>Contacto</strong></td>
                    <td>' . $data_contrato2['result']['data_propietarios'][0]['contacto_nombre'] . ' - Teléfono: ' . $data_contrato2['result']['data_propietarios'][0]['contacto_telefono'] . '</td>
                </tr>
            </table>';
        ...
        $body .= '
            ...
            <table>
                <tr>
                    <th colspan="2">Datos del Inmueble</th>
                </tr>
                <tr>
                    <td><strong>Ubicación</strong></td>
                    <td>' . $data_contrato2['result']['contratos'][0]['data_inmuebles'][0]['departamento'] . ', ' . $data_contrato2['result']['contratos'][0]['data_inmuebles'][0]['provincia'] . ', ' . $data_contrato2['result']['contratos'][0]['data_inmuebles'][0]['distrito'] . '</td>
                </tr>
                <tr>
                    <td><strong>Dirección</strong></td>
                    <td>' . $data_contrato2['result']['contratos'][0]['data_inmuebles'][0]['ubicacion'] . '</td>
                </tr>
                <tr>
                    <td><strong>Partida Registral</strong></td>
                    <td>' . $data_contrato2['result']['contratos'][0]['data_inmuebles'][0]['num_partida_registral'] . '</td>
                </tr>
            </table>';
        ...
        $body .= '
            ...
            <table>
                ...
                <tr>
                    <td><strong>Nombre</strong></td>
                    <td>' . $data_contrato2['result']['contratos'][0]['data_beneficiarios'][0]['nombre'] . '</td>
                </tr>
                <tr>
                    <td><strong>Tipo de Persona</strong></td>
                    <td>' . $data_contrato2['result']['contratos'][0]['data_beneficiarios'][0]['tipo_persona'] . '</td>
                </tr>
                <tr>
                    <td><strong>Documento de Identidad</strong></td>
                    <td>' . $data_contrato2['result']['contratos'][0]['data_beneficiarios'][0]['tipo_docu_identidad'] . ': ' . $data_contrato2['result']['contratos'][0]['data_beneficiarios'][0]['num_docu'] . '</td>
                </tr>
                <tr>
                    <td><strong>Forma de Pago</strong></td>
                    <td>' . $data_contrato2['result']['contratos'][0]['data_beneficiarios'][0]['forma_pago'] . '</td>
                </tr>
                <tr>
                    <td><strong>Banco</strong></td>
                    <td>' . $data_contrato2['result']['contratos'][0]['data_beneficiarios'][0]['banco'] . '</td>
                </tr>
                <tr>
                    <td><strong>Número de Cuenta</strong></td>
                    <td>' . $data_contrato2['result']['contratos'][0]['data_beneficiarios'][0]['num_cuenta_bancaria'] . '</td>
                </tr>
            </table>';
        ...
        $body .= '
            ...
            <div style="width: 700px; text-align: center; font-family: arial;">
                <a href="http://contratos.kurax.test:90/?sec_id=contrato&sub_sec_id=detalle_solicitudV2&id=' . $contrato_id . '" style="background-color: #4CAF50; border: none; color: white; padding: 5px 20px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px;">
                    <b>Ver Solicitud</b>
                </a>
            </div>
        </body>
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //NOTIFICAR AL ABOGADO SELECCIONADO EN LA APROBACION DE LA SOLICITUD DE ARRENDAMIENTO ///////////////////////////////////
    function enviar_email_solicitud_arrendamiento($contrato_id, $reenviar = false)
    {
        ...
        // die("correo body CORREOX1");
        $body = "";

        $body .= '<html>';
        $body = ' <!DOCTYPE html> <html lang="es"> <head> <meta charset="UTF-8"> <meta name="viewport" content="width=device-width, initial-scale=1.0"> <title>Detalle de Solicitud de Contrato</title> <style> body { font-family: Arial, sans-serif; width:700px; } table { width: 100%; border-collapse: collapse; margin-bottom: 20px; width:700px; } th, td { border: 1px solid #ddd; padding: 8px; text-align: left; } th { background-color: #395168; text-align: center; color: white;} .title { font-size: 18px; font-weight: bold; margin-bottom: 10px; text-align: center; width:700px;} </style> </head> <body> <div class="title">Aprobación del Contrato</div>';

        foreach ($data_contrato2['result']['datos_generales'] as $sel) {
            $sigla_correlativo = $sel['sigla_correlativo'];
            $codigo_correlativo = $sel['codigo_correlativo'];

            // Verificar si el contrato ha sido aprobado o no
            ...
            if (!is_null($fecha_aprobacion)) {
                if ($estado_aprobacion == 1) {
                    $titulo_estado = "Contrato Aprobado";
                    $mensaje_estado = "El contrato ha sido aprobado el " . $fecha_aprobacion;
                } else {
                    $titulo_estado = "Contrato Rechazado";
                    $mensaje_estado = "El contrato ha sido rechazado el " . $fecha_aprobacion;
                }
            } else {
                $titulo_estado = "Contrato Pendiente de Aprobación";
                $mensaje_estado = "El contrato está pendiente de aprobación.";
            }

            $body .= ' <!-- Datos Generales del Contrato --> <table> <tr> <th colspan="2">Datos Generales del Contrato</th> </tr> <tr> <td><strong>Empresa Suscriptora</strong></td> <td>' . $sel['empresa_suscribe'] . '</td> </tr> <tr> <td><strong>Código de Contrato</strong></td> <td>' . $sigla_correlativo . $codigo_correlativo . '</td> </tr> <tr> <td><strong>Fecha de Creación</strong></td> <td>' . $sel['created_at'] . '</td> </tr> <tr> <td><strong>Responsable</strong></td> <td>' . $sel['user_created'] . '</td> </tr>  <tr> <td><strong>Estado de Aprobación</strong></td> <td>' . $titulo_estado . '</td> </tr> <tr> <td><strong>Mensaje de Estado</strong></td> <td>' . $mensaje_estado . '</td> </tr> </table>';
        }

        $body .= ' <!-- Datos del Propietario --> <table> <tr> <th colspan="2">Datos del Propietario</th> </tr> <tr> <td><strong>Nombre</strong></td> <td>' . $data_contrato2['result']['data_propietarios'][0]['nombre'] . '</td> </tr> <tr> <td><strong>Tipo de Persona</strong></td> <td>' . $data_contrato2['result']['data_propietarios'][0]['tipo_persona'] . '</td> </tr> <tr> <td><strong>Documento de Identidad</strong></td> <td>' . $data_contrato2['result']['data_propietarios'][0]['tipo_docu_identidad'] . ': ' . $data_contrato2['result']['data_propietarios'][0]['num_docu'] . '</td> </tr> <tr> <td><strong>Dirección</strong></td> <td>' . $data_contrato2['result']['data_propietarios'][0]['direccion'] . '</td> </tr> <tr> <td><strong>Contacto</strong></td> <td>' . $data_contrato2['result']['data_propietarios'][0]['contacto_nombre'] . ' - Teléfono: ' . $data_contrato2['result']['data_propietarios'][0]['contacto_telefono'] . '</td> </tr> </table>';

        $body .= ' <!-- Datos del Inmueble --> <table> <tr> <th colspan="2">Datos del Inmueble</th> </tr> <tr> <td><strong>Ubicación</strong></td> <td>' . $data_contrato2['result']['contratos'][0]['data_inmuebles'][0]['departamento'] . ', ' . $data_contrato2['result']['contratos'][0]['data_inmuebles'][0]['provincia'] . ', ' . $data_contrato2['result']['contratos'][0]['data_inmuebles'][0]['distrito'] . '</td> </tr> <tr> <td><strong>Dirección</strong></td> <td>' . $data_contrato2['result']['contratos'][0]['data_inmuebles'][0]['ubicacion'] . '</td> </tr> <tr> <td><strong>Partida Registral</strong></td> <td>' . $data_contrato2['result']['contratos'][0]['data_inmuebles'][0]['num_partida_registral'] . '</td> </tr> </table>';

        $body .= ' <!-- Enlace para ver la solicitud --> <div style="width: 700px; text-align: center; font-family: arial;"> <a href="http://contratos.kurax.test:90/?sec_id=contrato&sub_sec_id=detalle_solicitudV2&id=' . $contrato_id . '" style="background-color: #4CAF50; border: none; color: white; padding: 5px 20px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px;"> <b>Ver Solicitud</b> </a> </div> </body> </html>';
        ...
        //     var_dump($body);
        // die("correo body CORREOX cc1");

        if ($reenviar) {
            $titulo = "Gestion - Sistema Contratos - Reenviar Confirmación de Contrato Arrendamiento: Código - " . $sigla_correlativo . $codigo_correlativo;
        } else {
            $titulo = "Gestion - Sistema Contratos - Confirmación de Contrato Arrendamiento: Código - " . $sigla_correlativo . $codigo_correlativo;
        }
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //NOTIFICAR AL ABOGADO SELECCIONADO EN LA FIRMA DE UN CONTRATO DE ARRENDAMIENTO ///////////////////////////////////

    function enviar_email_firma_contrato_arrendamiento($contrato_id, $reenviar = false)
    {
        $contrato_id = $_POST['contrato_id'];
        $host = $_SERVER["HTTP_HOST"];
        $model_contrato = new ContratoArrendamiento();
        ...
        $sigla_correlativo = $data_contrato2['result']['datos_generales'][0]['sigla_correlativo'];
        $codigo_correlativo = $data_contrato2['result']['datos_generales'][0]['codigo_correlativo'];
        $empresa_suscribe = $data_contrato2['result']['datos_generales'][0]['empresa_suscribe'];
        ...
        <head>
            <meta charset="UTF-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
            <title>Confirmación de Firma de Contrato de Arrendamiento</title>
            <style>
                body { font-family: Arial, sans-serif; width:700px; }
                table { width: 100%; border-collapse: collapse; margin-bottom: 20px; width:700px; }
                th, td { border: 1px solid #ddd; padding: 8px; text-align: left; }
        ...
        <body>
            <div class="title">Confirmación de Contrato de Arrendamiento Firmado</div>
            ...
            <table>
                ...
                <tr>
                    <td><strong>Ubicación</strong></td>
                    <td>' . $data_contrato2['result']['contratos'][0]['data_inmuebles'][0]['departamento'] . ', '
            . $data_contrato2['result']['contratos'][0]['data_inmuebles'][0]['provincia'] . ', '
            . $data_contrato2['result']['contratos'][0]['data_inmuebles'][0]['distrito'] . '</td>
                </tr>
                <tr>
                    <td><strong>Dirección</strong></td>
                    <td>' . $data_contrato2['result']['contratos'][0]['data_inmuebles'][0]['ubicacion'] . '</td>
            ...
            <div style="width: 700px; text-align: center; font-family: arial;">
                <a href="http://contratos.kurax.test:90/?sec_id=contrato&sub_sec_id=detalle_solicitudV2&id=' . $contrato_id . '" 
                   style="background-color: #4CAF50; border: none; color: white; padding: 5px 20px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px;">
        ...
        // var_dump($body);
        // die("estructura del correo firmado");
        ...
        $titulo = "Gestion - Sistema Contratos - Contrato de Arrendamiento Firmado: Código - " . $sigla_correlativo . $codigo_correlativo;
    ...
    //     // Obtener los correos a los que se enviará la notificación
    //     $correos = new Correos(env('SEND_EMAIL'), env('EMAIL_TEST'));
    //     $lista_correos = $correos->enviar_correo_notificacion_abogado($contrato_id);
    //     $cc = $lista_correos['cc'];
    //     $bcc = $lista_correos['bcc'];
    ...
    //         <meta charset="UTF-8">
    //         <meta name="viewport" content="width=device-width, initial-scale=1.0">
    //         <title>Notificación de Firma de Contrato de Arrendamiento</title>
    //         <style>
    //             body { font-family: Arial, sans-serif; width: 700px; }
    ...
    //     </head>
    //     <body>
    //         <div class="title">Notificación de Firma de Contrato de Arrendamiento</div>';
    ...
    //     $body .= '
    //     <div style="width: 700px; text-align: center; font-family: arial;">
    //         <a href="http://' . $host . '/?sec_id=contrato&sub_sec_id=detalle_solicitudV2&id=' . $contrato_id . '" style="background-color: #4CAF50; border: none; color: white; padding: 5px 20px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px;">
    //             <b>Ver Contrato</b>
    //         </a>
    //     </div>
    //     </body>
    ...
    //     if ($reenviar) {
    //         $titulo = "Gestion - Sistema Contratos - Reenviar Notificación de Firma de Contrato Arrendamiento: Código - " . $sigla_correlativo . $codigo_correlativo;
    //     } else {
    //         $titulo = "Gestion - Sistema Contratos - Notificación de Firma de Contrato Arrendamiento: Código - " . $sigla_correlativo . $codigo_correlativo;
    //     }
    ...
    function send_email_confirmacion_solicitud_contrato_arrendamiento($contrato_id, $reenviar)
    {
        ...
        if (env('SEND_EMAIL') == 'PRODUCTION') { // Imprimir lista de correos que se enviarian en producción pero solo se visualizara en Desarrollo 
    ...
    function send_email_solicitud_contrato_arrendamiento_detallado($contrato_id, $enviar_respuesta, $reenvio): never
    {
        $host = $_SERVER["HTTP_HOST"];
        $model_contrato = new ContratoArrendamiento();
        $data_contrato = $model_contrato->obtener_solicitud_contrato_local_detallado($contrato_id);
        var_dump($data_contrato);
        die("enviar_email_confirmacion_contrato_arrendamiento");
