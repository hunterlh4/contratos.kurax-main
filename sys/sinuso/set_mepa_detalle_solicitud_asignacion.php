...
if (isset($_POST["accion"]) && $_POST["accion"]==="guardar_detalle_solicitud_asignacion_atencion") 
{
	...
	$mepa_asignacion_caja_chica_id = $_POST["mepa_asignacion_caja_chica_id"];
	...
	if($txt_situacion == 6)
	{
		$query_update = "
					...
					WHERE id = '".$mepa_asignacion_caja_chica_id."' ";
	}
	else
	{
		$query_update = "
	...
	if ($error == '') 
	{
		...
		$result["mepa_asignacion_caja_chica_id"] = $mepa_asignacion_caja_chica_id;
		$result["status"] = "Datos obtenidos de gestión.";
		$result["error"] = $error;
		send_email_atencion_solicitud_asignacion_caja_chica($mepa_asignacion_caja_chica_id);
	} 
	else 
	{
...
function send_email_atencion_solicitud_asignacion_caja_chica($mepa_asignacion_caja_chica_id)
{
	include("db_connect.php");
	include("sys_login.php");
	...
	while($sel = $sel_query->fetch_assoc())
	{
		...
		if($situacion_etapa_id == 6)
		{
			...
			$body .= '<tr>';
				...
				$body .= '<td>La actual solicitud esta aprobado por su área correspondiente, Tesoreria hara el deposito correspondiente a su solicitud y lo notificara</td>';
			$body .= '</tr>';
		}
...
if (isset($_POST["accion"]) && $_POST["accion"]==="mepa_detalle_solicitud_asignacion_editar_cuenta_bancaria") 
{
	$usuario_id = $login?$login['id']:null;
	$created_at = date("Y-m-d H:i:s");
	$error = '';
	...
	if((int) $usuario_id > 0)
	{
		$form_txt_id_asignacion = $_POST["form_txt_id_asignacion"];
		$form_txt_editar_banco = $_POST["form_txt_editar_banco"];
		$form_txt_editar_numero_cuenta = $_POST["form_txt_editar_numero_cuenta"];

		$query_update = "
						UPDATE mepa_asignacion_cuenta_bancaria 
							SET banco_id = '".$form_txt_editar_banco."', 
							num_cuenta = '".$form_txt_editar_numero_cuenta."'
						WHERE asignacion_id = '".$form_txt_id_asignacion."' AND status = 1
						";
		...
		if ($error == '') 
		{
			...
			$result["status"] = "Datos obtenidos de gestión.";
			$result["error"] = $error;
			send_email_edicion_cuenta_bancaria($form_txt_id_asignacion);
		}
		else 
		{
			$result["http_code"] = 400;
...
function send_email_edicion_cuenta_bancaria($mepa_asignacion_caja_chica_id)
{
	include("db_connect.php");
	include("sys_login.php");
	...
	$sel_query = $mysqli->query("
		...
		FROM mepa_asignacion_caja_chica a
			...
			INNER JOIN tbl_bancos tb
			ON ac.banco_id = tb.id
		WHERE a.id = '".$mepa_asignacion_caja_chica_id."' AND ac.status = 1
	");
	...
	while($sel = $sel_query->fetch_assoc())
	{
		...
		$body .= '<div>';
			...
			$body .= '<tr>';
				$body .= '<th colspan="2" style="color: #fff; background-color: #395168; vertical-align: middle; font-size: 16px">';
					$body .= '<b>Cuenta Bancaria Editada</b>';
				$body .= '</th>';
			$body .= '</tr>';
			...
			$body .= '<tr>';
				$body .= '<td style="background-color: #ffffdd; width: 100px;"><b>Responsable:</b></td>';
	...
	$titulo_email = "Gestion - Sistema Mesa de Partes - Cuenta Bancaria Editada";
...
if (isset($_POST["accion"]) && $_POST["accion"]==="mepa_detalle_solicitud_asignacion_nuevo_cuenta_bancaria") 
{
	$usuario_id = $login?$login['id']:null;
	$created_at = date("Y-m-d H:i:s");
	$error = '';
	...
	{
		$form_txt_id_asignacion = $_POST["form_txt_id_asignacion"];
		$form_txt_banco = $_POST["form_txt_banco"];
		$form_txt_numero_cuenta = $_POST["form_txt_numero_cuenta"];
		...
		$query_insert = "
						...
						VALUES
						(
							'" . $form_txt_id_asignacion . "',
							'" . $form_txt_banco . "',
							'" . $form_txt_numero_cuenta . "',
							0,
							" . $usuario_id . ",
							'" . $created_at . "',
							" . $usuario_id . ",
		...
		$asignacion_cuenta_bancaria = mysqli_insert_id($mysqli);
		...
		if ($error == '') 
		{
			...
			$result["status"] = "Datos obtenidos de gestión.";
			$result["error"] = $error;
			send_email_nuevo_cuenta_bancaria($asignacion_cuenta_bancaria);
		}
		else 
		{
			$result["http_code"] = 400;
...
function send_email_nuevo_cuenta_bancaria($asignacion_cuenta_bancaria_id)
{
	include("db_connect.php");
	include("sys_login.php");
	...
	$sel_query = $mysqli->query("
		...
		FROM mepa_asignacion_cuenta_bancaria ac
		    ...
		    INNER JOIN tbl_bancos tb
			ON ac.banco_id = tb.id
		WHERE ac.id = '".$asignacion_cuenta_bancaria_id."'
	");
	...
	while($sel = $sel_query->fetch_assoc())
	{
		...
		$body .= '<div>';
			...
			$body .= '<tr>';
				$body .= '<th colspan="2" style="color: #fff; background-color: #395168; vertical-align: middle; font-size: 16px">';
					$body .= '<b>Solicitud Cuenta Bancaria</b>';
				$body .= '</th>';
			$body .= '</tr>';
	...
	$titulo_email = "Gestion - Sistema Mesa de Partes - Nueva Cuenta Bancaria Agregada";
...
if (isset($_POST["accion"]) && $_POST["accion"]==="sec_mepa_detalle_solicitud_asignacion_guardar_zona") 
{
	...
	if((int)$login_usuario_id > 0)
	{
		...
		// INICIO: ACTUALIZAR LA ZONA EN LA TABLA PRINCIPAL mepa_asignacion_caja_chica
		$query_update_asignacion = 
		"
			UPDATE mepa_asignacion_caja_chica 
				SET zona_asignacion_id = '".$txt_zona."', 
		...
		// FIN: ACTUALIZAR LA ZONA EN LA TABLA PRINCIPAL mepa_asignacion_caja_chica

		// INICIO: ACTUALIZAR EL ESTADO COMO DESACTIVADO EN LA TABLA HISTORIAL mepa_asignacion_historial_zona

		$query_update_asignacion_historial_zona = 
		"
			UPDATE mepa_asignacion_historial_zona 
				SET status = 0, 
					user_updated_id = '".$login_usuario_id."',
					updated_at = '".$created_at."'
			WHERE asignacion_id = '".$mepa_asignacion_caja_chica_id."' 
		";

		$mysqli->query($query_update_asignacion_historial_zona);
		...
		// FIN: ACTUALIZAR EL ESTADO COMO DESACTIVADO EN LA TABLA HISTORIAL mepa_asignacion_historial_zona

		// INICIO: INSERTAR LA ZONA VIGENTE EN LA TABLA HISTORIAL mepa_asignacion_historial_zona

		$query_insert_asignacion_historial_zona = 
		"
			INSERT INTO mepa_asignacion_historial_zona
			(
				asignacion_id,
				zona_asignacion_id,
				status,
		...
		$mysqli->query($query_insert_asignacion_historial_zona);
		...
		// FIN: INSERTAR LA ZONA VIGENTE EN LA TABLA HISTORIAL mepa_asignacion_historial_zona

		if ($error == '') 
		{
			$result["http_code"] = 200;
			$result["status"] = "Registro exitoso.";
			$result["texto"] = "Se registro exitosamente.";
		...
		{
			$result["http_code"] = 400;
			$result["status"] = "Error al guardar.";
			$result["texto"] = $error;
	...
	{
		$result["http_code"] = 400;
        $result["texto"] ="Sesión perdida. Por favor vuelva a iniciar sesión.";
...
if (isset($_POST["accion"]) && $_POST["accion"]==="mepa_detalle_solicitud_asignacion_editar_empresa") 
{
	$usuario_id = $login?$login['id']:null;
	$created_at = date("Y-m-d H:i:s");
	$error = '';
	...
	{
		$form_txt_id_asignacion = $_POST["form_txt_id_asignacion"];
		$form_txt_editar_empresa = $_POST["form_txt_editar_empresa"];

		$query_update = 
		"
			UPDATE mepa_asignacion_caja_chica 
				SET empresa_id = '".$form_txt_editar_empresa."'
			WHERE id = '".$form_txt_id_asignacion."'
		";
