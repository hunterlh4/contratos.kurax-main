...
function send_email_solicitud_adenda_contrato_proveedor($adenda_id, $requiere_aprobacion_id)
{
	include("db_connect.php");
	include("sys_login.php");
	...
	SELECT 
		...
		tp.correo AS email_del_solicitante,
		c.gerente_area_id,
		...
		puap.correo AS email_del_aprobante_adenda,
		pud.correo AS email_del_director,
		a.aprobado_estado_id,
		a.aprobado_el,
		a.codigo AS num_adenda,
		c.razon_social ,c.nombre_comercial ,c.ruc,
		per.correo AS email_creacion_contrato,
		pcuap.correo AS email_del_aprobante
	...
	FROM 
		...
		LEFT JOIN tbl_usuarios cuap ON c.aprobado_por = cuap.id
		LEFT JOIN tbl_personal_apt pcuap ON cuap.personal_id = pcuap.id
	...
	while($sel = $sel_query->fetch_assoc())
	{
		...
		$email_del_solicitante = $sel['email_del_solicitante'];
		$email_del_gerente_area = $sel['email_del_gerente_area'];
		$email_del_gerente_area_2 = $sel['gerente_area_email'];
		...
		$email_creacion_contrato = $sel['email_creacion_contrato'];
		$email_del_aprobante = $sel['email_del_aprobante'];
		$email_del_aprobante_adenda = $sel['email_del_aprobante_adenda'];
		$aprobado_el = $sel['aprobado_el'];
		$num_adenda = $sel['num_adenda'];
		...
		$body .= '<td style="background-color: #ffffdd"><b>RUC:</b></td>';
			$body .= '<td>'.$sel["ruc"].'</td>';
		$body .= '</tr>';
		$body .= '<tr>';
			$body .= '<td style="background-color: #ffffdd"><b>Razón Social:</b></td>';
			$body .= '<td>'.$sel["razon_social"].'</td>';
		$body .= '</tr>';
		$body .= '<tr>';
			$body .= '<td style="background-color: #ffffdd"><b>Nombre Comercial:</b></td>';
		...
		if ($requiere_aprobacion_id != 1) {
			if($aprobado_estado_id === 1) {
				$body .= '<tr>';
					...
					$body .= '<td>' . $adenda_aprobada_por . '</td>';
				$body .= '</tr>';

				$body .= '<tr>';
					$body .= '<td style="background-color: #ffffdd"><b>Aprobado el:</b></td>';
					$body .= '<td>' . $aprobado_el . '</td>';
				$body .= '</tr>';
			} else if($aprobado_estado_id === 0) {
				$body .= '<tr>';
				...
				$body .= '<tr>';
					$body .= '<td style="background-color: #ffffdd"><b>Rechazado el:</b></td>';
					$body .= '<td>' . $aprobado_el . '</td>';
				$body .= '</tr>';
			}
		}
		...
		$body .= '<div style="width: 700px; text-align: center; font-family: arial;">';
		    $body .= '<a href="'.$host.'/?sec_id=contrato&sub_sec_id=detalleSolicitudProveedor&id='.$contrato_id.'&adenda_id=' . $adenda_id . '" target="_blank" style="background-color: #4CAF50; border: none; color: white; padding: 5px 20px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px;">';
	...
	if ($requiere_aprobacion_id == 1) {
		array_push($correos_adicionales, $email_del_director);
		array_push($correos_adicionales, $email_del_solicitante);
		$lista_correos = $correos->send_email_solicitud_aprobacion_adenda($correos_adicionales);
	} else {
		...
		if (!empty($email_del_solicitante)) {
			array_push($correos_adicionales, $email_del_solicitante);
		}
		if (!empty($email_del_gerente_area)) {
			array_push($correos_adicionales, $email_del_gerente_area);
		}
		if (!empty($email_del_gerente_area_2)) {
			array_push($correos_adicionales, $email_del_gerente_area_2);
		}
		if (!empty($email_creacion_contrato)) {
			array_push($correos_adicionales, $email_creacion_contrato);
		}
		if (!empty($email_del_aprobante)) {
			array_push($correos_adicionales, $email_del_aprobante);
		}
		if (!empty($email_del_aprobante_adenda)) {
			array_push($correos_adicionales, $email_del_aprobante_adenda);
		}
	...
	$titulo_email = "Gestion - Sistema Contratos - ";

	if ($requiere_aprobacion_id == 1) {
		$titulo_email .= "Aprobación del Documento - Confirmación de";
	} elseif ($requiere_aprobacion_id == 2) {
		if($aprobado_estado_id == 1) {
			$titulo_email .= "Inicio de Proceso Legal - Nueva";
		} elseif($aprobado_estado_id == 0) {
			$rechazada = " Aprobación del Documento - Rechazada - ";
		}
	}

	$titulo_email .= " Solicitud de Adenda de Proveedor N° " . $num_adenda . $rechazada . ": Código - ";
...
if (isset($_POST["accion"]) && $_POST["accion"]==="obtener_adendas_detalle") {
	...
	if ($row_count > 0) {
		...
		while ($row = $list_query->fetch_assoc()) {
			...
			$html .= '<td>';
			$html .= '<a class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="top" title="Eliminar" onclick="sec_con_nuevo_aden_prov_eliminar_detalle_adenda(' . $row["id"] . ')"><i class="fa fa-trash"></i></a>';
			$html .= '</td>';
			$html .= '</tr>';
	...
	if ($row_count_otros > 0) {
		while ($row = $list_query_otros->fetch_assoc()) {
			if ($row["nombre_menu_usuario"] == 'Propietario') {
				...
				$html .= '<td rowspan="9">';
				$html .= '<a class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="top" title="Eliminar" onclick="sec_con_nuevo_aden_prov_eliminar_detalle_adenda(' . $row["id"] . ')"><i class="fa fa-trash"></i></a></td>';
				$html .= '</tr>';
...
if (isset($_POST["accion"]) && $_POST["accion"]==="guardar_adenda") {
	...
	if ($usuario_id == null) {
		...
		$result["message"] = 'Su sesion ha caducado, Ingrese de nuevo al sistema en otra pestaña. Luego vuelva a registrar la solicitud';
		$result["result"] = "error";
		$result["error"] = $error;
		echo json_encode($result);
		exit();
	...
	if($mysqli->error){
		$result["http_code"] = 400;
		$result["message"] = $mysqli->error . '. El la query: ' . $query_insert;
	...
	$query_contrato = "SELECT cc.area_responsable_id, cc.tipo_contrato_id, tpa.area_id 
				...
				WHERE ca.id = ".$adenda_id;
	$sel_contrato = $mysqli->query($query_contrato);
	$data_adenda = $sel_contrato->fetch_assoc();
	...
	if (!Empty($data_adenda['area_responsable_id'])) {
		$seg_proceso = new SeguimientoProceso();
		$data_proceso['tipo_documento_id'] = $ADENDA; //Adenda
		$data_proceso['proceso_id'] = $adenda_id;
		$data_proceso['proceso_detalle_id'] = 0;
		$data_proceso['area_id'] = $aprobacion_obligatoria_id == 1 ? $data_adenda['area_responsable_id'] : $AREA_LEGAL_ID;
		$data_proceso['etapa_id'] = $aprobacion_obligatoria_id == 1 ? $APROBACION_DEL_DOCUMENTO : $INICIO_DE_PROCESO_LEGAL;
	...
	send_email_solicitud_adenda_contrato_proveedor($adenda_id, $aprobacion_obligatoria_id);
...
if (isset($_POST["accion"]) && $_POST["accion"]==="send_email_solicitud_adenda_proveedor") 
{
	$adenda_id = $_POST["adenda_id"];
	send_email_solicitud_adenda_contrato_proveedor($adenda_id, 2);
}
