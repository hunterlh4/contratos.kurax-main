...
function send_email_solicitud_adenda_contrato_locacion($adenda_id,$reenvio){
	include("db_connect.php");
	include("sys_login.php");
	...
	$lista_correos = $correos->send_email_solicitud_adenda_contrato_locacion($correo_adicionales);

	$cc = $lista_correos['cc'];

	$titulo_email = "Gestion - Sistema Contratos - Solicitud Modificada de Adenda de locacion ".$nombre_tienda.": Código - ";

	if($reenvio){
		$titulo_email = "Gestion - Sistema Contratos - Reenvío de Solicitud de Adenda de locacion ".$nombre_tienda.": Código - ";
	}
	...
	if(isset($_POST["id_del_registro"]) && !empty($_POST["id_del_registro"])) {
		...
		if($_POST["nombre_tabla"] == "cont_contrato_detalle"){
			...
			if ($_POST["nombre_tabla"] == "cont_condicion_economica"){ // llaves primarias de la tabla es diferente al campo ID
				...
				$query_detalle_id = "SELECT
										cc.id as contrato_detalle_id 
									FROM
										cont_contraprestacion as con 
										LEFT JOIN cont_contrato_detalle as cc ON cc.contrato_id = con.contrato_id
									WHERE
										con.id = " . $id_del_registro;
			}  
			else if ($_POST["nombre_tabla"] == "cont_locacion") {
				$query_detalle_id = "SELECT
										loc.idcontratodetalle as contrato_detalle_id 
									FROM
										cont_locacion AS loc 
									WHERE
										loc.idlocacion =" . $id_del_registro;
			}   
...
if (isset($_POST["accion"]) && $_POST["accion"]==="obtener_adendas_detalle") {
	...
	if ($row_count > 0) {
		...
		while ($row = $list_query->fetch_assoc()) {
			...
			$html .= '<td>';
			$html .= '<a class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="top" title="Eliminar" onclick="sec_con_detalle_aden_locacion_eliminar_detalle_adenda(' . $row["id"] . ')"><i class="fa fa-trash"></i></a>';
			$html .= '</td>';
			$html .= '</tr>';
	...
	if ($row_count_otros > 0) {
		while ($row = $list_query_otros->fetch_assoc()) {
			...
			if ($row["nombre_menu_usuario"] == 'Propietario') {
				...
				$html .= '<td rowspan="9">';
				$html .= '<a class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="top" title="Eliminar" onclick="sec_con_detalle_aden_locacion_eliminar_detalle_adenda(' . $row["id"] . ')"><i class="fa fa-trash"></i></a></td>';
				$html .= '</tr>';
...
if (isset($_POST["accion"]) && $_POST["accion"]==="guardar_adenda") {
	...
	// send_email_solicitud_adenda_contrato_locacion($adenda_id,false);
	

	$result["status"] = 200;
	$result["message"] = "Se ha modificado exitosamente la adenda de contrato de locacion.";
	$result["result"] = "ok";
	$result["error"] = $error;
	echo json_encode($result);
	exit();
...
if (isset($_POST["accion"]) && $_POST["accion"]==="reenviar_email_solicitud_adenda_locacion") {
	try {
		send_email_solicitud_adenda_contrato_locacion($_POST['adenda_id'], true);
		$result["status"] = 200;
		$result["message"] = "Se ha reenviado correctamente la solicitud de adenda de locacion.";
		$result["result"] = '';
		echo json_encode($result);
		exit();
	}catch (Exception $e){
