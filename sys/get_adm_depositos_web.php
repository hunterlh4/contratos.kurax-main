...
include_once("globalFunctions/generalInfo/functions_saldos_web.php");
...
if (isset($_POST["get_locales_config_saldos_web"])) {
    ...
    $query = "  UPDATE tbl_saldo_web_limites
...
} else if (isset($_POST["get_limites_config_saldos_web"])) {
    $get_data = $_POST["get_limites_config_saldos_web"];
    $tipo_limite = $get_data['tipo_limite'];
    ...
    $query = "  SELECT 
                ...
                FROM tbl_saldo_web_limites l
                    INNER JOIN tbl_limites_tipos t on l.tipo_limite = t.id
                    $inner_local
                where
                    l.estado in (" . $estados . ")
                $where_item
    ...
    while ($lim = $result->fetch_assoc()) {
        ...
        if($lim['estado'] == 1){
            $btn_edit = '<button class="btn btn-xs btn-success btn-edit-limite" data-id="' . $lim['id'] . '" data-item-id="' . $lim['item_id'] . '" data-nombre="' . $lim['nombre'] . '" data-tipo-limite="' . $tipo_limite . '" title ="Editar límite"><span class="glyphicon glyphicon-edit"></span></button>' ;
        } else { $btn_edit = ''; }
        $btn_state = '<button class="btn btn-xs text-' . $color . ' btn-activar-limite" data-id="' . $lim['id'] . '" data-estado="' . $lim['estado'] . '" title="' . $title_btn_change_state . '"><i class="fa fa-toggle-' . $state . '"></i></button>';
        $btn_hist = '<button class="btn btn-xs btn-warning btn-get-historico-limite " data-id="' . $lim['id'] . '" data-nombre="' . $lim['nombre'] . '" title="Historial"><span class="glyphicon glyphicon-time"></span></button>';
    ...
    echo json_encode($resultado);
    exit();
} else if (isset($_POST["guardar_limite_saldos_web"])) {

    $get_data = $_POST["guardar_limite_saldos_web"];
    ...
    if( (is_numeric($limite) && ($limite) >= 0) && $limite != ""){
        ...
        $query = "  UPDATE tbl_saldo_web_limites l
                    INNER JOIN tbl_limites_tipos t on l.tipo_limite = t.id
                    set 
                        l.limite = $limite,
                        l.updated_at = now(),
        ...
        if ($updates == 0) {
            ...
            $query = "  INSERT INTO tbl_saldo_web_limites(tipo_limite, item_id, nombre, limite, estado, is_modified, created_at, user_created_id) 
                        VALUES((select id from tbl_limites_tipos where tipo = '$tipo_limite'), $item_id, '$nombre', $limite, $estado, 0, now(), $user_id)";

            $result = $mysqli->query($query);
            $limite_anterior['limite'] = 'null';

            if($tipo_limite == 'local' || $tipo_limite == 'cliente')
            {
        ...
        $query = " INSERT INTO tbl_saldo_web_limites_historico(saldo_web_limite_id, limite_anterior, limite_nuevo,user_created_id, created_at)
                    VALUES(
...
} else if (isset($_POST["accion"]) && $_POST["accion"] === "obtener_cliente") {

	$id_cliente = $_POST["id_web"];

	$array_cliente = array();
	$array_cliente = api_calimaco_checkUser($id_cliente, 'checkUser_limites_saldos_web');
