...
if (isset($_POST["accion"]) && $_POST["accion"] === "mepa_mantenimiento_cuenta_contable_guardar") {
    ...
    if ((int)$usuario_id > 0) {
        ...
        if ((int)$id > 0) {
            ...
            $sql_insert_razon_social = "
                INSERT INTO mepa_tipo_documento (
                    nombre,
                    codigo,
                    status,
                    cuenta_contable,
...
if (isset($_POST["accion"]) && $_POST["accion"] === "mepa_mant_cuenta_contable_eliminar") {
    $usuario_id = $login ? $login['id'] : null;
    $fecha = date("Y-m-d H:i:s");
    $result = array(); 
    ...
    if ((int)$usuario_id > 0) {
        ...
        if ((int)$id_cuenta > 0) {
            ...
            //  ACTUALIZAR ESTADO
            $query_update_cuenta_contable = "UPDATE mepa_tipo_documento 
                                         SET 
                                             status = 0,
                                             user_updated_id = ?,
                                             updated_at = ?
