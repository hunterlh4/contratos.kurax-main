...
if (isset($_POST["accion"]) && $_POST["accion"]==="cont_listar_proveedores")
{
	...
	$cont_proveedor_param_moneda = $_POST['cont_proveedor_param_moneda'];
	...
	if ($cont_proveedor_param_moneda != "")
	{
		$where_moneda = " AND (c.moneda_id = $cont_proveedor_param_moneda OR (SELECT ct.moneda_id FROM cont_contraprestacion ct WHERE ct.contrato_id = c.contrato_id LIMIT 1) = $cont_proveedor_param_moneda) ";
	}
	...
	while($reg = $list_query->fetch_object()) 
	{
		...
		$data[] = array(
			...
			"11" => $reg->proveedor_categoria,			
			"12" => $reg->proveedor_tipo_categoria,
			...
			"15" => '<a class="btn btn-rounded btn-primary btn-xs" 
						href="./?sec_id=contrato&amp;sub_sec_id=detalleSolicitudProveedor&id=' . $reg->contrato_id . '"
						title="Ver detalle">
						<i class="fa fa-eye"></i> Ver
					</a>
					<a onclick="sec_contrato_proveedor_btn_descargar(\''.str_replace("C:/laragon/www/contratos.kurax-main", "", $reg->ruta).$reg->nombre.'\')"; title="Descargar Contrato de Proveedor Final" class="btn btn-primary btn-xs"><i class="fa fa-download"></i> Descargar
					</a>',
			"16" => '<button type="button" class="btn btn-' . $clase_boton_alertar . ' btn-sm" title="' . $titulo_boton_alerta . '" onclick="sec_contrato_proveedor_alerta('.$reg->contrato_id.')">
...
if (isset($_POST["accion"]) && $_POST["accion"]==="cont_proveedor_reporte_proveedor_excel")
{
	...
	while ($fila = $list_query->fetch_array()) 
	{
		...
		->setCellValue('R'.$i, $fila['fecha_suscripcion_proveedor'])
	...
	$objPHPExcel->getActiveSheet()->setTitle('contrato proveedores');
	...
	header('Content-Disposition: attachment;filename="Contrato de proveedores AT.xls');
	header('Cache-Control: max-age=0');

	$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
	$excel_path = '/var/www/html/files_bucket/contratos/descargas/Contrato de proveedores AT.xls';
	$excel_path_download = '/files_bucket/contratos/descargas/Contrato de proveedores AT.xls';
