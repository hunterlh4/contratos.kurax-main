...
// 			WHERE contrato_id = '" . $contrato_id . "'
	...
	$query_update =
		"
			...
			WHERE contrato_id = '" . $contrato_id . "'
		"; 
		$list_query = $mysqli->query($query_update);
	...
	if ($mysqli->error) {
		...
		$query_select =
		"
			SELECT tipo_contrato_id from cont_contrato 
			WHERE contrato_id = '" . $contrato_id . "'
		";

		$list_query = $mysqli->query($query_select);
		$li = $list_query->fetch_assoc();
		$tipo_contrato_id = $li['tipo_contrato_id']; 
		...
		// ORDER BY ce.id ASC");
		// 		$cantRegEstado = mysqli_num_rows($query_select);
        ...
        //     if (isset($lista_correos['cc'])) {
        //         foreach ($lista_correos['cc'] as $cc) {
        //             $mail->addAddress($cc);
        //         }
        //     }

        //     if (isset($lista_correos['bcc'])) {
        //         foreach ($lista_correos['bcc'] as $bcc) {
        //             $mail->addBCC($bcc);
        //         }
        //     }

        //     $mail->isHTML(true);
        //     $mail->Subject = "Solicitud de contrato aprobada - Código: " . $contrato_id;
        //     $mail->Body = "La solicitud de contrato ha sido aprobada correctamente.";
        //     $mail->CharSet = 'UTF-8';
        //     $mail->Encoding = 'base64';
        //     $mail->send();

        //     error_log("Correo enviado correctamente a los destinatarios.");
        // } catch (Exception $e) {
        //     error_log("Error al enviar el correo: " . $mail->ErrorInfo);
        // }

		$result["status"] = true;
		$result["data"] = $tipo_contrato_id;
		$result["message"] = "Datos registrados correctamente";
	}
}
...
function send_email_observacion_contrato_arrendamiento($observacion_id, $correos_adjuntos = "")
{
	...
	$sql = "SELECT
				...
				IFNULL(co.sigla, 'Contrato') AS sigla_correlativo,
    IFNULL(c.codigo_correlativo, 'N°') AS codigo_correlativo,
				pdag.correo AS email_director_aprobacion,
				pap.correo AS email_aprobacion_gerencia
			...
			FROM cont_observaciones o
				...
				INNER JOIN tbl_areas ar ON p.area_id = ar.id
				INNER JOIN cont_contrato c ON o.contrato_id = c.contrato_id
				LEFT JOIN cont_inmueble i ON c.contrato_id = i.contrato_id AND i.status = 1
				LEFT JOIN tbl_usuarios us ON c.persona_responsable_id = us.id
				LEFT JOIN tbl_personal_apt ps ON us.personal_id = ps.id
				LEFT JOIN tbl_usuarios ujc ON c.user_created_id = ujc.id
				LEFT JOIN tbl_personal_apt pjc ON ujc.personal_id = pjc.id
...
if (isset($_POST["accion"]) && $_POST["accion"] === "anular_adenda_firmada") {
	$usuario_id = $login ? $login['id'] : null;
...
if (isset($_POST["accion"]) && $_POST["accion"] === "guardar_estado_solicitud") {
	...
	if ($list_query->num_rows > 0) {
		while ($fila = $list_query->fetch_array()) {
			...
			$fecha_solicitud =  isset($fila['fecha_atencion_gerencia_proveedor']) ? $fila['fecha_atencion_gerencia_proveedor'] : (isset($fila['fecha_atencion_gerencia_interno']) ? $fila['fecha_atencion_gerencia_interno'] :
				$fila['created_at']);
		}
...
if (isset($_POST["accion"]) && $_POST["accion"] === "guardar_resolucion_contrato_firmado") {
	...
	if ((int) $usuario_id > 0) {
		...
		switch ($tipo_contrato_id) {
			case '1':
				$path = "/var/www/html/files_bucket/contratos/resolucion_contratos_firmados/locales/";
				break;
			case '2':
				$path = "/var/www/html/files_bucket/contratos/resolucion_contratos_firmados/proveedores/";
				break;
			case '5':
				$path = "/var/www/html/files_bucket/contratos/resolucion_contratos_firmados/acuerdo_confidencialidad/";
				break;
			case '6':
				$path = "/var/www/html/files_bucket/contratos/resolucion_contratos_firmados/agentes/";
				break;
			case '7':
				$path = "/var/www/html/files_bucket/contratos/resolucion_contratos_firmados/contratos_internos/";
				break;
		}
...
function send_email_solicitud_resolucion_contrato_firmado($resolucion_id, $reenvio = false)
{
	...
	switch ($tipo_contrato_id) {
		case '1':
			$url_solicitud = "detalle_solicitudv2";
			break;
		case '2':
			$url_solicitud = "detalleSolicitudProveedor";
			break;
		case '5':
			$url_solicitud = "detalle_solicitudv2_acuerdo_confidencialidad";
			break;
		case '6':
			$url_solicitud = "detalle_agente";
			break;
		case '7':
			$url_solicitud = "detalle_solicitudv2_interno";
			break;
	}
