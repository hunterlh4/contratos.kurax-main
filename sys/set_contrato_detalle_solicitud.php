	...
	if ($row_count > 0)
	{
		while ($row = $query->fetch_assoc())
		{
			...
			if($row["user_created_id"] == $login['id'])
			{
				...
				$html .= '<a onclick="sec_contrato_detalle_solicitud_eliminar_observacion('.$row['id'].')"><i class="fa fa-trash text-danger"></i></a>';
				$html .= '</div>';
...
if (isset($_POST["accion"]) && $_POST["accion"]==="guardar_contrato_firmado") {
	...
	if ((int) $usuario_id > 0) {
		$nombre_tienda = sec_contrato_detalle_solicitud_formato_nombre_de_tienda(replace_invalid_caracters(trim($_POST["nombre_tienda"])));
		$contrato_id = $_POST["contrato_id"];
...
if (isset($_POST["accion"]) && $_POST["accion"]==="sec_contrato_detalle_solicitud_verificar_giro")
{
	$message = "";

	$contrato_id = $_POST["contrato_id"];
	$cont_detalle_solicitud_giro_boton_param = $_POST['cont_detalle_solicitud_giro_boton_param'];
	$cont_detalle_solicitud_param_texto_motivo_giro = $_POST['cont_detalle_solicitud_param_texto_motivo_giro'];

	$parametro_insert = "";

	if($cont_detalle_solicitud_giro_boton_param == "1")
	{
		$parametro_insert = $cont_detalle_solicitud_giro_boton_param;
	}
	else
	{
		$parametro_insert = $cont_detalle_solicitud_param_texto_motivo_giro;
	}
...
function send_email_solicitud_contrato_arrendamiento_firmada($contrato_id)
{
	...
	while($sel = $sel_query->fetch_assoc())
	{
		...
		$body .= '<div style="width: 500px; text-align: center; font-family: arial;">';
		    $body .= '<a href="'.$host.'/?sec_id=contrato&sub_sec_id=detalle_solicitud&id='.$contrato_id.'" target="_blank">';
...
function send_email_observacion_contrato_agente($observacion_id , $correos_adjuntos = "")
{
	...
	$body .= '<div style="width: 500px; text-align: center; font-family: arial;">';
	    $body .= '<a href="'.$host.'/?sec_id=contrato&sub_sec_id=detalle_solicitud&id='.$contrato_id_de_la_obs.'" target="_blank" style="background-color: #4CAF50; border: none; color: white; padding: 5px 20px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px;">';
			$body .= '<b>Ver Solicitud</b>';
	    $body .= '</a>';
	$body .= '</div>';
...
function send_email_observacion_contrato_proveedor($observacion_id , $correos_adjuntos = "")
{
	...
	if (env('SEND_EMAIL') == 'produccion') {
		if ( !(empty($gerente_area_email)) ) {
			if ( sec_contrato_detalle_solicitud_is_valid_email($gerente_area_email) ) {
...
function send_email_observacion_acuerdo_confidencialidad($observacion_id , $correos_adjuntos = "")
{
	...
	$body .= '<div style="width: 500px; text-align: center; font-family: arial;">';
	    $body .= '<a href="'.$host.'/?sec_id=contrato&sub_sec_id=detalle_solicitud_acuerdo_confidencialidad&id='.$contrato_id_de_la_obs.'" target="_blank" style="background-color: #4CAF50; border: none; color: white; padding: 5px 20px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px;">';
			$body .= '<b>Ver Solicitud</b>';
	    $body .= '</a>';
	$body .= '</div>';
	...
	if ((int) $usuario_id > 0) {
		...
		if($tipo_valor == 'select_option' && $nombre_tabla == 'cont_adelantos'){
			...
			if (!empty(trim($_POST["valor_varchar"]))) {
				...
				$nombre_tienda = sec_contrato_detalle_solicitud_formato_nombre_de_tienda(replace_invalid_caracters(trim($_POST["valor_varchar"])));
...
if (isset($_POST["accion"]) && $_POST["accion"]==="cont_detalle_solicitud_proveedor_obtener_tipo_contrato")
{
...
if (isset($_POST["accion"]) && $_POST["accion"]==="guardar_estado_solicitud") {
	...
	if ($list_query->num_rows > 0) {
    	...
    	if (!is_numeric($dias_habiles)) {
			$dias_habiles = sec_contrato_detalle_solicitud_num_dias_habiles($fecha_solicitud);
...
function sec_contrato_detalle_solicitud_is_valid_email($str) {
...
function sec_contrato_detalle_solicitud_formato_nombre_de_tienda($nombre_de_tienda)
{
		...
		if ($num_registros == 0) {
			...
			while ($fila = $resultado->fetch_assoc()) {
				if ($autorizacion_municipal_id == "7") {
					...
					$body .= '<td>';
					$body .= '<a class="btn btn-rounded btn-primary btn-xs" onclick="sec_contrato_detalle_solicitud_ver_documento_en_visor(\'/files_bucket/contratos/licencias_municipales/DJ/\',\'' . $fila["nombre_file"] . '\', \'' . $fila["extension"] . '\' , \'Declaración Jurada de Actividades Simultáneas\');" data-toggle="tooltip" data-placement="left" title="' . $otros_detalles . '">';
					...
					if ($fila["status_licencia"] == "CONCLUIDO") {
						$body .= '<a class="btn btn-rounded btn-primary btn-xs" onclick="sec_contrato_detalle_solicitud_ver_documento_en_visor(\'/files_bucket/contratos/licencias_municipales/' . $nombre_carpeta . '/\',\'' . $fila["nombre_file"] . '\', \'' . $fila["extension"] . '\' , \'' . $nombre_autorización . '\');" data-toggle="tooltip" data-placement="left" title="' . $otros_detalles . '">';
					} else {
...
if (isset($_POST["accion"]) && $_POST["accion"]==="cancelar_solicitud")
{
	...
	if ( !((int) $usuario_id > 0) ) {
		...
		sec_contrato_detalle_solicitud_enviar_error($error);
	}
	...
	if($mysqli->error){
		sec_contrato_detalle_solicitud_enviar_error($mysqli->error . $query_update);
	}
...
function send_email_cancelar_solicitud($contrato_id)
{
	...
	if($mysqli->error){
		sec_contrato_detalle_solicitud_enviar_error($mysqli->error . $query_solicitud_cancelada);
	}
	...
	while($sel = $sel_query->fetch_assoc())
	{
		...
		if (sec_contrato_detalle_solicitud_is_valid_email($correo_cancelador)) {
			array_push($correos_add,$correo_cancelador);
		}

		if (sec_contrato_detalle_solicitud_is_valid_email($correo_aprobador)) {
			array_push($correos_add,$correo_aprobador);
		} else {
			if (sec_contrato_detalle_solicitud_is_valid_email($correo_director)) {
				array_push($correos_add,$correo_director);
			}
		}

		if (sec_contrato_detalle_solicitud_is_valid_email($correo_gerente)) {
			array_push($correos_add,$correo_gerente);
		} else {
			if (sec_contrato_detalle_solicitud_is_valid_email($correo_gerente_2)) {
...
function sec_contrato_detalle_solicitud_enviar_error($error) {
	$result["http_code"] = 500;
	$result["status"] = "error";
...
function send_email_observacion_adenda_acuerdo_confidencialidad_gerencia($observacion_id)
{
	...
	$body .= '<div style="width: 500px; text-align: center; font-family: arial;">';
	$body .= '<a href="' . $host . '/?sec_id=contrato&sub_sec_id=detalle_solicitud_acuerdo_confidencialidad&id=' . $contrato_id_de_la_obs . '&adenda_id=' . $adenda_id . '" target="_blank" style="background-color: #4CAF50; border: none; color: white; padding: 5px 20px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px;">';
	$body .= '<b>Ver Solicitud de Adenda</b>';
	$body .= '</a>';
	$body .= '</div>';
...
function sec_contrato_detalle_solicitud_num_dias_habiles($fecha_cambio_estado_solicitud, $fecha_actual_string = '')
{
...
if (isset($_POST["accion"]) && $_POST["accion"]==="corregir_dias_habiles") {
	...
	if ($list_query->num_rows > 0) {
		...
		if (is_numeric($dias_habiles)) {
			$dias_habiles = sec_contrato_detalle_solicitud_num_dias_habiles($fecha_solicitud, $fecha_cambio_de_estado);
