...
function send_email_solicitud_adenda_contrato_arrendamiento($adenda_id,$reenvio){
	include("db_connect.php");
	include("sys_login.php");
	...
	while($sel = $sel_query->fetch_assoc())
	{
		...
		if(!Empty($sel['correo'])){
			array_push($correo_adicionales, $sel['correo'] );
		}
	...
	$lista_correos = $correos->send_email_solicitud_adenda_contrato_arrendamiento($correo_adicionales);

	$cc = $lista_correos['cc'];

	$titulo_email = "Gestion - Sistema Contratos - Solicitud Modificada de Adenda de Arrendamiento ".$nombre_tienda.": Código - ";

	if($reenvio){
		$titulo_email = "Gestion - Sistema Contratos - Reenvío de Solicitud de Adenda de Arrendamiento ".$nombre_tienda.": Código - ";
	}
...
if (isset($_POST["accion"]) && $_POST["accion"]==="guardar_adenda_detalle") {
	...
	if(isset($_POST["id_del_registro"]) && !empty($_POST["id_del_registro"])) {
		...
		if($_POST["nombre_tabla"] == "cont_contrato_detalle"){
            ...
            else if($_POST["nombre_tabla"] == "cont_contraprestacion"){
                $query_detalle_id = "SELECT i.contrato_detalle_id FROM cont_inmueble as i INNER JOIN cont_inmueble_suministros AS s ON i.id = s.inmueble_id WHERE s.id = ".$id_del_registro;
			}else if ($_POST["nombre_tabla"] == "cont_mutuodinero") {
                $query_detalle_id = "
                    SELECT cd.id AS idcontratodetalle
                    FROM cont_mutuodinero AS m
                    INNER JOIN cont_contrato_detalle AS cd ON m.idcontrato = cd.contrato_id
                    WHERE m.idmutuodinero = " . $id_del_registro;
            }else if ($_POST["nombre_tabla"] == "cont_contrato") {
                $query_detalle_id = "
                    SELECT id AS contrato_detalle_id 
                    FROM cont_contrato_detalle 
                    WHERE contrato_id = " . $id_del_registro;
            }
            ...
            // var_dump($query_detalle_id);
            // die("hasta aqui llego");
			if($row_count > 0){
				$row = $list_query->fetch_assoc();
				// $contrato_detalle_id =  $row['contrato_detalle_id'];
                if ($_POST["nombre_tabla"] == "cont_mutuodinero") {
                    $contrato_detalle_id = $row['idcontratodetalle']; // Para cont_mutuodinero
                } else {
                    $contrato_detalle_id = $row['contrato_detalle_id']; // Para otras tablas
                }
			}
		}
	} else {
		...
		while ($row = $query->fetch_assoc()) {
			...
			if ($tipo_busqueda == '1' || $tipo_busqueda == '2') {
				if ($tipo_solicitud == 'CambiarPropietario') {
					$html .= '<td>';
					$html .= '<a class="btn btn-success btn-xs" onclick="sec_con_detalle_aden_arrend_reemplazar_propietario(' . $row["id"] . ')">';
				...
				if ($tipo_solicitud == 'NuevoPropietario') {
					$html .= '<td>';
					$html .= '<a class="btn btn-success btn-xs" onclick="sec_con_detalle_aden_arrend_agignar_propietario(' . $row["id"] . ')">';
					$html .= '<i class="fa fa-plus"></i> Agregar a la adenda como propietario';
					$html .= '</a>';
					$html .= '</td>';
				}
...
if (isset($_POST["accion"]) && $_POST["accion"]==="obtener_adendas_detalle_mutuo_dinero") {
	...
	if ($row_count > 0) {
		...
		while ($row = $list_query->fetch_assoc()) {
			...
			$html .= '<td>';
			$html .= '<a class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="top" title="Eliminar" onclick="sec_con_detalle_aden_arrendamiento_eliminar_detalle_adenda(' . $row["id"] . ')"><i class="fa fa-trash"></i></a>';
			$html .= '</td>';
			$html .= '</tr>';
	...
	if ($row_count_otros > 0) {
		while ($row = $list_query_otros->fetch_assoc()) {
			...
			if ($row["nombre_menu_usuario"] == 'Propietario') {
				...
				$html .= '<td rowspan="9">';
				$html .= '<a class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="top" title="Eliminar" onclick="sec_con_detalle_aden_arrendamiento_eliminar_detalle_adenda(' . $row["id"] . ')"><i class="fa fa-trash"></i></a></td>';
				$html .= '</tr>';
	...
	if ($row_count_otros > 0) {
		while ($row = $list_query_otros->fetch_assoc()) {
			...
			if ($row["nombre_menu_usuario"] == 'Propietario') {
				...
				$html .= '<td>' . $valores_nuevos[0]["tipo_persona"] . '</td>';
				$html .= '<td rowspan="9">';
				$html .= '<a class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="top" title="Eliminar" onclick="sec_con_detalle_aden_arrend_eliminar_detalle_adenda(' . $row["id"] . ')"><i class="fa fa-trash"></i></a></td>';
				$html .= '</tr>';
...
if (isset($_POST["accion"]) && $_POST["accion"]==="guardar_adenda") {
	...
	send_email_solicitud_adenda_contrato_arrendamiento($adenda_id,false);
	

	$result["status"] = 200;
	$result["message"] = "Se ha modificado exitosamente la adenda de contrato de arrendamiento.";
	$result["result"] = "ok";
	$result["error"] = $error;
	echo json_encode($result);
	exit();
			...
			while ($row = $query->fetch_assoc()){
				...
				<tr>
					...
					<td class="text-center">
						<a class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="top" title="Eliminar" onclick="sec_con_detalle_aden_arrend__eliminar_archivo_anexo('.$row['archivo_id'].')"><i class="fa fa-trash"></i></a>
					</td>
				</tr>';
				$index++;
			}
...
if (isset($_POST["accion"]) && $_POST["accion"]==="reenviar_email_solicitud_adenda_arrendamiento") {
	try {
		send_email_solicitud_adenda_contrato_arrendamiento($_POST['adenda_id'], true);
		$result["status"] = 200;
		$result["message"] = "Se ha reenviado correctamente la solicitud de adenda de Arrendamiento.";
		$result["result"] = '';
		echo json_encode($result);
		exit();
	}catch (Exception $e){
