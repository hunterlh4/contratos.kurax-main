...
if (isset($_POST["accion"]) && $_POST["accion"] === "guardar_adenda_detalle") {
	...
	if (isset($_POST["id_del_registro"]) && !empty($_POST["id_del_registro"])) {
		...
		if ($_POST["nombre_tabla"] == "cont_contrato_detalle") {
			...
			}  //llaves primarias de la tabla es igual al campo ID
...
if (isset($_POST["accion"]) && $_POST["accion"] === "guardar_adenda") {
	...
	$result["status"] = 200;
	$result["message"] = "Se ha modificado exitosamente la adenda de contrato de arrendamiento12." . $adenda_id . '-' . $aprobacion_obligatoria_id . 'LEGO';
	$result["result"] = "ok";
	$result["error"] = $error;
	echo json_encode($result);
	exit();
