	...
	if ((int) $usuario_id > 0) {
		...
		if($mysqli->error){
			...
			if ($id_tabla > 0) {
				...
				if($nombre_tabla == "cont_mandato"){
				$query_id_tabla = 'idmandato'; 
				}
			} else {
				...
				if ($tipo_valor == 'id_tabla') {
					...
					if ($nombre_tabla == 'cont_persona') {
						...
						SET
							" . $nombre_campo . " = '" . $nuevo_valor . "'
						WHERE id = '" . $id_del_registro_a_modificar . "'";
					}else if($nombre_tabla == 'cont_mandato'){
						$query_update = "
						UPDATE " . $nombre_tabla . "
						SET
							" . $nombre_campo . " = '" . $nuevo_valor . "'
						WHERE idmandato = '" . $id_del_registro_a_modificar . "'";
					}
...
//         $usuario_id = $login ? $login['id'] : null;
//         if (!((int) $usuario_id > 0)) {
//             $result["mensaje"] = "La sesión ha caducado. Se recomienda abrir una nueva pestaña e ingresar a gestión, ingresar su usuario y contraseña, ya logueado volver a esta pestaña y hacer click en el botón verde: Agregar adenda firmada.";
//             echo json_encode($result);
//             exit;
//         }
...
//         $path = "/var/www/html/files_bucket/contratos/adendas/locales/";
//         $name_file = 'adenda_firmada_' . $adenda_id;
//         $archivo_id = 0;
...
//             if (move_uploaded_file($filenametem, $path . $nombre_archivo)) {
//                 $comando = "INSERT INTO cont_archivos (
//                     contrato_id, adenda_id, tipo_archivo_id, nombre, extension, size, ruta, user_created_id, created_at
//                 ) VALUES (
//                     '" . $contrato_id . "', 
//                     '" . $adenda_id . "', 
//                     17, 
...
//                 $archivo_id = mysqli_insert_id($mysqli);
//             } else {
//                 throw new Exception("Error al mover el archivo adjunto.");
//             }
//         } else {
//             throw new Exception("No se ha subido un archivo válido.");
//         }

//         // Procesar los detalles de la adenda
//         $query = $mysqli->query("SELECT contrato_id FROM cont_adendas WHERE id = " . $adenda_id . " AND status = 1 AND procesado = 0");
//         if ($query->num_rows > 0) {
//             $row = $query->fetch_assoc();
//             $contrato_id = $row["contrato_id"];

//             $query = $mysqli->query("
//                 SELECT id, adenda_id, contrato_detalle_id, nombre_tabla, valor_original, nombre_campo_usuario, nombre_menu_usuario, nombre_campo, tipo_valor, valor_varchar, valor_int, valor_date, valor_decimal, valor_select_option, valor_id_tabla, id_del_registro_a_modificar, status 
//                 FROM cont_adendas_detalle 
//                 WHERE adenda_id = " . $adenda_id . " AND status = 1
//             ");
...
//                         break;
//                     case 'select_option':
//                         $nuevo_valor = ($nombre_campo == "ubigeo_id") ? $row['valor_varchar'] : $row['valor_int'];
//                         break;
//                     case 'id_tabla':
//                     case 'registro':
//                     case 'eliminar':
...
//                 // Ejecutar la actualización correspondiente
//                 if ($nuevo_valor !== null) {
//                     $query_update = "UPDATE " . $nombre_tabla . " SET " . $nombre_campo . " = '" . $nuevo_valor . "' WHERE idmandato = '" . $row['id_del_registro_a_modificar'] . "'";
//                     $mysqli->query($query_update);

//                     if ($mysqli->error) {
//                         throw new Exception("Error al actualizar la tabla " . $nombre_tabla . ": " . $mysqli->error);
//                     }
//                 }
//             }

//             // Actualizar el estado de la adenda
//             $fecha_aplicacion = date("Y-m-d", strtotime(str_replace("/", "-", $_POST["fecha_aplicacion"])));
//             $query_update = "UPDATE cont_adendas SET procesado = 1, archivo_id = " . $archivo_id . ", fecha_de_ejecucion_del_cambio = '" . $fecha_aplicacion . "' WHERE id = " . $adenda_id;
//             $mysqli->query($query_update);

//             if ($mysqli->error) {
//                 throw new Exception("Error al actualizar la adenda: " . $mysqli->error);
//             }

//             // Respuesta exitosa
//             $result["http_code"] = 200;
//             $result["status"] = "success";
//             $result["mensaje"] = "Adenda firmada guardada correctamente.";
//         } else {
//             throw new Exception("No se encontró la adenda o ya fue procesada.");
//         }
//     } catch (Exception $e) {
//         // Manejo de errores
