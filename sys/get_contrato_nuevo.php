...
if (isset($_POST["accion"]) && $_POST["accion"]==="sec_contrato_nuevo_obtener_tipos_de_archivos") {
	...
	WHERE
		...
		AND tipo_archivo_id NOT IN (16 , 17, 19,159)
...
if (isset($_POST["accion"]) && $_POST["accion"]==="sec_con_nuevo_prov_cargar_bancos") {
	...
	if (count($list_con_registros) == 0) {
		$result["http_code"] = 400;
		$result["status"] = "No hay registros.";
	} elseif (count($list_con_registros) > 0) {
		$result["http_code"] = 200;
		$result["status"] = "OK";
