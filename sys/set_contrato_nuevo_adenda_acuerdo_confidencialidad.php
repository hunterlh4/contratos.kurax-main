...
function send_email_solicitud_adenda_acuerdo_confidencialidad($adenda_id, $requiere_aprobacion_id)
{
	include("db_connect.php");
	include("sys_login.php");
	...
	while($sel = $sel_query->fetch_assoc())
	{
		...
		$body .= '<div style="width: 700px; text-align: center; font-family: arial;">';
		    $body .= '<a href="'.$host.'/?sec_id=contrato&sub_sec_id=detalle_solicitud_acuerdo_confidencialidad&id='.$contrato_id.'&adenda_id=' . $adenda_id . '" target="_blank" style="background-color: #4CAF50; border: none; color: white; padding: 5px 20px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px;">';
	...
	if ($requiere_aprobacion_id == 1) {
		...
		$lista_correos = $correos->send_email_solicitud_adenda_acuerdo_confidencialidad($correos_adicionales);
	}
	...
	$titulo_email .= " Solicitud de Adenda de Acuerdo de Confidencialidad N° " . $num_adenda . $rechazada . ": Código - ";
...
if (isset($_POST["accion"]) && $_POST["accion"]==="guardar_adenda") {
	...
	send_email_solicitud_adenda_acuerdo_confidencialidad($adenda_id, $aprobacion_obligatoria_id);
	

	$result["http_code"] = 200;
	$result["message"] = "Se ha registrado exitosamente la adenda de acuerdo de confidencialidad.";
	$result["result"] = "ok";
	$result["error"] = $error;
	echo json_encode($result);
	exit();
...
if (isset($_POST["accion"]) && $_POST["accion"]==="send_email_solicitud_adenda_acuerdo_confidencialidad") 
{
	$adenda_id = $_POST["adenda_id"];
	send_email_solicitud_adenda_acuerdo_confidencialidad($adenda_id, 2);
}
