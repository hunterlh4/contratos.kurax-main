...
include("../api/where_validar_usuario_permisos.php");
...
if(isset($_POST["opt"])){
	...
	if($opt=="save_item"){
		...
		//print_r($data["validacion"]);
		$cod_user= $data["validacion"]["user_id"];
		$cod_btn= 9;
		$sec_id=$data["validacion"]["sec_id"]; 
		$sub_sec_id= $data["validacion"]["sub_sec_id"];
		if (validar_usuario_permisos($cod_user,$cod_btn,$sec_id,$sub_sec_id)==1) {
			if($data["id"]=="new"){
				add_item($data);
			}else{
				save_item($data);
