...
if (isset($_POST["accion"]) && $_POST["accion"] === "cont_listar_mutuodinero") {
	...
	SELECT 
		...
		mut.plazo_devolucion,
		ce.fecha_inicio,
		mut.plazo_devolucion as fecha_fin, 
		mut.num_alerta_vencimiento,
		mut.se_enviara_alerta,
		mut.idmutuodinero,
	...
	FROM
		cont_contrato c
		LEFT JOIN cont_contrato_detalle as cd ON cd.contrato_id = c.contrato_id
		LEFT JOIN cont_inmueble i ON cd.id = i.contrato_detalle_id AND i.status = 1
		...
		LEFT JOIN cont_contraprestacion as contra on contra.contrato_id = c.contrato_id
		LEFT JOIN tbl_moneda m ON contra.moneda_id = m.id
	WHERE c.status IN (1,2) AND (c.etapa_id = 5 OR (c.etapa_id = 1 && c.estado_aprobacion = 1)) AND c.tipo_contrato_id = 15
		...
		if ($area_id == 2 || $area_id == 6) {
			...
			$data[] = array(
				...
				"9" => '<a class="btn btn-rounded btn-primary btn-sm" 
							href="./?sec_id=contrato&amp;sub_sec_id=detalle_solicitud_mutuo_dinero&id=' . $reg->contrato_id . '"
							title="Ver detalle">
							<i class="fa fa-eye"></i> Ver
						</a>' . $provision,
				"10" => ($reg->num_alerta_vencimiento == 0) ? '<button type="button" class="btn btn-primary btn-sm" title="Alerta por configurar" onclick="contrato_alerta_mutuodinero(' . $reg->contrato_id . ')">
					  <i class="glyphicon glyphicon-bell"></i>
					</button>' : '<button type="button" class="btn btn-success btn-sm" title="Alerta configurada" 
						onclick="contrato_alerta_mutuodinero(' . $reg->contrato_id . ')">
					  <i class="glyphicon glyphicon-bell"></i>
					</button>',
				"11" => ($reg->num_alerta_vencimiento == 0)
					? 'No existe'
					: '<input class="switch" id="checkbox_' . $reg->idmutuodinero . '" type="checkbox" 
						data-table="cont_mutuodinero" 
						data-id="' . $reg->idmutuodinero . '" 
						data-col="se_enviara_alerta" 
						data-on-value="1" 
						data-off-value="0" 
						data-ignore="true" ' . ($reg->se_enviara_alerta != 0 ? 'checked' : '') . '>',
				"12" => $estado,
				"13" => $reg->codigo_correlativo
			...
			$data[] = array(
				...
				"11" => ($reg->num_alerta_vencimiento == 0) ? '<button type="button" class="btn btn-primary btn-sm" title="Alerta por configurar" onclick="contrato_alerta_mutuodinero(' . $reg->contrato_id . ')">
					  <i class="glyphicon glyphicon-bell"></i>
					</button>' : '<button type="button" class="btn btn-success btn-sm" title="Alerta configurada" 
						onclick="contrato_alerta_mutuodinero(' . $reg->contrato_id . ')">
					  <i class="glyphicon glyphicon-bell"></i>
					</button>',
				"12" => ($reg->num_alerta_vencimiento == 0)
					? 'No existe'
					: '<input class="switch" id="checkbox_' . $reg->idmutuodinero . '" type="checkbox" 
						data-table="cont_mutuodinero" 
						data-id="' . $reg->idmutuodinero . '" 
...
if (isset($_POST["accion"]) && $_POST["accion"] === "obtener_dato_contrato") {
	...
	$query = "SELECT
				c.contrato_id,
				ce.idmutuodinero, 
				ce.plazo_devolucion as fecha_fin,
				ce.num_alerta_vencimiento,
				ce.se_enviara_alerta 
			FROM
				cont_contrato c
				INNER JOIN cont_mutuodinero ce ON c.contrato_id = ce.idcontrato 
...
if (isset($_POST["accion"]) && $_POST["accion"] === "actualizar_alerta_contrato") {
	...
	$query_update = "UPDATE cont_mutuodinero 
						SET num_alerta_vencimiento = '" . $numAlerta . "', 
							se_enviara_alerta = '1'  
						WHERE idmutuodinero = '" . $condicion_economica_id . "' 
					";
