...
function listar_transacciones($fecha_inicial, $fecha_final)
{
    ...
    $sql_query_locales_altenar = "
      SELECT id, cc_id, nombre
      FROM tbl_locales
      WHERE 
        UPPER(nombre) NOT LIKE '%TEST%' 
        AND UPPER(nombre) NOT like '%CAPACI%'
        AND cc_id is not null
        AND cc_id != ''
    ";
    $response_locales_altenar = $mysqli->query($sql_query_locales_altenar);
    $data_locales_altenar = array();
    while ($row = $response_locales_altenar->fetch_assoc()) {
      $data_locales_altenar[] = $row["cc_id"];
    }
    $search_data_locales_altenar = implode(", ", $data_locales_altenar);
    ...
    $query_teleservicios_apuestas_calimaco_ventas_x_producto = "
        ...
        from (
         ...
         AND CAST(Fecha AS DATE) < '" . $fecha_final . "'
          ...
          AND CT.CC_ID IN ($search_data_locales_altenar)
