...
//*******************************************************************************************************************
//*******************************************************************************************************************
function listar_transacciones($fecha_inicial, $fecha_final, $test_ok = false)
{
    ...
    $query_tabla_ventas_x_producto = "	
		SELECT
			tvp.id_reporte_teleservicios_concepto as id_concepto,
			tc.concepto,
		...
		WHERE
			tvp.fecha >= '{$fecha_inicial}' AND tvp.fecha < '{$fecha_final}'
			AND tvp.id_estado = 1
		ORDER BY tc.id ASC
		";
    ...
    if ($test_ok) {
        ...
        echo "<br>";
        echo "<br>";
        echo "<b> -- 1) MENSUAL - ANUAL: query_tabla_ventas_x_producto </b>";
        echo "<br>";
        echo "<br>";
        echo "<pre>";
        echo $query_tabla_ventas_x_producto;
        echo "</pre>";
        echo "<br>";
        echo "<br>";
        echo "<br>";
    ...
    $res_query_tabla_ventas_x_producto = $mysqli->query($query_tabla_ventas_x_producto);
    if ($mysqli->error) {
        $result["error_query_tabla_ventas_x_producto"] = $mysqli->error;
    }
    ...
    $query_teleservicios_ventas_x_producto = "
        SELECT case when ca.producto_id = 1 then 'Apuestas Deportivas BC'
                 when ca.producto_id = 4 then 'Bingo'
                    when ca.producto_id = 2 then 'Juevos Virtuales'  end as descripcion,
         IFNULL(SUM(ca.num_tickets), 0) AS tickets,
         IFNULL(SUM(ca.total_apostado), 0) AS apostado,
         IFNULL(sum(ca.total_apostado)/sum(ca.num_tickets), 0) promedio,
         IFNULL(sum(ca.num_tickets_ganados), 0) tickets_calculados,
         IFNULL(sum(ca.total_ganado), 0) calculado,
         IFNULL(sum(ca.total_apostado)-sum(ca.total_ganado), 0) resultado,
         IFNULL((sum(total_apostado)-sum(total_ganado))/sum(total_apostado), 0) hold,
         IFNULL(sum(num_tickets_ganados_pagados), 0)TicketsPagado,
         IFNULL(sum(total_pagado), 0)Pagado,
         IFNULL(sum(num_tickets_ganados_pagados)/sum(num_tickets_ganados), 0)PorcentajeTksPagado -- select *
        ...
        group by case when ca.producto_id = 1 then 'Apuestas Deportivas BC'
                 when ca.producto_id = 4 then 'Bingo'
                    when ca.producto_id = 2 then 'Juevos Virtuales' end
                    ;
    ";
    ...
    foreach ($res_query_teleservicios_ventas_x_producto as $key => $value) {
    ...
    $query_teleservicios_apuestas_calimaco_ventas_x_producto = "
        select Fecha
         ...
         ,sum(TicketsApostado)tickets
            ...
            ,sum(Pagado)Pagado
            ,ROUND(sum(TicketsPagado)/sum(TicketsGanado),2)PorcentajeTksPagado,
    ...
    if ($test_ok) {
        ...
        echo "<br>";
        echo "<br>";
        echo "<b> -- 2) query_teleservicios_apuestas_calimaco_ventas_x_producto </b>";
        echo "<br>";
        echo "<br>";
        echo "<pre>";
        echo $query_teleservicios_apuestas_calimaco_ventas_x_producto;
        echo "</pre>";
        echo "<br>";
        echo "<br>";
        echo "<br>";
    ...
    foreach ($res_query_teleservicios_apuestas_calimaco_ventas_x_producto as $key => $value) {
        $list_teleservicios_apuestas_calimaco_tabla_ventas_x_producto[] = $value;
    }

    array_splice($list_teleservicios_tabla_ventas_x_producto, 1, 0, $list_teleservicios_apuestas_calimaco_tabla_ventas_x_producto);
    ...
    if (count($list_teleservicios_tabla_ventas_x_producto) > 0) {
        ...
        if ((float)$result_temp['apostado'] <> 0 && (float)$result_temp['calculado'] <> 0) {
            $result_temp['resultado'] = ((float)$result_temp['apostado'] - (float)$result_temp['calculado']);
        }
    ...
    if ($test_ok) {
        ...
        echo "<br>";
        echo "<br>";
        echo "<b> -- 3) res_query_teleservicios_tabla_ventas_AD_x_caja </b>";
        echo "<br>";
        echo "<br>";
        echo "<pre>";
        echo $query_teleservicios_tabla_ventas_AD_x_caja;
        echo "</pre>";
        echo "<br>";
        echo "<br>";
        echo "<br>";
    ...
    foreach ($res_query_teleservicios_tabla_ventas_AD_x_caja as $key => $value) {
 ...
 ,IFNULL(sum(total_apostado)/sum(num_tickets_apostado), 0) Promedio
    ...
    ,sum(pagado)pagado
    ,IFNULL(sum(num_tickets_pagado)/sum(num_tickets_calculado), 0)PorcentajeTksPagado 
from (
...
group by Producto;";
    ...
    foreach ($res_query_teleservicios_tabla_ventas_jv_x_juego_update as $key => $value) {
        $list_teleservicios_tabla_ventas_jv_x_juego_update[] = $value;
    }

    $query_teleservicios_tabla_ventas_recargas_web = "SELECT 	
     'Recargas Web' AS concepto, 
 ...
 FROM	(
	...
	FROM  wwwapuestatotal_gestion.tbl_televentas_clientes_transaccion CT 
	-- INNER JOIN  wwwapuestatotal_gestion.tbl_locales CL ON CT.cc_id = CL.cc_id
	INNER JOIN tbl_caja c ON CT.turno_id = c.id
    INNER JOIN  tbl_locales CL ON c.local_caja_id= CL.id
...
;";
    ...
    $list_tabla_ventas_x_producto = array();
    $list_tabla_ventas_transaccionales = array();
    $list_tabla_ventas_x_caja = array();
    //$list_tabla_ventas_recargas_web = array();
    //$list_tabla_ventas_recargas_web_dif = array();
    $list_tabla_ventas_terminales = array();
    $list_tabla_ventas_otros_pagos = array();
    $list_tabla_ventas_jv_x_juego = array();

    while ($li = $res_query_tabla_ventas_x_producto->fetch_assoc()) {
        if ((int)$li["id_concepto"] >= 1 && (int)$li["id_concepto"] <= 2) {
            $list_tabla_ventas_x_producto[] = $li;
        }
        if ((int)$li["id_concepto"] === 4) {
            $list_tabla_ventas_transaccionales[] = $li;
        }
        if ((int)$li["id_concepto"] >= 5 && (int)$li["id_concepto"] <= 6) {
            $list_tabla_ventas_x_caja[] = $li;
        }
        /*if ((int)$li["id_concepto"] >= 7 && (int)$li["id_concepto"] <= 8) {
            $list_tabla_ventas_recargas_web_dif[] = $li;
        }*/
        if ((int)$li["id_concepto"] === 9) {
            $list_tabla_ventas_terminales[] = $li;
        }
        if ((int)$li["id_concepto"] >= 10 && (int)$li["id_concepto"] <= 11) {
            $list_tabla_ventas_otros_pagos[] = $li;
        }
        if ((int)$li["id_concepto"] >= 12 && (int)$li["id_concepto"] <= 15) {
            $list_tabla_ventas_jv_x_juego[] = $li;
        }
    }

    if (count($list_tabla_ventas_jv_x_juego) > 0) {
        ...
        foreach ($list_tabla_ventas_jv_x_juego as $value) {
            $cont_array_detalle++;
        ...
        $list_tabla_ventas_jv_x_juego[] = $result_temp;
        $list_tabla_ventas_jv_x_juego[$cont_array_detalle]["id_concepto"] = 3;
        $list_tabla_ventas_jv_x_juego[$cont_array_detalle]["concepto"] = "Juevos Virtuales";
        $list_tabla_ventas_x_producto[] = $list_tabla_ventas_jv_x_juego[$cont_array_detalle];
        $list_tabla_ventas_jv_x_juego[$cont_array_detalle]["id_concepto"] = 0;
        $list_tabla_ventas_jv_x_juego[$cont_array_detalle]["concepto"] = "Acumulado";

        // Format numero
        foreach ($list_tabla_ventas_jv_x_juego as $key => $value) {
            $list_tabla_ventas_jv_x_juego[$key]["resultado"] = number_format($value["resultado"], 2, ".", ",");
            $list_tabla_ventas_jv_x_juego[$key]["total_tickets_apostado"] = number_format($value["total_tickets_apostado"], 2, ".", ",");
            $list_tabla_ventas_jv_x_juego[$key]["total_tickets_calculado"] = number_format($value["total_tickets_calculado"], 2, ".", ",");
            $list_tabla_ventas_jv_x_juego[$key]["total_tickets_pagado"] = number_format($value["total_tickets_pagado"], 2, ".", ",");
            $list_tabla_ventas_jv_x_juego[$key]["promedio"] = number_format($value["promedio"], 2, ".", ",");
            $list_tabla_ventas_jv_x_juego[$key]["hold"] = number_format($value["hold"], 2, ".", ",");
            $list_tabla_ventas_jv_x_juego[$key]["porcentaje_tickets_pagado"] = number_format($value["porcentaje_tickets_pagado"], 2, ".", ",");
        }
    }

    if (count($list_tabla_ventas_x_producto) > 0) {
        ...
        foreach ($list_tabla_ventas_x_producto as $value) {
            $result_temp['num_tickets_apostado'] += $value["num_tickets_apostado"];
            $result_temp['total_tickets_apostado'] += $value["total_tickets_apostado"];
        ...
        $list_tabla_ventas_x_producto[] = $result_temp;

        // Format numero
        foreach ($list_tabla_ventas_x_producto as $key => $value) {
            $list_tabla_ventas_x_producto[$key]["resultado"] = number_format($value["resultado"], 2, ".", ",");
            $list_tabla_ventas_x_producto[$key]["total_tickets_apostado"] = number_format($value["total_tickets_apostado"], 2, ".", ",");
            $list_tabla_ventas_x_producto[$key]["total_tickets_calculado"] = number_format($value["total_tickets_calculado"], 2, ".", ",");
            $list_tabla_ventas_x_producto[$key]["total_tickets_pagado"] = number_format($value["total_tickets_pagado"], 2, ".", ",");
            $list_tabla_ventas_x_producto[$key]["promedio"] = number_format($value["promedio"], 2, ".", ",");
            $list_tabla_ventas_x_producto[$key]["hold"] = number_format($value["hold"], 2, ".", ",");
            $list_tabla_ventas_x_producto[$key]["porcentaje_tickets_pagado"] = number_format($value["porcentaje_tickets_pagado"], 2, ".", ",");
        }

    }

    if (count($list_tabla_ventas_x_caja) > 0) {
        ...
        foreach ($list_tabla_ventas_x_caja as $value) {
            $result_temp['num_tickets_apostado'] += $value["num_tickets_apostado"];
            $result_temp['total_tickets_apostado'] += $value["total_tickets_apostado"];
        ...
        $list_tabla_ventas_x_caja[] = $result_temp;

        // Format numero
        foreach ($list_tabla_ventas_x_caja as $key => $value) {
            $list_tabla_ventas_x_caja[$key]["resultado"] = number_format($value["resultado"], 2, ".", ",");
            $list_tabla_ventas_x_caja[$key]["total_tickets_apostado"] = number_format($value["total_tickets_apostado"], 2, ".", ",");
            $list_tabla_ventas_x_caja[$key]["total_tickets_calculado"] = number_format($value["total_tickets_calculado"], 2, ".", ",");
            $list_tabla_ventas_x_caja[$key]["total_tickets_pagado"] = number_format($value["total_tickets_pagado"], 2, ".", ",");
            $list_tabla_ventas_x_caja[$key]["promedio"] = number_format($value["promedio"], 2, ".", ",");
            $list_tabla_ventas_x_caja[$key]["hold"] = number_format($value["hold"], 2, ".", ",");
            $list_tabla_ventas_x_caja[$key]["porcentaje_tickets_pagado"] = number_format($value["porcentaje_tickets_pagado"], 2, ".", ",");
        }
    }

    /*if (count($list_tabla_ventas_recargas_web_dif) > 0) {
        ...
        foreach ($list_tabla_ventas_recargas_web_dif as $value) {
            $result_temp['num_tickets_apostado'] += $value["num_tickets_apostado"];
            $result_temp['total_tickets_apostado'] += $value["total_tickets_apostado"];
        }
        if ((float)$result_temp['total_tickets_apostado'] <> 0 && (float)$result_temp['num_tickets_apostado'] <> 0) {
        ...
        $list_tabla_ventas_recargas_web[] = $result_temp;

        // Format numero
        foreach ($list_tabla_ventas_recargas_web as $key => $value) {
            $list_tabla_ventas_recargas_web[$key]["total_tickets_apostado"] = number_format($value["total_tickets_apostado"], 2, ".", ",");
            $list_tabla_ventas_recargas_web[$key]["promedio"] = number_format($value["promedio"], 2, ".", ",");
        }
    }*/

    if (count($list_tabla_ventas_otros_pagos) > 0) {
        ...
        foreach ($list_tabla_ventas_otros_pagos as $value) {
            $result_temp['num_tickets_apostado'] += $value["num_tickets_apostado"];
            $result_temp['total_tickets_apostado'] += $value["total_tickets_apostado"];
        }
        ...
        $list_tabla_ventas_otros_pagos[] = $result_temp;

        // Format numero
        foreach ($list_tabla_ventas_otros_pagos as $key => $value) {
            $list_tabla_ventas_otros_pagos[$key]["total_tickets_apostado"] = number_format($value["total_tickets_apostado"], 2, ".", ",");
            $list_tabla_ventas_otros_pagos[$key]["promedio"] = number_format($value["promedio"], 2, ".", ",");
        }
    }
    ...
    if (count($list_teleservicios_tabla_ventas_jv_x_juego_update) > 0) {
        ...
        $result_temp = array();
        $result_temp['Producto'] = 'Acumulado';
        $result_temp['num_tickets_apostado'] = 0;
        ...
        /*if ((float)$result_temp['num_tickets_pagado'] <> 0 && (float)$result_temp['num_tickets_calculado'] <> 0) {
            $result_temp['PorcentajeTksPagado'] = ((float)$result_temp['num_tickets_pagado'] / (float)$result_temp['num_tickets_calculado']);
        }*/

        if ((float)$result_temp['total_apostado'] <> 0 && (float)$result_temp['Apostado'] <> 0) {
            $result_temp['Resultado'] = ((float)$result_temp['total_apostado'] - (float)$result_temp['Apostado']);
        }
    ...
    $result["result_tabla_ventas_x_producto"] = [];
    $result["result_tabla_ventas_transaccionales"] = [];
    $result["result_tabla_ventas_x_caja"] = [];
    //$result["result_tabla_ventas_recargas_web"] = [];
    $result["result_tabla_ventas_terminales"] = [];
    $result["result_tabla_ventas_otros_pagos"] = [];
    $result["result_tabla_ventas_jv_x_juego"] = [];
    ...
    } elseif (count($list_tabla_ventas_x_producto) > 0) {
        $result["http_code"] = 200;
        $result["status"] = "ok";
        $result["result_tabla_ventas_x_producto"] = $list_tabla_ventas_x_producto;
        $result["result_tabla_ventas_transaccionales"] = $list_tabla_ventas_transaccionales;
        $result["result_tabla_ventas_x_caja"] = $list_tabla_ventas_x_caja;
        //$result["result_tabla_ventas_recargas_web"] = $list_tabla_ventas_recargas_web;
        $result["result_tabla_ventas_terminales"] = $list_tabla_ventas_terminales;
        $result["result_tabla_ventas_otros_pagos"] = $list_tabla_ventas_otros_pagos;
        $result["result_tabla_ventas_jv_x_juego"] = $list_tabla_ventas_jv_x_juego;
    } else {
