...
if(isset($_POST["get_archivos"])){
	if(array_key_exists($menu_id,$usuario_permisos) && in_array("view", $usuario_permisos[$menu_id])){
		...
		$list_where="WHERE ad.active = 1 AND ac.active = 1";
			...
			for ($i=0; $i < count($_FILES['fileArchivosModal']["name"]); $i++) { 
				...
				if($size <= 10485760){
					if(in_array($ext, $valid_extensions)) {
						if ($data["categoria"] == 10) {
							$filepath = '/var/www/storage/website/landing-dinamico/'.$data["chk_digital"].'/'.$filename;
							move_uploaded_file($tmp,$filepath);

							$src = 'https://storage.apuestatotal.com/website/landing-dinamico/'.$data["chk_digital"].'/'.$filename;
							$arr_files[$i]['src'] = $src;
							...
							$filepath = '/var/www/html/files_bucket/drive/'.$filename; 
							move_uploaded_file($tmp,$filepath);
							$app_url = env('APP_URL');
						...
						$status = false;
						$message .= "<div class='small text-left alert alert-danger'><b>".$file."</b> extensión inválida. Solo es permitido las extensiones: 'jpeg', 'jpg', 'png', 'gif', 'pdf', 'doc', 'docx', 'xls', 'xlsx', 'csv', 'pdf'.</div>";
					}
				}
				else {
					$status = false;
