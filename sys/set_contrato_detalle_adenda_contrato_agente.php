...
function send_email_solicitud_adenda_contrato_proveedor($adenda_id)
{
	...
	$correos_adicionales = [$usuario_creacion_correo];
...
if (isset($_POST["accion"]) && $_POST["accion"]==="obtener_adenda_detalle_contrato") {
	...
	$result["status"] = 200;
	$result["message"] = "Se ha registrado exitosamente la adenda de Contrato de Agente.";
...
if (isset($_POST["accion"]) && $_POST["accion"]==="obtener_adendas_detalle") {
	...
	if ($row_count > 0) {
		...
		while ($row = $list_query->fetch_assoc()) {
			...
			$html .= '<td>';
			$html .= '<a class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="top" title="Eliminar" onclick="sec_con_detalle_aden_cont_agente_eliminar_detalle_adenda(' . $row["id"] . ')"><i class="fa fa-trash"></i></a>';
			$html .= '</td>';
			$html .= '</tr>';
	...
	if ($row_count_otros > 0) {
		while ($row = $list_query_otros->fetch_assoc()) {
			if ($row["nombre_menu_usuario"] == 'Propietario') {
				...
				$html .= '<td rowspan="9">';
				$html .= '<a class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="top" title="Eliminar" onclick="sec_con_detalle_aden_cont_agente_eliminar_detalle_adenda(' . $row["id"] . ')"><i class="fa fa-trash"></i></a></td>';
				$html .= '</tr>';
...
if (isset($_POST["accion"]) && $_POST["accion"]==="guardar_adenda_detalle_nuevos_registros") {
	...
	if($_POST['tabla'] == "nuevo_propietario"){
		$contrato_id = $_POST['contrato_id'];
		$tipo_persona_propietario = $_POST['tipo_persona_propietario'];
		$nombreNuevoPropietario = $_POST['nombreNuevoPropietario'];
		$tipo_documento_identidad = $_POST['tipo_documento_identidad'];
		$dniPropietario = $_POST['dniPropietario'];
		$rucpropietario = $_POST['rucpropietario'];
		$domicilio_propietario = $_POST['domicilio_propietario'];
