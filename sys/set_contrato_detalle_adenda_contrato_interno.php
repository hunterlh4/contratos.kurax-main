...
if (isset($_POST["accion"]) && $_POST["accion"]==="guardar_adenda_detalle") {
	...
	if ($tipo_valor == 'varchar') {
		...
		$valor_varchar = $_POST["valor_textarea"];
		$valor_int = "NULL";
		$valor_date = "NULL";
		$valor_decimal = "NULL";
		$valor_select_option = "NULL";
...
if (isset($_POST["accion"]) && $_POST["accion"]==="obtener_adendas_detalle") {
	...
	if ($row_count > 0) {
		...
		while ($row = $list_query->fetch_assoc()) {
			...
			$html .= '<td>';
			$html .= '<a class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="top" title="Eliminar" onclick="sec_con_detalle_aden_cont_interno_eliminar_detalle_adenda(' . $row["id"] . ')"><i class="fa fa-trash"></i></a>';
			$html .= '</td>';
			$html .= '</tr>';
	...
	if ($row_count_otros > 0) {
		while ($row = $list_query_otros->fetch_assoc()) {
			if ($row["nombre_menu_usuario"] == 'Propietario') {
				...
				$html .= '<td rowspan="9">';
				$html .= '<a class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="top" title="Eliminar" onclick="sec_con_detalle_aden_cont_interno_eliminar_detalle_adenda(' . $row["id"] . ')"><i class="fa fa-trash"></i></a></td>';
				$html .= '</tr>';
...
if (isset($_POST["accion"]) && $_POST["accion"]==="guardar_adenda_detalle_nuevos_registros") {
	...
	if ($_POST['tabla'] == "nuevo_representante_legal") {
		$contrato_id = $_POST['contrato_id'];
		$numero_dni = $_POST['numero_dni'];
		$nombre_representante = $_POST['nombre_representante'];
		$id_banco = $_POST['id_banco'];
		$numero_cuenta_detraccion = $_POST['numero_cuenta_detraccion'];
		$numero_cuenta = $_POST['numero_cuenta'];
		$numero_cci = $_POST['numero_cci'];
		...
		if($mysqli->error){
			...
			$result["http_code"] = 404;
			$result["status"] = "A ocurrido un error al registrar al Representante Legal.";
			$result["result"] = 0;
			$result["error"] = $error;
