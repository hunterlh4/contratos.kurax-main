...
function transacciones_build_liquidaciones($data){
	...
	if(array_key_exists("servicio_id", $data)){
		...
		$db_cabeceras['info']['time_init'] = date('c');
			...
			foreach (array_keys($locales) as $cc_id) {
				...
				*/
				...
				$db_cabeceras[$cc_id] = [
					...
					'pagado_en_otra_tienda' => 0,//$response_tickets["pagado_en_otra_tienda"],
					'pagado_de_otra_tienda' => 0,//$response_tickets["pagado_de_otra_tienda"],
					'total_pagos_fisicos' => ($response_tickets["total_pagado"]),
		...
		elseif((int)$data["servicio_id"] == 13){ // CARRERA DE CABALLOS
			...
			*/
			...
			// termmina proceso. aqui insertaba
	...
	// termmina proceso. aqui insertaba
		...
		$db_cabeceras['info']['time_end'] = date('c');
	}else{
		$return["error"]="no_servicio_id";
	}

	echo json_encode($db_cabeceras);
}
