...
if(isset($_POST["sec_servicio_tecnico_derivacion_cargar_solicitud_ant"])){
...
if(isset($_POST["sec_servicio_tecnico_derivacion_cargar_solicitud"])){
	$solicitud_id = $_POST["solicitud_id"];
	$command = "SELECT
				sth.id AS id,
				...
				inci.incidencia_txt  as reporte,
				/*sth.equipo ,*/
				stequipo.nombre as equipo ,
				stequipo.id as equipo_id ,
				...
				LEFT JOIN tbl_servicio_tecnico_equipo stequipo ON stequipo.id = sth.equipo_id
				LEFT JOIN tbl_locales l ON  l.id = inci.local_id
				LEFT JOIN tbl_zonas z ON  z.id = l.zona_id
				LEFT JOIN tbl_razon_social rs ON rs.id = z.razon_social_id 
				WHERE sth.id = {$solicitud_id}" ;
	$list_query = $mysqli->query($command);
    $list = $list_query->fetch_assoc();
...
if(isset($_POST["set_servicio_tecnico_derivacion_update_ante"])){
	$user_id = $login["id"];
	extract($_POST);
	...
	$bcc = ["rockypsn@testtest.gmail.com"];
	$mail = [
		"subject" => "Servicio Técnico Sol. Terminada - " .$registro_sol["Local"],
		"body"    => $body,
		"cc"      => $cc,
		"bcc"     => $bcc,
	];
	...
	$tecnico_id = isset($tecnico_id) && $tecnico_id != "" ? ",tecnico_id = '" . $tecnico_id . "'" : "";
	
	$command = "UPDATE tbl_servicio_tecnico SET 
					 estado_vt = '$estado'			 
					,updated_at = now()
...
if(isset($_POST["set_servicio_tecnico_derivacion_update"])){
	$user_id = $login["id"];
	extract($_POST);
	if(!isset($estado) || $estado == ""){
		$return["error"] = "estado";
	...
	if($estado == 3){
		if(!isset($comentario_observado) || $comentario_observado == ""){
			$return["error"] = "comentario observado";
			$return["error_msg"] = "El campo comentario es necesario";
			$return["error_focus"] = "comentario observado";
			die(json_encode($return));
		}
	}

	if($estado == 4){
		if((!isset($comentario_terminado) || $comentario_terminado == "") && !file_exists($_FILES['foto_terminado_update']['tmp_name'])){
	...
	$mail = [
		"subject" => "Servicio Tecnico Sol. Terminada - " .$registro_sol["Local"],
		"body"    => $body,
		"cc"      => $cc,
		"bcc"     => $bcc,
	];
	...
	$equipo_temp = isset($equipo) ? ",equipo" : "";
	$equipo = isset($equipo) ? ",'$equipo'" : "";

	$equipo_id_temp = isset($equipo_id) ? ",equipo_id" : "";
	$equipo_id = isset($equipo_id) ? ",'$equipo_id'" : "";

	$tecnico_id_temp = isset($tecnico_id) && $tecnico_id != "" ? ",tecnico_id = '" . $tecnico_id . "'" : "";
	$tecnico_id = isset($tecnico_id) && $tecnico_id != "" ? ",tecnico_id'" . $tecnico_id . "'" : "";
	...
	if($estado == 3){
		$comentario =      	     isset($comentario_observado) && $comentario_observado != "" ? "$comentario_observado" : "";
	}

	$insert_command = "INSERT INTO tbl_servicio_tecnico_historial
	(
		servicio_tecnico_id
	...
	$equipo_upd = $equipo_id != "" ? ",equipo_id = " . $_POST["equipo_id"]: " " ;
