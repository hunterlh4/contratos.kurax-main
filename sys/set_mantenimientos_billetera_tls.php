...
function mant_bil_obtener_balances($cliente_id){
	global $mysqli;
...
if (isset($_POST["accion"]) && $_POST["accion"] === "obtener_televentas_cliente") {
	...
	if ((int) $busqueda_tipo >= 0 && (int) $busqueda_tipo <= 2) {
		...
		$tipo_texto = "número de documento";
	}
	...
	if ($mysqli->error) {
 	...
 	if (count($list_1) > 0) {
		...
		$result["result"] = "El " . $tipo_texto . " ingresado no existe como cliente.";
	}
}


//*******************************************************************************************************************
// OBTENER DIFERENCIAS DE BALANCE
//*******************************************************************************************************************
if (isset($_POST["accion"]) && $_POST["accion"] === "obtener_diferencias_balance") {
	...
	$query = "
			...
			from (
					SELECT 
					  ...
					  ba1.id balance_id_tipo_4,
					  ba1.updated_at updated_at_tipo_4,
					  IFNULL(ba1.balance, 0) balance, 
					  IFNULL(ba4.balance, 0) balance_deposito,
					  IFNULL(ba5.balance, 0) balance_retiro_disponible,
					  IFNULL(ba6.balance, 0) balance_bono_at
					FROM 
					  tbl_televentas_clientes c 
					  LEFT JOIN tbl_televentas_clientes_balance ba1 ON ba1.cliente_id = c.id AND ba1.tipo_balance_id = 1 
					  LEFT JOIN tbl_televentas_clientes_balance ba4 ON ba4.cliente_id = c.id AND ba4.tipo_balance_id = 4 
			...
			where a.balance != (a.balance_deposito+a.balance_retiro_disponible)
			";
		//echo $query;
...
if (isset($_POST["accion"]) && $_POST["accion"] === "obtener_transacciones_por_cliente") {
	...
	// BALANCE BILLETERO
	$list_balance = array();
	$list_balance = mant_bil_obtener_balances($id_cliente);
	...
	if(count($list_balance)>0){
		...
		$query = "
				...
				IFNULL(tra.id_tipo_balance, 0), id_tipo_balance
			FROM
			...
			WHERE
				...
				OR (tra.tipo_id IN (38)) -- Corrección Balance Aumentar
				OR (tra.tipo_id IN (39)) -- Corrección Balance Disminuir
				)
				$where_balance_dinero_at
...
//*******************************************************************************************************************
// EXPORTAR EXCEL DIFERENCIAS BALANCE
//*******************************************************************************************************************
if (isset($_POST["accion"]) && $_POST["accion"] === "tabla_diferencias_balance_export_xls") {
	...
	$query = "
			SELECT
				if (tipo_doc = '0', 'DNI' , if (tipo_doc = '1', 'CE/PTP' , 'PASAPORTE')) as tipo_doc,
				num_doc,
				nombre,
				apellido_paterno,
				apellido_materno,
			...
			if ($mysqli->error) {
				...
				$headers = [
					...
					"balance_deposito" => "Balance No Retirable<",
					"balance_retiro_disponible" => "Balance Retirable",
					"balance_bono_at" => "Balance Bono AT"
				];
				array_unshift($result_data, $headers);
				...
				$objPHPExcel->getActiveSheet()->fromArray($result_data, null, 'A1');
				$date = new DateTime();
				$file_title = "reporte_diferencias_balance_" . $date->getTimestamp();

				if (!file_exists('/var/www/html/export/files_exported/mantenimiento_biletera_tls/')) {
					mkdir('/var/www/html/export/files_exported/mantenimiento_biletera_tls/', 0777, true);
				}
				...
				$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
				$excel_path = '/var/www/html/export/files_exported/mantenimiento_biletera_tls/' . $file_title . '.xls';
				$excel_path_download = '/export/files_exported/mantenimiento_biletera_tls/' . $file_title . '.xls';
				$url = $file_title . '.xls';
...
if ($_POST["accion"] === "sec_mant_bil_obtener_balance_x_tipo") {
	include("function_replace_invalid_caracters.php");
	...
	$cmd = "
		SELECT 
			balance
		FROM tbl_televentas_clientes_balance cb
		WHERE 
			tipo_balance_id = " . $tipo_balance . "
			AND cliente_id = " . $id_cliente;

	$list_cmd = $mysqli->query($cmd);
	$list = array();
	if ($mysqli->error) {
		$result["query_error"] = $mysqli->error;
		$result["query"] = $cmd;
	} else {
		while ($li_1 = $list_cmd->fetch_assoc()) {
			$list[] = $li_1;
		}
	}
...
if ($_POST["accion"] === "sec_mant_bil_set_balance_edit") {
	include("function_replace_invalid_caracters.php");
	$area_id = $login ? $login['area_id'] : 0;
	if($area_id != 6){
		$result["http_code"] = 400;
		$result["status"] = 'Lo sentimos, este modulo es de uso restringido para T.I.';
	...
	$id_cliente		= $_POST["id_cliente"];
	$monto			= $_POST["monto"];
	$monto_anterior	= $_POST["monto_anterior"];
	$nombre_balance	= $_POST["nombre_balance"];
	...
	$list_balance = array();
	$list_balance = mant_bil_obtener_balances($id_cliente);
	$balance_detalle_actual = 0;
	$monto_balance = 0;
	$monto_balance_deposito = 0;
	$monto_balance_retiro_disponible = 0;
	$monto_balance_dinero_at = 0;

	if(count($list_balance)>0){

		$monto_balance = $list_balance[0]["balance"];
		$monto_balance_deposito = $list_balance[0]["balance_deposito"];
		$monto_balance_retiro_disponible = $list_balance[0]["balance_retiro_disponible"];
		$monto_balance_dinero_at = $list_balance[0]["balance_dinero_at"];

		if((int)$tipo_balance === 1){
			$balance_detalle_actual = $list_balance[0]["balance"];
		}else if((int)$tipo_balance===4){
		...
		$result["http_code"] = 400;
		$result["status"] = 'Ocurrió un error al obtener los balances del cliente';
		echo json_encode($result);exit();
	}
	$cod_tipo_transaccion = '0';
	$monto_transaccion = 0;
	$nuevo_balance = 0;
	if((float)$monto > (float)$balance_detalle_actual){
		$cod_tipo_transaccion = '38'; //Aumentar
		$monto_transaccion = $monto - $balance_detalle_actual;
	}else if((float)$monto < (float)$balance_detalle_actual){
		$cod_tipo_transaccion = '39'; //Disminuir
		$monto_transaccion = $balance_detalle_actual - $monto;
	}
	
	$monto_transaccion = (double)number_format($monto_transaccion, 2);
	...
	$query_3 = "
		...
		AND monto ='$monto_transaccion' 
		AND estado = 1
		AND id_tipo_balance = '$tipo_balance'
		";
	$list_query = $mysqli->query($query_3);
	$list_transaccion = array();
	while ($li2 = $list_query->fetch_assoc()) {
		$list_transaccion[] = $li2;
	}
	if (count($list_transaccion) === 1) {
		$transaccion_id = $list_transaccion[0]["id"];
		$auditoria_id = mant_bil_api_calimaco_auditoria_insert('mant_billetera_tls_edit_balance', $transaccion_id, $id_cliente, '');

		mant_bil_query_tbl_televentas_clientes_balance('update', $id_cliente, $tipo_balance, $monto);
		mant_bil_query_tbl_televentas_clientes_balance_transaccion('insert', $transaccion_id, 
				$id_cliente, $tipo_balance, $balance_detalle_actual, $monto_transaccion, $monto);

		send_mail_alert_edit_balance($cliente_nombre, $id_cliente, $usuario_nombre, $usuario_id, $nombre_balance, $tipo_balance, $monto, $balance_detalle_actual, $num_doc_cliente, $tipo_doc, $observacion, $monto_balance, $monto_balance_deposito, $monto_balance_retiro_disponible, $monto_balance_dinero_at);

		$result["http_code"] = 200;
		$result["status"] = "OK";

		mant_bil_api_calimaco_auditoria_update($auditoria_id, $transaccion_id, '', 1);
	}else{
		$result["http_code"] = 400;
		$result["status"] = "Error al consultar la transacción insertada";
	}
}

function mant_bil_query_tbl_televentas_clientes_balance($action, $cliente_id, $tipo_id, $balance){
	global $mysqli;
...
function mant_bil_query_tbl_televentas_clientes_balance_transaccion($action, $transaccion_id, $cliente_id, $tipo_balance_id, $balance_actual, $monto, $balance_nuevo){
	global $mysqli;
	global $login;
...
function mant_bil_api_calimaco_auditoria_insert($method, $txn_id, $client_id, $body){
	global $mysqli;
	global $login;
...
function mant_bil_api_calimaco_auditoria_update($auditoria_id, $txn_id, $response, $status){
	global $mysqli;
	date_default_timezone_set("America/Lima");
	$date_time = date('Y-m-d H:i:s');
...
function send_mail_alert_edit_balance($cliente_nombre, $id_cliente, $usuario_nombre, $usuario_id, $nombre_balance, $tipo_balance, $nuevo_balance, $balance_anterior, $num_doc_cliente, $tipo_doc, $observacion, $monto_balance, $monto_balance_deposito, $monto_balance_retiro_disponible, $monto_balance_dinero_at){

	$new_balance = $monto_balance;
	$new_balance_deposito = $monto_balance_deposito;
	$new_balance_retiro_disponible = $monto_balance_retiro_disponible;
	$new_balance_dinero_at = $monto_balance_dinero_at;
	$monto_diferencia = 0;

	$css_balance = '';
	$css_balance_deposito = '';
	$css_balance_retiro_disponible = '';
	$css_balance_dinero_at = '';

	if($nombre_balance=='BALANCE GENERAL'){
		$new_balance = $nuevo_balance;
		$css_balance = 'color:red;';

		$monto_diferencia = number_format(($nuevo_balance - $monto_balance), 2, '.', '');

	}elseif($nombre_balance=='BALANCE NO RETIRABLE'){
		$new_balance_deposito = $nuevo_balance;
		$css_balance_deposito = 'color:red;';

		$monto_diferencia = number_format(($nuevo_balance - $monto_balance_deposito), 2, '.', '');

	}elseif($nombre_balance=='BALANCE RETIRABLE'){

		$new_balance_retiro_disponible = $nuevo_balance;
		$css_balance_retiro_disponible = 'color:red;';

		$monto_diferencia = number_format(($nuevo_balance - $monto_balance_retiro_disponible), 2, '.', '');

	}elseif($nombre_balance=='BALANCE BONO AT'){

		$new_balance_dinero_at = $nuevo_balance;
		$css_balance_dinero_at = 'color:red;';

		$monto_diferencia = number_format(($nuevo_balance - $monto_balance_dinero_at), 2, '.', '');
	}
	
	$body = "Se realizó una modificación del balance del cliente <b>" . $cliente_nombre . "<b><br>" .
			"ID Cliente: " . $id_cliente . " <br>" .
			"Tipo de Documento: " . $tipo_doc . " <br>" .
			"Número de Documento: " . $num_doc_cliente . " <br>" .
			"Cliente: " . $cliente_nombre . " <br>" .
			"<br><br>" .
			"<table border='1' cellpadding='5' cellspacing='0'>" .
			"	<thead>" .
			"		<tr>" .
			"			<td style='font-weight: bold;" . $css_balance . "'>Balance General Anterior</td>" .
			"			<td style='font-weight: bold; text-align: right; " . $css_balance . "'>" . $monto_balance . "</td>" .
			"		</tr>" .
			"		<tr>" .
			"			<td style='font-weight: bold;" . $css_balance_retiro_disponible . "'>Balance Retirable Anterior</td>" .
			"			<td style='font-weight: bold; text-align: right; " . $css_balance_retiro_disponible . "'>" . $monto_balance_retiro_disponible . "</td>" .
			"		</tr>" .
			"		<tr>" .
			"			<td style='font-weight: bold;" . $css_balance_deposito . "'>Balance No Retirable Anterior</td>" .
			"			<td style='font-weight: bold; text-align: right; " . $css_balance_deposito . "'>" . $monto_balance_deposito . "</td>" .
			"		</tr>" .
			"		<tr>" .
			"			<td style='font-weight: bold;" . $css_balance_dinero_at . "'>Balance Bono AT Anterior</td>" .
			"			<td style='font-weight: bold; text-align: right; " . $css_balance_dinero_at . "'>" . $monto_balance_dinero_at . "</td>" .
			"		</tr>" .
			"		<tr>" .
			"			<tdstyle='font-weight: bold;'>Tipo Balance a Editar</tdstyle=>" .
			"			<td style='font-weight: bold; color:red;'>" . $nombre_balance . "</td>" .
			"		</tr>" .
			"		<tr>" .
			"			<td>Monto Diferencia</td>" .
			"			<td style='font-weight: bold; text-align: right; color:red;'>" . $monto_diferencia . "</td>" .
			"		</tr>" .
			"		<tr>" .
			"			<td>Usuario Modifica</td>" .
			"			<td style='font-weight: bold;'>" . $usuario_nombre . "</td>" .
			"		</tr>" .
			"		<tr>" .
			"			<td>Observación</td>" .
			...
			"		</tr>" .
			"		<tr>" .
			"			<td style='font-weight: bold;" . $css_balance . "'>Balance General Actual</td>" .
			"			<td style='font-weight: bold; text-align: right; " . $css_balance . "'>" . $new_balance . "</td>" .
			"		</tr>" .
			"		<tr>" .
			"			<td style='font-weight: bold;" . $css_balance_retiro_disponible . "'>Balance Retirable Actual</td>" .
			"			<td style='font-weight: bold; text-align: right; " . $css_balance_retiro_disponible . "'>" . $new_balance_retiro_disponible . "</td>" .
			"		</tr>" .
			"		<tr>" .
			"			<td style='font-weight: bold;" . $css_balance_deposito . "'>Balance No Retirable Actual</td>" .
			"			<td style='font-weight: bold; text-align: right; " . $css_balance_deposito . "'>" . $new_balance_deposito . "</td>" .
			"		</tr>" .
			"		<tr>" .
			"			<td style='font-weight: bold;" . $css_balance_dinero_at . "'>Balance Bono AT Actual</td>" .
			"			<td style='font-weight: bold; text-align: right;" . $css_balance_dinero_at . "'>" . $new_balance_dinero_at . "</td>" .
			"		</tr>" .
			"	</thead>" .
			"</table>";
	...
	$mail = [
	    "subject" => "TELESERVICIOS - MANT BILLETERA TLS -".$num_doc_cliente,
	    "body" => $body,
	    "cc" => $cc,
	    "bcc" => $bcc,
	    "attach" => [
...
if ($_POST["accion"]==="listar_motivos_balance") {
...
if ($_POST["accion"]==="guardar_motivo_edit_balance") {
	$tipo_edit = $_POST["tipo_edit"];
	$nombre = $_POST["nombre"];

	$insert_reason= " 
		INSERT INTO tbl_televentas_motivo_balances (
			motivo, 
			tipo_balance, 
			estado
		) VALUES (
	...
	$mysqli->query($insert_reason);

	if ($mysqli->error) {
		$result["insert_reason_error"] = $mysqli->error;
		$result["http_code"] = 400;
		$result["status"] = "Error al registrar los datos";
	}else{
		$result["http_code"] = 200;
