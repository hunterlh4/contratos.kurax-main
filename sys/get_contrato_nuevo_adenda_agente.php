...
if (isset($_POST["accion"]) && $_POST["accion"]==="obtener_contratos") {
	...
	$query = "SELECT c.contrato_id, c.fecha_inicio_agente, c.nombre_agente, c.c_costos, CONCAT(p.nombre,' ',IFNULL(p.apellido_paterno,''),' ',IFNULL(p.apellido_paterno,'')) AS supervisor
	FROM cont_contrato c
	LEFT JOIN tbl_usuarios u ON c.persona_responsable_id = u.id
	...
	// if (!Empty($supervisor) && $supervisor != "0") {
		while ($li = $list_query->fetch_assoc()) {
			$date = new DateTime($li['fecha_inicio_agente']);
			$fecha_inicio = $date->format('Y-m-d');
			array_push($list,array(
				'id' => $li['contrato_id'],
				'nombre' => $li['supervisor'].' | '.$fecha_inicio.' - '.$li['c_costos'].' | '.$li['nombre_agente'],
			));
		}
	// }
	...
	<div class="panel">
		...
		<div class="panel-body">
			<form>
				...
				<div class="col-xs-12 col-md-12 col-lg-12">
					<button type="button" class="btn btn-success" id="sec_nuevo_modal_guardar_archivo" onclick="anadirArchivo_adenda_agente()">
