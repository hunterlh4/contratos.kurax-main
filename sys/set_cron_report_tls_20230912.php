...
function listar_transacciones($fecha_inicial, $fecha_final)
{
    ...
    $query_teleservicios_ventas_x_producto = "
        SELECT 
            ca.Fecha,
            case when ca.producto_id = 1 then 'Apuestas Deportivas BC'
                 when ca.producto_id = 2 then 'Juevos Virtuales'
         ...
         IFNULL(sum(num_tickets_ganados_pagados)/sum(num_tickets_ganados), 0) PorcentajeTksPagado
    ...
    /*else {
        $value = [
            ...
            'descripcion' => 'Apuestas deportivas - Altenar',
            'tickets' => 0,
            'apostado' => 0,
            'promedio' => 0,
            'tickets_calculados' => 0,
            'calculado' => 0,
            'resultado' => 0,
            'hold' => 0,
            'TicketsPagado' => 0,
            'Pagado' => 0,
            'PorcentajeTksPagado' => 0
        ];
        $list_teleservicios_apuestas_calimaco_tabla_ventas_x_producto[] = $value;
    }*/

    if(count($list_teleservicios_tabla_ventas_x_producto) > 0 && count($list_teleservicios_apuestas_calimaco_tabla_ventas_x_producto) > 0) {
        array_splice($list_teleservicios_tabla_ventas_x_producto, 0, 0, $list_teleservicios_apuestas_calimaco_tabla_ventas_x_producto);
    }


    $query_teleservicios_apuestas_torito_ventas_x_producto = "
        select 
            Fecha
            ,descripcion
        ,sum(tickets) tickets
        ,sum(MontoApostado) apostado
        ,sum(tickets) / sum(MontoApostado) promedio
        ,sum(TicketsGanado) tickets_calculados
        ,sum(MontoGanado) calculado
        ,sum(MontoApostado) - sum(MontoGanado) resultado
        ,(sum(MontoApostado) - sum(MontoGanado)) / sum(MontoApostado) hold
        ,sum(TicketsGanado) TicketsPagado
        ,sum(MontoGanado) Pagado
        ,sum(TicketsGanado) / sum(TicketsGanado) PorcentajeTksPagado
        from(
        SELECT ca.date as Fecha,
        'Torito' as descripcion
        ,sum(case when ca.id_torito_tipo_transaccion IN (1,4) then 1 else 0 end) AS tickets
        ,sum(case when ca.id_torito_tipo_transaccion IN (1,4) then ca.amount else 0 end) AS MontoApostado
        ,sum(case when ca.id_torito_tipo_transaccion IN (2,5) then 1 else 0 end) AS TicketsGanado
        ,sum(case when ca.id_torito_tipo_transaccion IN (2,5) then ca.amount else 0 end) AS MontoGanado
        FROM wwwapuestatotal_gestion.tbl_torito_transaccion ca
        INNER JOIN wwwapuestatotal_gestion.tbl_locales l ON ca.cc_id = l.cc_id 
        WHERE ca.date >= '" . $fecha_inicial . " 00:00:00'  
        AND ca.date < '" . $fecha_final . " 00:00:00' 
        and ca.status = 1
        AND l.red_id = 8
        AND ca.id_torito_tipo_transaccion IN (1,2,4,5)
        AND UPPER(l.nombre) NOT LIKE '%TEST%' 
        AND UPPER(l.nombre) NOT like '%CAPACI%'
) a
    ";

    $res_query_teleservicios_apuestas_torito_ventas_x_producto = $mysqli->query($query_teleservicios_apuestas_torito_ventas_x_producto);

    if ($mysqli->error) {
        $result["error_query_teleservicios_apuestas_torito_ventas_x_producto"] = $mysqli->error;
    }

    $list_teleservicios_apuestas_torito_tabla_ventas_x_producto = array();

    if ($res_query_teleservicios_apuestas_torito_ventas_x_producto) {
        foreach ($res_query_teleservicios_apuestas_torito_ventas_x_producto as $value) {
            $value['query'] = $query_teleservicios_apuestas_torito_ventas_x_producto;
            $list_teleservicios_apuestas_torito_tabla_ventas_x_producto[] = $value;
        }
    } else {
        $value = [
            'Fecha' => date("Y-m-d H:i:s"),
    ...
    if(count($list_teleservicios_tabla_ventas_x_producto) > 0 && count($list_teleservicios_apuestas_torito_tabla_ventas_x_producto) > 0) {
        array_splice($list_teleservicios_tabla_ventas_x_producto, count($list_teleservicios_tabla_ventas_x_producto), 0, $list_teleservicios_apuestas_torito_tabla_ventas_x_producto);
    }

    $result_temp = array();

    $result_temp['Fecha'] = date("Y-m-d H:i:s");
