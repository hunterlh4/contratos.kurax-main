...
if (isset($_POST["accion"]) && $_POST["accion"]==="obtener_adenda_detalle_contrato") {
	...
	$result["status"] = 200;
	$result["message"] = "Se ha obtenido exitosamente las adendas de Contrato de Acuerdo de Confidencialidad.";
...
if (isset($_POST["accion"]) && $_POST["accion"]==="obtener_adendas_detalle") {
	...
	if ($row_count > 0) {
		...
		while ($row = $list_query->fetch_assoc()) {
			...
			$html .= '<td>';
			$html .= '<a class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="top" title="Eliminar" onclick="sec_con_detalle_aden_acu_confidencialidad_eliminar_detalle_adenda(' . $row["id"] . ')"><i class="fa fa-trash"></i></a>';
			$html .= '</td>';
			$html .= '</tr>';
	...
	if ($row_count_otros > 0) {
		while ($row = $list_query_otros->fetch_assoc()) {
			if ($row["nombre_menu_usuario"] == 'Propietario') {
				...
				$html .= '<td rowspan="9">';
				$html .= '<a class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="top" title="Eliminar" onclick="sec_con_detalle_aden_acu_confidencialidad_eliminar_detalle_adenda(' . $row["id"] . ')"><i class="fa fa-trash"></i></a></td>';
				$html .= '</tr>';
