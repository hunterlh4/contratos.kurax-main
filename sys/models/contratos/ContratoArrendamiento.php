<?php
class ContratoArrendamiento extends Model
{
    ...
    public function registrar($data)
    {
        ...
        try {
            ...
            $query = " INSERT INTO cont_contrato(
                ...
                " . $data["abogado_id"] . ",
               NULL,
                " . $data["cargo_aprobador_id"] . ",
                '" . $data["observaciones"] . "',
                " . $data["status"] . ",
                " . $data["etapa_id"] . ",
                1, -- Estado en proceso por default
    ...
    public function obtener_formato_version_actual($tipo_contrato_id)
    {
        try {
            $query = "SELECT idformato 
                      FROM cont_formato 
                      WHERE tipo_contrato_id = :tipo_contrato_id 
                        AND STATUS = 1 
                      ORDER BY idformato DESC 
                      LIMIT 1";

            $db_query = $this->db->prepare($query);
            $db_query->bindParam(':tipo_contrato_id', $tipo_contrato_id, PDO::PARAM_INT);
            $db_query->execute();
            $resultado = $db_query->fetchAll(PDO::FETCH_ASSOC);

            if ($resultado) {
                $result['status'] = 200;
                $result['idformato'] = $resultado[0]['idformato'];
                $result['message'] = 'Última versión obtenida correctamente';
            } else {
                $result['status'] = 404;
                $result['idformato'] = null;
                $result['message'] = 'No se encontró ningún formato con ese tipo de contrato';
            }
            ...
            $result['message'] = 'Ocurrió un error al obtener el formato';
            return $result;
        }
    }
