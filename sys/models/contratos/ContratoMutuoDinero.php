<?php
class ContratoMutuoDinero extends Model
{
    ...
    public function registrar($data)
    {
        ...
        try {
            ...
            $query = " INSERT INTO cont_contrato(
                ...
                " . $data["etapa_id"] . ",
                '" . $data["ciudad"] . "',
                '" . $data["fecha_suscripcion_contrato"] . "',
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////
    public function obtener_solicitud_contrato_mutuodinero_detallado($contrato_id) {
        try {
            // Inicializar el array que contendrá toda la data
            $contratos = array();
    
            // Obtener datos generales del contrato
            $query_contrato = "SELECT c.contrato_id, c.codigo_correlativo, c.empresa_suscribe_id, c.abogado_id, c.user_created_id, c.created_at, c.fecha_suscripcion_contrato, c.fecha_aprobacion, c.estado_aprobacion, c.ciudad, rs.nombre AS empresa_suscribe, rs.num_ruc AS empresa_ruc, rs.direccion AS empresa_direccion, rs.representante_legal AS empresa_representante, u.usuario AS user_created, CONCAT(p.nombre, ' ', p.apellido_paterno, ' ', p.apellido_materno) AS abogado, car.nombre AS cargo_abogado 
            ...
            // Obtener datos del mutuo dinero
            $query_mutuodinero = "SELECT cm.mutuante_descripcion, cm.mutuatario_descripcion, cm.tasa_interes, cm.plazo_devolucion 
                                  FROM cont_mutuodinero cm 
                                  WHERE cm.idcontrato = :contrato_id";
            $stmt_mutuodinero = $this->db->prepare($query_mutuodinero);
            $stmt_mutuodinero->execute(['contrato_id' => $contrato_id]);
            $data_mutuodinero = $stmt_mutuodinero->fetchAll(PDO::FETCH_ASSOC);
    
            // Construir el array de respuesta
            $contratos = [
                'datos_generales' => $datos_generales,
                'data_mutuodinero' => $data_mutuodinero
            ];
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////
    public function registrar_mutuodinero($data)
    {
        $query = '';
        try {
            $query = "INSERT INTO cont_mutuodinero  ( 
                        idcontrato,
                        mutuante_descripcion,
                        mutuatario_descripcion,
                        tasa_interes,
                        plazo_devolucion,
                        ...
                        '" . (isset($data["mutuante_descripcion"]) ? $data["mutuante_descripcion"] : "") . "',
                        '" . (isset($data["mutuatario_descripcion"]) ? $data["mutuatario_descripcion"] : "NULL") . "',
                        '" . (isset($data["tasa_interes"]) ? $data["tasa_interes"] : "NULL") . "',
                        '" . (isset($data["plazo_devolucion"]) ? $data["plazo_devolucion"] : "NULL") . "',

                        '" . (isset($data["user_created_id"]) ? $data["user_created_id"] : "") . "',
            ...
            $result['status'] = 200;
            $result['result'] = $id;
            $result['message'] = 'Se ha registrado exitosamente el contrato de mutuo de dinero';
            return $result;
        } catch (\Throwable $th) {
            $result['status'] = 404;
            $result['result'] = $th->getMessage();
            $result['query'] = $query;
            $result['message'] = 'Ha ocurrido un error con la consulta SQL (mutuo de dinero)';
            return $result;
        }
    }
    public function registrar_contraprestacion($data)
    {
        ...
        try {
            ...
            (
            " . $data['contrato_id'] . ", 
            '" . (isset($data["moneda_id"]) ? $data["moneda_id"] : 1) . "',
            " . $data['monto'] . ",
