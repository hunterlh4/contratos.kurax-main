...
function send_email_solicitud_adenda_contrato_agente($adenda_id,$requiere_aprobacion_id)
{
	include("db_connect.php");
	include("sys_login.php");
	...
	if($requiere_aprobacion_id == 2){
		...
		$lista_correos = $correos->send_email_solicitud_adenda_contrato_agente($correos_add);
	...
	$titulo_email = "Gestion - Sistema Contratos - Nueva Solicitud de Adenda de Contrato de Agente: Código - ";
	...
	if ($row_count_otros > 0) {
		while ($row = $list_query_otros->fetch_assoc()) {
			...
			if ($row["nombre_menu_usuario"] == 'Propietario') {
				...
				$html .= '<td>' . $valores_nuevos[0]["tipo_persona"] . '</td>';
				$html .= '<td rowspan="9">';
				$html .= '<a class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="top" title="Eliminar" onclick="sec_con_nuevo_aden_agente_eliminar_detalle_adenda(' . $row["id"] . ')"><i class="fa fa-trash"></i></a></td>';
				$html .= '</tr>';
...
if (isset($_POST["accion"]) && $_POST["accion"]==="guardar_adenda") {
	...
	send_email_solicitud_adenda_contrato_agente($adenda_id, $aprobacion_obligatoria_id);
...
if (isset($_POST["accion"]) && $_POST["accion"]==="send_email_solicitud_adenda_agente") {
	$adenda_id = $_POST["adenda_id"];
	send_email_solicitud_adenda_contrato_agente($adenda_id, 2);
}
