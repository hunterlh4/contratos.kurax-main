...
if (isset($_POST["accion"]) && $_POST["accion"] === "cont_listar_mandatos") {
	...
	SELECT 
		...
		man.fecha_inicio,
		man.fecha_fin,
		man.num_alerta_vencimiento,
		man.se_enviara_alerta,
		man.idmandato,
	...
	FROM
		...
		LEFT JOIN tbl_moneda m ON ce.tipo_moneda_id = m.id
	...
	WHERE c.status IN (1,2) AND (c.etapa_id = 5 OR (c.etapa_id = 1 && c.estado_aprobacion = 1)) AND  c.tipo_contrato_id = 14
	";
	...
	while ($reg = $list_query->fetch_object()) {
		...
		if ($area_id == 2 || $area_id == 6) {
			...
			$data[] = array(
				...
				"2" => $reg->mandante,
				"3" => $reg->created_at,
				"4" => $reg->fecha_suscripcion,
				"5" => $reg->fecha_inicio,
				"6" => $reg->fecha_fin,
				"7" => ($reg->idetapa == 5) ? ' <span class="badge bg-success text-white"> Firmado</span>' : ' <span class="badge bg-warning text-white">No firmado</span>',

				"8" => '<a class="btn btn-rounded btn-primary btn-sm" 
							href="./?sec_id=contrato&amp;sub_sec_id=detalle_solicitud_mandato&id=' . $reg->contrato_id . '"
							title="Ver detalle">
							<i class="fa fa-eye"></i> Ver
						</a>' . $provision,
				"9" => ($reg->num_alerta_vencimiento == 0) ? '<button type="button" class="btn btn-primary btn-sm" title="Alerta por configurar" onclick="contrato_alerta_mandato(' . $reg->contrato_id . ')">
					  <i class="glyphicon glyphicon-bell"></i>
					</button>' : '<button type="button" class="btn btn-success btn-sm" title="Alerta configurada" 
						onclick="contrato_alerta_mandato(' . $reg->contrato_id . ')">
					  <i class="glyphicon glyphicon-bell"></i>
					</button>',
				"10" => ($reg->num_alerta_vencimiento == 0)
					? 'No existe'
					: '<input class="switch" id="checkbox_' . $reg->idmandato . '" type="checkbox" 
						data-table="cont_mandato" 
						data-id="' . $reg->idmandato . '" 
						data-col="se_enviara_alerta" 
						data-on-value="1" 
						data-off-value="0" 
						data-ignore="true" ' . ($reg->se_enviara_alerta != 0 ? 'checked' : '') . '>',
				"11" => $estado,
				"12" => $reg->codigo_correlativo
			...
			$data[] = array(
				...
				"10" => ($reg->num_alerta_vencimiento == 0) ? '<button type="button" class="btn btn-primary btn-sm" title="Alerta por configurar" onclick="contrato_alerta_mandato(' . $reg->contrato_id . ')">
					  <i class="glyphicon glyphicon-bell"></i>
					</button>' : '<button type="button" class="btn btn-success btn-sm" title="Alerta configurada" 
						onclick="contrato_alerta_mandato(' . $reg->contrato_id . ')">
					  <i class="glyphicon glyphicon-bell"></i>
...
if (isset($_POST["accion"]) && $_POST["accion"] === "obtener_dato_contrato") {
	...
	$query = "SELECT
			...
			FROM
				cont_contrato c
				INNER JOIN cont_mandato ce ON c.contrato_id = ce.idcontrato  
			WHERE
				c.STATUS = 1   
...
if (isset($_POST["accion"]) && $_POST["accion"] === "actualizar_alerta_contrato") {
	...
	$query_update = "UPDATE cont_mandato 
						SET num_alerta_vencimiento = '" . $numAlerta . "', 
							se_enviara_alerta = '1'  
						WHERE idmandato = '" . $condicion_economica_id . "' 
					";
