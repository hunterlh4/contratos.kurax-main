...
if (isset($_POST["accion"]) && $_POST["accion"] === "cont_listar_locales") {
	...
	WHERE c.status IN (1,2) AND (c.etapa_id = 5 OR (c.etapa_id = 1 && c.estado_aprobacion = 1))
	";
	...
	while ($reg = $list_query->fetch_object()) {
		...
		if ($area_id == 2 || $area_id == 6) {
			...
			$data[] = array(
				...
				"1" => $reg->ubicacion,
				"2" => number_format($reg->monto_renta, 2, '.', ','),
				"3" => $reg->tipo_moneda,
				"4" => $reg->created_at,
				...
				"9" => '<a class="btn btn-rounded btn-primary btn-sm" 
							href="./?sec_id=contrato&amp;sub_sec_id=detalle_solicitudV2&id=' . $reg->contrato_id . '"
							title="Ver detalle">
							<i class="fa fa-eye"></i> Ver
						</a>' . $provision,
				"10" => ($reg->num_alerta_vencimiento == 0) ? '<button type="button" class="btn btn-primary btn-sm" title="Alerta por configurar" onclick="contrato_alerta(' . $reg->contrato_detalle_id . ')">
					  <i class="glyphicon glyphicon-bell"></i>
					</button>' : '<button type="button" class="btn btn-success btn-sm" title="Alerta configurada" 
						onclick="contrato_alerta(' . $reg->contrato_detalle_id . ')">
					  <i class="glyphicon glyphicon-bell"></i>
					</button>',
				"11" => ($reg->num_alerta_vencimiento == 0) ? 'No existe' : '<input class="switch" id="checkbox_' . $reg->condicion_economica_id . '" type="checkbox" data-table="cont_condicion_economica" data-id="' . $reg->condicion_economica_id . '" data-col="se_enviara_alerta" data-on-value="1" data-off-value="0" data-ignore="true"' . ($reg->se_enviara_alerta != 0 ? 'checked' : '') . '>',
				"12" => $estado
			);
		} else {
			if ($reg->estado_resolucion == 2) {
			...
			// 	case 3: $bg_estado_solicitud = 'bg-warning'; break;
			// }
			// 
			...
			$data[] = array(
				...
				"11" => ($reg->num_alerta_vencimiento == 0) ? '<button type="button" class="btn btn-primary btn-sm" title="Alerta por configurar" onclick="contrato_alerta(' . $reg->contrato_detalle_id . ')">
					  <i class="glyphicon glyphicon-bell"></i>
					</button>' : '<button type="button" class="btn btn-success btn-sm" title="Alerta configurada" 
						onclick="contrato_alerta(' . $reg->contrato_detalle_id . ')">
					  <i class="glyphicon glyphicon-bell"></i>
					</button>',
				"12" => ($reg->num_alerta_vencimiento == 0) ? 'No existe' : '<input class="switch" id="checkbox_' . $reg->condicion_economica_id . '" type="checkbox" data-table="cont_condicion_economica" data-id="' . $reg->condicion_economica_id . '" data-col="se_enviara_alerta" data-on-value="1" data-off-value="0" data-ignore="true"' . ($reg->se_enviara_alerta != 0 ? 'checked' : '') . '>',
			);
		}
	}
...
if (isset($_POST["accion"]) && $_POST["accion"] === "obtener_dato_contrato") {
	...
	$contrato_detalle_id = $_POST["parametro"];

	$query = "	SELECT c.contrato_id, ce.condicion_economica_id, c.nombre_tienda, i.ubicacion, ce.fecha_inicio, ce.fecha_fin, 
					ce.num_alerta_vencimiento, ce.se_enviara_alerta
				FROM cont_contrato c 
					...
					ON c.contrato_id = i.contrato_id AND i.contrato_detalle_id = '" . $contrato_detalle_id . "'
				WHERE c.status = 1 
...
if (isset($_POST["accion"]) && $_POST["accion"] === "actualizar_alerta_contrato") {
	...
	$query_update = "UPDATE cont_condicion_economica 
						SET num_alerta_vencimiento = '" . $numAlerta . "', 
							se_enviara_alerta = '1'  
						WHERE condicion_economica_id = '" . $condicion_economica_id . "' 
					";
