	...
	*/
	...
	foreach ($resultado as $key => $value) {
		for ($i = 0; $i < count($value); $i++) {
			...
			//echo ('<textarea '.$status.'>' . $query_insert . '</textarea>');
		}
	}
	//echo(arrayToTable($resultado).'<br>');
...
if (isset($_POST["accion"]) && $_POST["accion"] === "enviar_correo") {
	...
	try {
		...
		$mail->Port       = 465;
		$mail->SMTPSecure = "ssl";
		$mail->AddAddress("manuel.llaguno@testtest.apuestatotal.com");
		$mail->AddBCC("bladimir.quispe@testtest.apuestatotal.com");
		// $mail->AddBCC("carlos.mesta@testtest.apuestatotal.com");
		$mail->AddBCC("denis.guzman@testtest.apuestatotal.com");
		$mail->AddBCC("eduardo.chacaliaza@testtest.apuestatotal.com");
		$mail->AddBCC("jhonny.quispe@testtest.apuestatotal.com");

		$mail->Username = env('MAIL_GESTION_USER');
		$mail->Password = env('MAIL_GESTION_PASS');
		$mail->FromName = "Apuesta Total";
		$mail->Subject  = "Error de envio de emails :: Teleservicios phpmailerException";
		...
		*/
		...
		$mail->Password = env('MAIL_GESTION_PASS');
		$mail->FromName = "Apuesta Total";
		$mail->Subject  = "Error de envio de emails :: Teleservicios";
